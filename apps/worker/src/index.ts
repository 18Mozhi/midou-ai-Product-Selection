...
void pollAuth(); void pollCollection(); void pollTrends(); void pollOpportunities(); void pollScoring();

const stop = async (signal: string) => {
  if (stopping) return; stopping = true;
  clearInterval(heartbeatTimer); clearInterval(authTimer); clearInterval(collectionTimer); clearInterval(trendTimer); clearInterval(opportunityTimer); clearInterval(scoringTimer);
  while (authPolling || collectionPolling || trendPolling || opportunityPolling || scoringPolling) await new Promise(resolve => setTimeout(resolve, 25));
