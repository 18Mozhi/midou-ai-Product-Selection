...
let stopping = false, authPolling = false, collectionPolling = false, trendPolling = false, opportunityPolling = false, scoringPolling = false, profitPolling = false, competitorPolling = false, sourcingPolling = false;

const heartbeat = () => console.log(JSON.stringify({ service: 'product-scout-worker', status: stopping ? 'stopping' : 'idle', worker_id: config.identity.workerId, registered_sources: registry.describe().map(item => item.key), trend_projection: 'registered', opportunity_refresh: 'registered', opportunity_scoring: 'registered', opportunity_profit: 'registered', competitor_monitor: 'registered', sourcing_projection: 'registered', config_fingerprint: config.configFingerprint, observed_at: new Date().toISOString() }));
...
void pollAuth(); void pollCollection(); void pollTrends(); void pollOpportunities(); void pollScoring(); void pollProfit(); void pollCompetitors(); void pollSourcing();

const stop = async (signal: string) => {
  if (stopping) return; stopping = true;
  clearInterval(heartbeatTimer); clearInterval(authTimer); clearInterval(collectionTimer); clearInterval(trendTimer); clearInterval(opportunityTimer); clearInterval(scoringTimer); clearInterval(profitTimer); clearInterval(competitorTimer); clearInterval(sourcingTimer);
  while (authPolling || collectionPolling || trendPolling || opportunityPolling || scoringPolling || profitPolling || competitorPolling || sourcingPolling) await new Promise(resolve => setTimeout(resolve, 25));
