...
let stopping = false, authPolling = false, collectionPolling = false, trendPolling = false;

const heartbeat = () => console.log(JSON.stringify({ service: 'product-scout-worker', status: stopping ? 'stopping' : 'idle', worker_id: config.identity.workerId, registered_sources: registry.describe().map(item => item.key), trend_projection: 'registered', config_fingerprint: config.configFingerprint, observed_at: new Date().toISOString() }));
...
void pollAuth(); void pollCollection(); void pollTrends();

const stop = async (signal: string) => {
  if (stopping) return; stopping = true;
  clearInterval(heartbeatTimer); clearInterval(authTimer); clearInterval(collectionTimer); clearInterval(trendTimer);
  while (authPolling || collectionPolling || trendPolling) await new Promise(resolve => setTimeout(resolve, 25));
