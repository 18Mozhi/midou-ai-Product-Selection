...
export class MySqlCollectionTaskWorkerRepository {
  ...
  async recoverExpired(now: Date, actorId = "collection-scheduler") {
    ...
    try {
      ...
      for (const row of rows) {
        const classification = classifyCollectionFailure("timeout", Number(row.attempt_count)),
          next =
            classification.status === "retry_scheduled"
              ? retryAvailableAt(Number(row.attempt_count), now, this.random())
              : now;
        await c.query(
          statement(
        ...
        if (classification.status === "dead_letter") await this.dead(c, row, "timeout", now);
        await this.event(
          c,
          row,
          row.status,
      ...
      for (const row of overflowRows) {
        ...
        await this.dead(c, row, "collection_attempt_overflow", now);
        await this.event(
          c,
          row,
          "queued",
  ...
  async complete(
    ...
    try {
      await c.beginTransaction();
      const row = await this.lock(c, task);
      for (const stage of ["parsing", "validating", "persisted"] as const) {
        ...
        await this.event(c, row, row.status, stage, row.lease_owner, "worker", {}, now);
        row.status = stage;
      }
  ...
  async fail(task: ClaimedCollectionTask, error: CollectionExecutionError, now: Date) {
    ...
    try {
      ...
      if (classification.status === "retry_scheduled")
        available = retryAvailableAt(task.attemptCount, now, this.random());
      ...
      if (classification.status === "dead_letter") await this.dead(c, row, error.code, now);
      await this.event(
        c,
        row,
        row.status,
  ...
  private async lock(c: PoolConnection, task: ClaimedCollectionTask) {
    const [rows] = await c.query<RowDataPacket[]>(
  ...
  private async dead(c: PoolConnection, row: RowDataPacket, errorCode: string, now: Date) {
