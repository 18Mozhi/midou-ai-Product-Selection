...
export class MySqlTrendProjectionWorker {
  ...
  private async project(job: ClaimedJob) {
    ...
    try {
      ...
      if (insert.affectedRows) {
        ...
        await this.evaluateMonitoringRules(c, job, topicId, normalizedTitle, providerContext.market, providerContext.language, now);
        stage = 'event_insert';
  ...
  private async evaluateMonitoringRules(c:PoolConnection,job:ClaimedJob,topicId:string,title:string,market:string,language:string,now:Date){
    const[rows]=await c.query<RowDataPacket[]>("SELECT id,include_keywords_json,negative_keywords_json,market,language,category,created_by FROM trend_monitoring_rules WHERE organization_id=? AND workspace_id=? AND status='enabled' FOR UPDATE",[job.organizationId,job.workspaceId]);
    for(const row of rows){const include=typeof row.include_keywords_json==='string'?JSON.parse(row.include_keywords_json):row.include_keywords_json,negative=typeof row.negative_keywords_json==='string'?JSON.parse(row.negative_keywords_json):row.negative_keywords_json;const matched=Array.isArray(include)&&include.some((keyword:unknown)=>title.includes(String(keyword).normalize('NFKC').toLocaleLowerCase('en-US')))&&!((Array.isArray(negative)?negative:[]).some((keyword:unknown)=>title.includes(String(keyword).normalize('NFKC').toLocaleLowerCase('en-US'))))&&(!row.market||String(row.market).toUpperCase()===market)&&(!row.language||String(row.language)===language)&&!row.category;if(matched){await c.query("INSERT IGNORE INTO trend_topic_follows (id,organization_id,workspace_id,topic_id,user_id,created_at) VALUES (?,?,?,?,?,?)",[randomUUID(),job.organizationId,job.workspaceId,topicId,row.created_by,now]);await this.event(c,job,'trend.monitoring_rule.matched','trend_topic',topicId,{rule_id:String(row.id),market,language});}}
    if(rows.length)await c.query("UPDATE trend_monitoring_rules SET last_evaluated_at=?,updated_at=updated_at WHERE organization_id=? AND workspace_id=? AND status='enabled'",[now,job.organizationId,job.workspaceId]);
  }
