...
import {
  BusinessTaskError,
  type BusinessTaskRepository,
} from "./business-task-service.js";
...
export class MySqlBusinessTaskRepository implements BusinessTaskRepository {
  ...
  async list(i: any) {
    ...
    const [count] = await this.pool.query<RowDataPacket[]>(
      ...
      [rows] = await this.pool.query<RowDataPacket[]>(
        `SELECT * FROM tasks WHERE ${where.join(" AND ")} ORDER BY CASE priority WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'normal' THEN 3 ELSE 4 END,due_at IS NULL,due_at,created_at DESC LIMIT ? OFFSET ?`,
  ...
  async detail(i: any) {
    ...
    const [comments] = await this.pool.query<RowDataPacket[]>(
        "SELECT id,body,created_by,created_at FROM task_comments WHERE task_id=? AND organization_id=? AND workspace_id=? ORDER BY created_at,id",
        [i.taskId, i.organizationId, i.workspaceId],
      ),
      [events] = await this.pool.query<RowDataPacket[]>(
        "SELECT id,event_type,actor_id,payload_json,created_at FROM task_events WHERE task_id=? AND organization_id=? AND workspace_id=? ORDER BY created_at,id",
        [i.taskId, i.organizationId, i.workspaceId],
      );
    return {
      ...this.task(rows[0]),
  ...
  async create(i: any) {
    ...
    try {
      ...
      await c.query(
        "INSERT INTO tasks (id,organization_id,workspace_id,title,description,status,priority,assignee_id,source_type,source_ref_id,due_at,completed_at,created_by,version,created_at,updated_at) VALUES (?,?,?,?,?,'todo',?,?,'manual',NULL,?,NULL,?,1,?,?)",
        [
          i.id,
          i.organizationId,
          i.workspaceId,
  ...
  private save(
    ...
    return c.query(
      "INSERT INTO task_operations (id,actor_id,route_key,idempotency_key,task_id,result_json,created_at) VALUES (?,?,?,?,?,?,?)",
      [
        randomUUID(),
        i.actorId,
        i.route,
  ...
  private async record(
    ...
    await c.query(
      "INSERT INTO audit_logs (id,organization_id,workspace_id,actor_id,action,resource_type,resource_id,request_id,trace_id,metadata_json,occurred_at,schema_version) VALUES (?,?,?,?,?,'task',?,?,?,?,?,1)",
      [
        randomUUID(),
        i.organizationId,
        i.workspaceId,
