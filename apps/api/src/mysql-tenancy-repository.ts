...
import {
  ...
  type Workspace,
} from "@scoutops/tenancy";

const organizationColumns =
  "id,name,slug,status,timezone,data_retention_days,default_workspace_id,created_by,version,created_at,updated_at";
const workspaceColumns =
  "id,organization_id,name,slug,status,created_by,version,created_at,updated_at";
const membershipColumns =
  "id,organization_id,user_id,status,joined_at,version,created_at,updated_at";
...
const auditValues = (event: TenancyAuditEvent) => [
  event.id,
...
export class MySqlTenancyRepository implements TenancyRepository {
  ...
  async provision(input: {
    ...
    try {
      await withTransaction(this.pool, async (connection) => {
        ...
        await connection.query("UPDATE organizations SET default_workspace_id=? WHERE id=?", [
          w.id,
          o.id,
        ]);
        await connection.query(
        ...
        await connection.query(
          "INSERT INTO tenancy_audit_events (id,organization_id,workspace_id,actor_id," +
            "action,resource_type,resource_id,request_id,trace_id,occurred_at,schema_version) VALUES " +
            "(?,?,?,?,?,?,?,?,?,?,?)",
          auditValues(input.audit),
        );
      });
    } catch (error) {
  ...
  async listOrganizations(userId: string) {
    ...
    return rows.map((row) => ({
      organization: asOrganization(row),
      membership: asMembership({
  ...
  async findActiveMembership(userId: string, organizationId: string) {
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT ${membershipColumns} FROM memberships WHERE user_id=? AND organization_id=? AND status='active' LIMIT 1`,
      [userId, organizationId],
    );
    return rows[0] ? asMembership(rows[0]) : null;
  }
  async findOrganization(id: string) {
    ...
    return rows[0] ? asOrganization(rows[0]) : null;
  }
  async findWorkspace(id: string) {
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT ${workspaceColumns} FROM workspaces WHERE id=? LIMIT 1`,
      [id],
    );
    return rows[0] ? asWorkspace(rows[0]) : null;
  }
  async listWorkspaces(organizationId: string) {
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT ${workspaceColumns} FROM workspaces WHERE organization_id=? ORDER BY name,id`,
      [organizationId],
    );
  ...
  async listTeams(organizationId: string) {
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT ${teamColumns} FROM teams WHERE organization_id=? ORDER BY name,id`,
      [organizationId],
    );
  ...
  async saveContext(context: SessionContext, audit: TenancyAuditEvent) {
    await withTransaction(this.pool, async (connection) => {
      await connection.query(
        "INSERT INTO user_session_contexts (session_id,user_id,organization_id,workspace_id," +
          "selected_at) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE user_id=VALUES(user_id)," +
          "organization_id=VALUES(organization_id),workspace_id=VALUES(workspace_id)," +
