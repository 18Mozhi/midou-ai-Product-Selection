...
export interface CapacityBoundaryPolicy {
...
export interface CapacityBoundaryRepositoryContract {
  ...
  attestDrill(input: {
    ...
    reason: string;
    now: Date;
  }): Promise<{ status: "verified"; observed_at: string }>;
}
export class CapacityBoundaryError extends Error {
  constructor(
...
export function evaluateCapacityBoundary(
  snapshot: CapacityBoundarySnapshot,
  policy: CapacityBoundaryPolicy,
  now = new Date(),
  ...
  if (snapshot.measured_concurrency < 20) {
    const expectedNext =
      snapshot.measured_concurrency === 5 ? 10 : snapshot.measured_concurrency === 10 ? 20 : null;
    if (
      snapshot.boundary_stop_reason !== "next_stage_gate_failed" ||
      snapshot.failed_next_concurrency !== expectedNext ||
      !snapshot.failed_next_code
    )
      blocked(
    ...
    snapshot.boundary_stop_reason !== "planning_ceiling_reached" ||
    snapshot.failed_next_concurrency !== null ||
    snapshot.failed_next_code !== null
  )
    blocked(
...
export class CapacityBoundaryService {
  constructor(
    private readonly repository: CapacityBoundaryRepositoryContract,
  ...
  async read(input: { actorId: string; requestId: string; traceId: string }) {
    ...
    return {
      ...
      boundary: {
        ...
        capacity_claim:
          evaluation.state === "blocked" ? "unverified" : "measured_single_host_limited",
        stop_reason: snapshot.boundary_stop_reason,
        failed_next_concurrency: snapshot.failed_next_concurrency,
        failed_next_code: snapshot.failed_next_code,
      },
      performance: {
      ...
      resource: {
        load_basis_points: snapshot.load_basis_points,
        available_memory_mb: snapshot.available_memory_mb,
        free_disk_mb: snapshot.free_disk_mb,
      },
      resilience: {
        archive_verified: snapshot.archive_verified,
        recovery_verified: snapshot.recovery_verified,
      },
      degradation: { mode, actions: evaluation.findings.map((item) => item.action_hint) },
  ...
  async attestDrill(input: {
    ...
    if (input.kind !== "archive_recovery")
      throw new CapacityBoundaryError(
        "capacity_drill_kind_invalid",
        400,
        "仅允许核验 archive_recovery 演练。",
      );
    if (
      ...
      throw new CapacityBoundaryError(
        "capacity_drill_reason_invalid",
        400,
        "提供 2–500 字符演练原因。",
      );
    const snapshot = await this.repository.snapshot(this.now());
    if (!snapshot || !snapshot.archive_verified || !snapshot.recovery_verified)
      throw new CapacityBoundaryError(
        "capacity_drill_not_verified",
        409,
        "先在宝塔有限任务中完成归档与隔离恢复事实核验。",
      );
    return this.repository.attestDrill({
