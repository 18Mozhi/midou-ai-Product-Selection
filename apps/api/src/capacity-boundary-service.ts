...
export interface CapacityBoundarySnapshot{measured_concurrency:number;read_p95_ms:number;write_p95_ms:number;error_rate_basis_points:number;async_lag_seconds:number;load_basis_points:number;available_memory_mb:number;free_disk_mb:number;archive_verified:boolean;recovery_verified:boolean;observed_at:string}
...
export function evaluateCapacityBoundary(snapshot:CapacityBoundarySnapshot,policy:CapacityBoundaryPolicy,now=new Date()):CapacityBoundaryEvaluation{
  const findings:CapacityBoundaryFinding[]=[],blocked=(code:string,action_hint:string)=>findings.push({code,severity:"blocked",action_hint}),warning=(code:string,action_hint:string)=>findings.push({code,severity:"warning",action_hint});
  ...
  if(snapshot.measured_concurrency<1)blocked("capacity_measurement_missing","必须至少完成一个受控并发档位，禁止用规划值代替实测。");
...
export class CapacityBoundaryService{
  ...
  async attestDrill(input:{actorId:string;requestId:string;traceId:string;idempotencyKey:string;kind:unknown;reason:unknown}){
    ...
    const snapshot=await this.repository.snapshot(this.now());if(!snapshot||!snapshot.archive_verified||!snapshot.recovery_verified)throw new CapacityBoundaryError("capacity_drill_not_verified",409,"先在宝塔有限任务中完成归档与隔离恢复事实核验。");
