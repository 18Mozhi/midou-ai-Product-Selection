...
import type { ProviderDefinitionInput } from "@scoutops/contracts";
...
export interface ProviderRegistryRouteOptions {
...
export function registerProviderRegistryRoutes(
  app: FastifyInstance,
  o: ProviderRegistryRouteOptions,
) {
  ...
  app.get("/api/v1/platform/providers", async (r, reply) => {
    await actor(r);
    reply.header("cache-control", "private, no-store");
    return envelope(await o.service.list(), r);
  });
  app.post("/api/v1/platform/providers", async (r, reply) => {
    const actorId = await write(r),
      data = await o.service.create(r.body as ProviderDefinitionInput, {
        actorId,
  ...
  app.put("/api/v1/platform/providers/:providerId", async (r) => {
    const actorId = await write(r),
      body = r.body as ProviderDefinitionInput & { expected_version: number },
      providerId = (r.params as { providerId: string }).providerId;
    const { expected_version, ...value } = body;
    return envelope(
      await o.service.update(providerId, value, expected_version, {
        actorId,
        idempotencyKey: requireIdempotencyKey(r),
        ...ids(r),
      }),
