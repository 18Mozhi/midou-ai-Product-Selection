...
export interface CrawlerProfileRuntime {
  id: string;
  code: string;
  name: string;
  provider_id: string;
...
export interface BrowserCollectionAssignment {
  job: {
    ...
    collection_subquery_id: string;
    provider_id: string;
    provider_code: string;
    execution_request: Record<string, unknown>;
  };
  run: CrawlerRunSummary;
  profile: { id: string; locale: string; timezone: string };
  credential: {
    asset_id: string;
    asset_version: number;
    kind: "browser_profile" | "cookie_bundle";
    key_version: string;
...
export interface CrawlerRuntimeRepository {
  ...
  ): Promise<{ run: CrawlerRunSummary; replayed: boolean }>;
  heartbeat(input: {
    runId: string;
    profileId: string;
    leaseTokenHash: string;
  ...
  finish(input: {
    ...
    traceId: string;
    now: Date;
    status: Exclude<CrawlerRuntimeStatus, "running">;
    pageCount: number;
    itemCount: number;
    detailCount: number;
    durationMs: number;
  ...
  recoverExpired(
    input: RuntimeContext & { now: Date; idempotencyKey: string },
  ...
  ): Promise<BrowserCollectionAssignment | null>;
  heartbeatJob(
    input: RuntimeContext & {
      jobId: string;
      runId: string;
...
export class CrawlerRuntimeError extends Error {
  constructor(
...
export class CrawlerRuntimeService {
  ...
  async acquire(input: AcquireInput) {
    if (
      ...
      throw new CrawlerRuntimeError("crawler_scope_invalid", 400, "刷新组织、工作区与档案后重试。");
    ...
    if (
      ...
      throw new CrawlerRuntimeError("crawler_lease_ttl_invalid", 400, "租约时长应为 30–600 秒。");
    const now = this.now(),
      lease = createLeaseToken(),
      result = await this.repository.acquire({
        ...input,
        runId: randomUUID(),
        leaseId: lease.id,
        leaseTokenHash: hashLeaseToken(lease.token),
        now,
        expiresAt: new Date(now.getTime() + input.leaseSeconds * 1000),
      });
    return { ...result, lease_token: result.replayed ? null : lease.token };
  }
  async heartbeat(
    input: {
      runId: string;
    ...
    if (
      ...
      throw new CrawlerRuntimeError(
        "crawler_heartbeat_invalid",
        400,
        "使用当前运行的有效租约并保持 30–600 秒心跳时长。",
      );
    const now = this.now();
  ...
  async finish(
    ...
    if (
      ...
      (input.errorCode !== null && !/^[A-Za-z0-9._:-]{1,80}$/.test(input.errorCode))
    )
      throw new CrawlerRuntimeError(
        "crawler_result_invalid",
        400,
        "按浏览器运行结果合同提交计数、状态与错误码。",
      );
  ...
  async acquireJob(input: { leaseOwner: string; leaseSeconds: number } & RuntimeContext) {
    ...
    return assignment ? { ...assignment, lease_token: lease.token } : null;
  }
  async heartbeatJob(
    input: {
      jobId: string;
    ...
    const now = this.now();
    await this.repository.heartbeatJob({
      ...input,
      leaseTokenHash: hashLeaseToken(input.leaseToken),
      now,
      expiresAt: new Date(now.getTime() + input.leaseSeconds * 1000),
  ...
  async finishJob(
    ...
    if (
      ...
      Buffer.byteLength(JSON.stringify(input.result)) > 2_000_000
    )
      throw new CrawlerRuntimeError(
        "crawler_result_invalid",
        400,
