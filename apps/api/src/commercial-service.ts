...
const quotas = (value: unknown) => {
  ...
  if (!entries.length || entries.some(([key, amount]) => !quotaKeys.has(key) || !Number.isSafeInteger(Number(amount)) || Number(amount) < 0 || Number(amount) > 1_000_000_000))
    throw new CommercialError("quotas_invalid", 400, "配额仅支持 collection_tasks、open_api_requests、report_exports，值为 0–1000000000。");
  return Object.fromEntries(entries.map(([key, amount]) => [key, Number(amount)]));
};

const date = (value: unknown, label: string) => {
  const parsed = new Date(String(value ?? ""));
  if (Number.isNaN(parsed.getTime())) throw new CommercialError(`${label}_invalid`, 400, "填写有效日期时间。");
  return parsed;
};
...
export class CommercialService {
  constructor(private readonly repository: CommercialRepository, private readonly limit = 100) {}
  ...
  createPlan(input: any) {
    ...
    if (!/^[a-z0-9][a-z0-9_-]{0,79}$/.test(code))
      throw new CommercialError("plan_code_invalid", 400, "使用 1–80 位小写字母、数字、下划线或连字符。");
  ...
  assign(input: any) {
    ...
    if (end <= start) throw new CommercialError("commercial_period_invalid", 400, "账期结束时间必须晚于开始时间。");
  ...
  assignmentAction(input: any) {
    ...
    return this.repository.assignmentAction({
      ...
      value: { action, expected_version: version(input.value?.expected_version), reason: reason(input.value?.reason) }
    });
  }
  ...
  adjust(input: any) {
    ...
    if (!quotaKeys.has(quotaKey) || !Number.isSafeInteger(delta) || delta === 0 || Math.abs(delta) > 1_000_000_000)
      throw new CommercialError("quota_adjustment_invalid", 400, "选择已支持计量项，并填写非零整数调整值。");
  ...
  revokeAdjustment(input: any) {
    return this.repository.revokeAdjustment({
      ...
      value: { expected_version: version(input.value?.expected_version), reason: reason(input.value?.reason) }
    });
  }
}
