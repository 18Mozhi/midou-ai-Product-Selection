...
function setSessionAndClearChallenge(reply:FastifyReply,token:string,secure:boolean){reply.header('set-cookie',[`${cookieName(secure)}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict${secure?'; Secure':''}`,`${challengeCookieName(secure)}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${secure?'; Secure':''}`]).header('cache-control','no-store');}
