...
import { AuthError, LocalAuthService, MfaService, digestOpaqueToken, normalizeEmail, type AuthContext } from '@scoutops/auth';
...
export interface AuthIdempotency {
  ...
  executeSensitive<T>(input: { scope: string; route: string; method: 'POST'|'DELETE'; key: string; requestId: string; traceId: string }, work: () => Promise<IdempotentResponse<T>>): Promise<IdempotentResponse<T>>;
}
...
const challengeCookieName = (secure: boolean) => secure ? '__Host-scoutops_mfa_challenge' : 'scoutops_mfa_challenge';
...
export function sessionToken(request: FastifyRequest, secure: boolean) {
  const token = cookieValue(request,cookieName(secure)); if (!token) throw new AuthError('session_invalid', 401, '重新登录后重试。'); return token;
}
...
function setSessionCookie(reply: FastifyReply, token: string, secure: boolean,ttlMinutes=43200) {
  reply.header('set-cookie', `${cookieName(secure)}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${Math.floor(ttlMinutes*60)}${secure ? '; Secure' : ''}`).header('cache-control','no-store');
}
...
function setChallengeCookie(reply:FastifyReply,token:string,secure:boolean){reply.header('set-cookie',`${challengeCookieName(secure)}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=600${secure?'; Secure':''}`).header('cache-control','no-store');}
function setSessionAndClearChallenge(reply:FastifyReply,token:string,secure:boolean,ttlMinutes=43200){reply.header('set-cookie',[`${cookieName(secure)}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${Math.floor(ttlMinutes*60)}${secure?'; Secure':''}`,`${challengeCookieName(secure)}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${secure?'; Secure':''}`]).header('cache-control','no-store');}
...
export function registerLocalAuthRoutes(app: FastifyInstance, options: LocalAuthRouteOptions) {
  ...
  const sensitiveWrite = async <T>(request: FastifyRequest, scope: string, route: string, method: 'POST'|'DELETE', work: () => Promise<IdempotentResponse<T>>) => options.idempotency.executeSensitive({ scope, route, method, key: requireIdempotencyKey(request), requestId: request.headers['x-request-id']!.toString(), traceId: request.headers['x-trace-id']!.toString() }, work);
  ...
  app.post('/api/v1/auth/email-verification/confirm',{schema:{body:bodySchema(['token'],{token:{type:'string',minLength:20,maxLength:512}})}},async(request,reply)=>{assertOrigin(request,options.webOrigin);const body=request.body as{token:string};const result=await write(request,`action:${digestOpaqueToken(body.token)}`,'/auth/email-verification/confirm','POST',async()=>({status:200,body:envelope({status:await options.service.verifyEmail(body.token,requestIds(request))},request)}));reply.code(result.status);return result.body;});
    app.post('/api/v1/auth/login',{schema:{body:bodySchema(['email','password'],{email:{type:'string',maxLength:254},password:{type:'string',maxLength:1024}})}},async(request,reply)=>{assertOrigin(request,options.webOrigin);const body=request.body as{email:string;password:string};const login=await options.service.login(body,requestIds(request));if('mfa_required' in login){setChallengeCookie(reply,login.challenge_token,options.secureCookie);reply.code(202);return envelope({mfa_required:true,expires_at:login.expires_at},request);}setSessionCookie(reply,login.token,options.secureCookie,options.sessionTtlMinutes);return envelope({user:login.user,session:login.session,security_setup:login.security_setup},request);});
  ...
  app.get('/api/v1/me/security-setup',async(request)=>envelope(await options.service.securitySetupStatus(sessionToken(request,options.secureCookie)),request));
  ...
  if(options.mfa){
    app.get('/api/v1/me/mfa',async(request)=>{const authenticated=await options.service.authenticate(sessionToken(request,options.secureCookie),{allowSecuritySetup:true});return envelope(await options.mfa!.status(authenticated.user.id),request);});
    app.post('/api/v1/me/mfa/totp/enrollment',{schema:{body:bodySchema(['current_password'],{current_password:{type:'string',maxLength:1024}})}},async(request,reply)=>{assertOrigin(request,options.webOrigin);const token=sessionToken(request,options.secureCookie);const authenticated=await options.service.authenticate(token,{allowSecuritySetup:true});const body=request.body as{current_password:string};const result=await sensitiveWrite(request,`session:${digestOpaqueToken(token)}`,'/me/mfa/totp/enrollment','POST',async()=>({status:201,body:envelope(await options.mfa!.startEnrollment(authenticated.user,body.current_password,requestIds(request)),request)}));reply.code(result.status);return result.body;});
    app.post('/api/v1/me/mfa/totp/confirm',{schema:{body:bodySchema(['code'],{code:{type:'string',minLength:6,maxLength:32}})}},async(request,reply)=>{assertOrigin(request,options.webOrigin);const token=sessionToken(request,options.secureCookie);const authenticated=await options.service.authenticate(token,{allowSecuritySetup:true});const body=request.body as{code:string};const result=await sensitiveWrite(request,`session:${digestOpaqueToken(token)}`,'/me/mfa/totp/confirm','POST',async()=>({status:200,body:envelope(await options.mfa!.confirmEnrollment(authenticated.user.id,body.code,requestIds(request)),request)}));reply.code(result.status);return result.body;});
    app.post('/api/v1/auth/mfa/totp/verify',{schema:{body:bodySchema(['code'],{code:{type:'string',minLength:6,maxLength:32}})}},async(request,reply)=>{assertOrigin(request,options.webOrigin);const challenge=cookieValue(request,challengeCookieName(options.secureCookie));if(!challenge)throw new AuthError('mfa_challenge_invalid',401,'重新输入邮箱和密码后重试。');const body=request.body as{code:string};let sessionTokenValue='';const result=await write(request,`mfa:${digestOpaqueToken(challenge)}`,'/auth/mfa/totp/verify','POST',async()=>{const login=await options.mfa!.verifyLogin(challenge,body.code,requestIds(request));sessionTokenValue=login.token;return{status:200,body:envelope({user:login.user,session:login.session},request)};});if(result.replayed||!sessionTokenValue)throw new AuthError('mfa_challenge_invalid',401,'登录已完成，请刷新页面或重新登录。');setSessionAndClearChallenge(reply,sessionTokenValue,options.secureCookie,options.sessionTtlMinutes);reply.code(result.status);return result.body;});
    app.delete('/api/v1/me/mfa/totp',{schema:{body:bodySchema(['current_password','code'],{current_password:{type:'string',maxLength:1024},code:{type:'string',minLength:6,maxLength:32}})}},async(request,reply)=>{assertOrigin(request,options.webOrigin);const token=sessionToken(request,options.secureCookie);const authenticated=await options.service.authenticate(token);const body=request.body as{current_password:string;code:string};const result=await write(request,`session:${digestOpaqueToken(token)}`,'/me/mfa/totp','DELETE',async()=>{await options.mfa!.disable(authenticated.user,body.current_password,body.code,requestIds(request));return{status:204,body:null};});clearSessionCookie(reply,options.secureCookie);reply.code(result.status);return result.body;});
  }
}
