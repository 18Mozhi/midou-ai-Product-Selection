...
export function registerLocalAuthRoutes(app: FastifyInstance, options: LocalAuthRouteOptions) {
  ...
  if(options.mfa){
    app.get('/api/v1/me/mfa',async(request)=>{const authenticated=await options.service.authenticate(sessionToken(request,options.secureCookie));return envelope(await options.mfa!.status(authenticated.user.id),request);});
