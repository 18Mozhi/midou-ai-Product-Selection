...
export function createAuthGuard(verifier: TokenVerifier, requiredScope?: string) {
  return async (request: FastifyRequest, _reply: FastifyReply): Promise<void> => {
    ...
    if (!match)
      throw new ApiError(401, "authentication_required", "需要有效登录。", "登录后重试。");
    const claims = await verifier(match[1]!);
    if (!claims) throw new ApiError(401, "session_invalid", "登录已失效。", "重新登录后重试。");
    try {
      assertOrganizationScope({
        organization_id: claims.organization_id as OrganizationId,
        ...(claims.workspace_id ? { workspace_id: claims.workspace_id as WorkspaceId } : {}),
      });
    } catch {
      throw new ApiError(
        403,
