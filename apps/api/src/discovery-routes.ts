...
import type { AuthorizationService, NavigationShell } from "@scoutops/authorization";
...
const ids = (request: FastifyRequest) => ({
  ...
  envelope = <T>(data: T, request: FastifyRequest) => ({
    data,
...
export function registerDiscoveryRoutes(app: FastifyInstance, options: DiscoveryRouteOptions) {
  ...
  const memberScope = async (request: FastifyRequest) => {
    const identity = await authenticated(request),
      resolved = await options.authorization.resolveSession(identity.user.id, identity.session.id);
  ...
  app.get(
    ...
    async (request, reply) => {
      const identity = await authenticated(request),
        ...
        guard = await options.authorization.guardNavigationShell(
          identity.user.id,
          identity.session.id,
          shell,
          ids(request),
