...
export function registerOpportunityRoutes(app:FastifyInstance,options:OpportunityRouteOptions){
 ...
 app.get('/api/v1/opportunities',async request=>{const query=request.query as Record<string,string|undefined>,page=Number(query.page??1),pageSize=Number(query.page_size??20);const result=await options.service.list({...await scope(request,'opportunity:read'),page,pageSize,...(query.q?{query:query.q}:{}),...(query.market?{market:query.market}:{}),...(query.decision_status?{decisionStatus:query.decision_status as OpportunityDecision}:{})});return envelope(result.items,request,{page,page_size:pageSize,total:result.total});});
