...
export function registerOpportunityRoutes(app: FastifyInstance, options: OpportunityRouteOptions) {
  const scope = async (
    ...
    capability: "opportunity:read" | "opportunity:decide",
  ) => {
  ...
  const write = async (request: FastifyRequest) => {
    ...
    return {
      ...(await scope(request, "opportunity:decide")),
      ...ids(request),
  ...
  app.get("/api/v1/opportunities", async (request) => {
    ...
    const result = await options.service.list({
      ...(await scope(request, "opportunity:read")),
      page,
      pageSize,
      ...
      ...(query.decision_status
        ? { decisionStatus: query.decision_status as OpportunityDecision }
        : {}),
      ...(query.coverage_status
        ? { coverageStatus: query.coverage_status as OpportunityCoverageStatus }
        : {}),
  ...
  app.post("/api/v1/opportunities", async (request, reply) => {
    const result = await options.service.create({
      ...(await write(request)),
      value: request.body as OpportunityCreateInput,
    });
    reply.code(201);
  ...
  app.post("/api/v1/opportunities/:id/decisions", async (request) => {
