...
import type { DecisionAction, OpportunityCoverageStatus, OpportunityCreateInput, OpportunityDecision, OpportunityService } from './opportunity-service.js';
...
export function registerOpportunityRoutes(app:FastifyInstance,options:OpportunityRouteOptions){
 ...
 app.get('/api/v1/opportunities',async request=>{const query=request.query as Record<string,string|undefined>,page=Number(query.page??1),pageSize=Number(query.page_size??20);const result=await options.service.list({...await scope(request,'opportunity:read'),page,pageSize,scope:query.scope==='all'?'all':'product',...(query.q?{query:query.q}:{}),...(query.market?{market:query.market}:{}),...(query.decision_status?{decisionStatus:query.decision_status as OpportunityDecision}:{}),...(query.coverage_status?{coverageStatus:query.coverage_status as OpportunityCoverageStatus}:{})});return envelope(result.items,request,{page,page_size:pageSize,total:result.total});});
