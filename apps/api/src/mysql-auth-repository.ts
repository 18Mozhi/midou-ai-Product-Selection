...
const sessionColumns =
  "id,user_id,token_hash,status,device_label,user_agent_hash,ip_hash,expires_at,last_seen_at,revoked_at,created_at";
const asUser = (row: RowDataPacket) =>
  ({
    ...row,
    must_change_password: Boolean(row.must_change_password),
    must_enroll_mfa: Boolean(row.must_enroll_mfa),
  }) as UserRecord;
...
export class MySqlAuthRepository implements AuthRepository {
  ...
  async findUserByEmail(email: string) {
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT ${userColumns} FROM users WHERE email_normalized=? LIMIT 1`,
      [email],
    );
    ...
    try {
      await this.pool.query(
        ...
        [
          ...
          user.must_enroll_mfa,
          user.security_setup_completed_at,
          user.version,
          user.created_at,
          user.updated_at,
        ],
  ...
  async discardPendingRegistration(userId: string) {
    await withTransaction(this.pool, async (connection) => {
      ...
      await connection.query("DELETE FROM users WHERE id=? AND status='pending_verification'", [
        userId,
      ]);
    });
  }
  ...
  async createActionToken(token: ActionTokenRecord) {
    await this.pool.query(
      "INSERT INTO auth_action_tokens (id,user_id,purpose,token_hash,expires_at,consumed_at,created_at) VALUES (?,?,?,?,?,?,?)",
      [
        token.id,
        token.user_id,
        token.purpose,
  ...
  async consumeActionToken(tokenHash: string, purpose: ActionTokenPurpose, now: Date) {
    return withTransaction(this.pool, async (connection) => {
      const [rows] = await connection.query<RowDataPacket[]>(
        "SELECT id,user_id,purpose,token_hash,expires_at,consumed_at,created_at FROM auth_action_tokens " +
          "WHERE token_hash=? AND purpose=? AND consumed_at IS NULL AND expires_at>? LIMIT 1 FOR " +
          "UPDATE",
        [tokenHash, purpose, now],
      );
      if (!rows[0]) return null;
      await connection.query(
        "UPDATE auth_action_tokens SET consumed_at=? WHERE id=? AND consumed_at IS NULL",
        [now, rows[0].id],
      );
  ...
  async createSession(session: SessionRecord) {
    await this.pool.query(
      "INSERT INTO user_sessions (id,user_id,token_hash,status,device_label,user_agent_hash," +
        "ip_hash,expires_at,last_seen_at,revoked_at,created_at) VALUES (?,?,?,?,?," +
        "?,?,?,?,?,?)",
      [
        session.id,
        session.user_id,
  ...
  async findSessionByTokenHash(hash: string, now: Date) {
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT ${sessionColumns} FROM user_sessions WHERE token_hash=? LIMIT 1`,
      [hash],
    );
    const row = rows[0];
    ...
    await this.pool.query("UPDATE user_sessions SET last_seen_at=? WHERE id=?", [now, row.id]);
    row.last_seen_at = now;
    return asSession(row);
  }
  async listSessions(userId: string, now: Date) {
    await this.pool.query(
      "UPDATE user_sessions SET status='expired' WHERE user_id=? AND status='active' AND expires_at<=?",
      [userId, now],
    );
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT ${sessionColumns} FROM user_sessions WHERE user_id=? ORDER BY created_at DESC`,
      [userId],
    );
  ...
  async appendSecurityEvent(event: AuthSecurityEvent) {
    await this.pool.query(
      "INSERT INTO auth_security_events (id,user_id,event_type,outcome,request_id," +
        "trace_id,ip_hash,user_agent_hash,occurred_at,schema_version) VALUES (?,?," +
        "?,?,?,?,?,?,?,?)",
      [
        event.id,
        event.user_id,
