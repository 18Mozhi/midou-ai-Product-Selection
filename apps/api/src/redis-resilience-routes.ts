...
export interface RedisResilienceRouteOptions { service: Pick<RedisResilienceService,"read">; authorization: AuthorizationService; auth: LocalAuthService; secureCookie: boolean }
const ids = (request: FastifyRequest) => ({requestId:String(request.headers["x-request-id"]),traceId:String(request.headers["x-trace-id"])});

export function registerRedisResilienceRoutes(app: FastifyInstance, options: RedisResilienceRouteOptions) {
