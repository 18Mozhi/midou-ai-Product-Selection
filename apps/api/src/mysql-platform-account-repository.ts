...
const iso = (v: unknown) =>
  v === null || v === undefined
    ? null
    : new Date(v as string | Date).toISOString();
export class MySqlPlatformAccountRepository implements PlatformAccountRepository {
  ...
  async overview(input: Parameters<PlatformAccountRepository["overview"]>[0]) {
    ...
    const [organizations, users, admins, summary] = await Promise.all([
      ...
      this.pool.query<RowDataPacket[]>(
        "SELECT u.id,u.email,u.status,GROUP_CONCAT(pra.role_code ORDER BY pra.role_code SEPARATOR ',') roles,MIN(pra.created_at) granted_at FROM users u LEFT JOIN platform_role_assignments pra ON pra.user_id=u.id WHERE (?='' OR u.email LIKE ? ESCAPE '\\\\') AND (?='' OR u.status=?) GROUP BY u.id ORDER BY (MIN(pra.created_at) IS NULL),MIN(pra.created_at),u.email LIMIT ?",
        [input.query, like, status, status, input.limit],
      ),
      this.pool.query<RowDataPacket[]>(
        "SELECT (SELECT COUNT(*) FROM organizations) organizations,(SELECT COUNT(*) FROM organizations WHERE status='active') active_organizations,(SELECT COUNT(*) FROM users) users,(SELECT COUNT(*) FROM users WHERE status='active') active_users,(SELECT COUNT(DISTINCT user_id) FROM platform_role_assignments) platform_admins",
        [],
      ),
    ]);
    return {
  ...
  async createOrganization(
    input: Parameters<PlatformAccountRepository["createOrganization"]>[0],
  ) {
  ...
  async updateOrganization(
    input: Parameters<PlatformAccountRepository["updateOrganization"]>[0],
  ) {
    return this.write(input, async (c) => {
  ...
  async createUser(
    input: Parameters<PlatformAccountRepository["createUser"]>[0],
  ) {
    return this.write(input, async (c) => {
      ...
      await c.query(
        "INSERT INTO users (id,email,email_normalized,password_hash,status,email_verified_at,failed_login_count,locked_until,password_changed_at,must_change_password,must_enroll_mfa,security_setup_completed_at,version,created_at,updated_at) VALUES (?,?,?,?,'active',?,0,NULL,?,1,?,NULL,1,?,?)",
        [
          input.userId,
          input.email,
          input.email,
  ...
  async userDetail(
    input: Parameters<PlatformAccountRepository["userDetail"]>[0],
  ) {
    const [[users], [memberships], [sessions]] = await Promise.all([
      this.pool.query<RowDataPacket[]>(
        "SELECT id,email,status,must_change_password,must_enroll_mfa,password_changed_at,created_at,updated_at FROM users WHERE id=? LIMIT 1",
        [input.userId],
      ),
      this.pool.query<RowDataPacket[]>(
      ...
      this.pool.query<RowDataPacket[]>(
        "SELECT id,status,device_label,expires_at,last_seen_at,created_at FROM user_sessions WHERE user_id=? ORDER BY last_seen_at DESC LIMIT 100",
        [input.userId],
      ),
    ]);
    if (!users[0])
  ...
  async resetUserPassword(
    input: Parameters<PlatformAccountRepository["resetUserPassword"]>[0],
  ) {
    return this.write(input, async (c) => {
      const [rows] = await c.query<RowDataPacket[]>(
      ...
      await c.query(
        "UPDATE users SET password_hash=?,password_changed_at=?,must_change_password=1,failed_login_count=0,locked_until=NULL,status='active',version=version+1,updated_at=? WHERE id=?",
      ...
      await this.audit(
        c,
        input,
        "user.password.forced_reset",
        "user",
        input.userId,
        { reason: input.reason, sessions_revoked: true },
      );
      return result;
    });
  }
  async revokeUserSessions(
    input: Parameters<PlatformAccountRepository["revokeUserSessions"]>[0],
  ) {
    return this.write(input, async (c) => {
  ...
  async setOrganizationStatus(
    ...
    return this.write(input, async (c) => {
      ...
      await c.query(
        "UPDATE organizations SET status=?,version=version+1,updated_at=? WHERE id=?",
        [input.status, input.now, input.organizationId],
      );
      const result = {
        id: input.organizationId,
        status: input.status,
  ...
  async setUserStatus(
    input: Parameters<PlatformAccountRepository["setUserStatus"]>[0],
  ) {
    return this.write(input, async (c) => {
      const [rows] = await c.query<RowDataPacket[]>(
  ...
  async setPlatformRole(
    input: Parameters<PlatformAccountRepository["setPlatformRole"]>[0],
  ) {
    return this.write(input, async (c) => {
      const [rows] = await c.query<RowDataPacket[]>(
      ...
      if (input.enabled)
        ...
        await c.query(
          "DELETE FROM platform_role_assignments WHERE user_id=? AND role_code=?",
          [input.userId, input.roleCode],
        );
      const result = {
        id: input.userId,
  ...
  private audit(
    ...
    return c.query(
      "INSERT INTO platform_audit_events (id,organization_id,workspace_id,actor_id,action,resource_type,resource_id,outcome,request_id,trace_id,metadata,occurred_at,schema_version) VALUES (?,NULL,NULL,?,?,?,?,'succeeded',?,?,?,?,1)",
      [
        randomUUID(),
        input.actorId,
        action,
