...
import { registerApprovalRoutes, type ApprovalRouteOptions } from "./approval-routes.js";
...
import { registerRealtimeRoutes, type RealtimeRouteOptions } from "./realtime-routes.js";
...
import {
  registerOrganizationAdminRoutes,
  type OrganizationAdminRouteOptions,
} from "./organization-admin-routes.js";
import { PlatformDashboardError } from "./platform-dashboard-service.js";
import {
  registerPlatformDashboardRoutes,
  type PlatformDashboardRouteOptions,
} from "./platform-dashboard-routes.js";
import {
  registerPlatformAccountRoutes,
  type PlatformAccountRouteOptions,
} from "./platform-account-routes.js";
import { CollectionConsoleError } from "./collection-console-service.js";
import {
  registerCollectionConsoleRoutes,
  type CollectionConsoleRouteOptions,
} from "./collection-console-routes.js";
import { SecurityOperationsError } from "./security-operations-service.js";
import {
  registerSecurityOperationsRoutes,
  type SecurityOperationsRouteOptions,
} from "./security-operations-routes.js";
import { OpenPlatformError } from "./open-platform-service.js";
import {
  registerOpenPlatformRoutes,
  type OpenPlatformRouteOptions,
} from "./open-platform-routes.js";
...
import {
  registerBackupRecoveryRoutes,
  type BackupRecoveryRouteOptions,
} from "./backup-recovery-routes.js";
import {
  registerReleaseRolloutRoutes,
  type ReleaseRolloutRouteOptions,
} from "./release-rollout-routes.js";
import {
  registerRuntimeTopologyRoutes,
  type RuntimeTopologyRouteOptions,
} from "./runtime-topology-routes.js";
import {
  registerRedisResilienceRoutes,
  type RedisResilienceRouteOptions,
} from "./redis-resilience-routes.js";
import {
  registerMySqlResilienceRoutes,
  type MySqlResilienceRouteOptions,
} from "./mysql-resilience-routes.js";
import {
  registerFileResilienceRoutes,
  type FileResilienceRouteOptions,
} from "./file-resilience-routes.js";
import { CrawlerSchedulerError } from "./crawler-scheduler-service.js";
import {
  registerCrawlerSchedulerRoutes,
  type CrawlerSchedulerRouteOptions,
} from "./crawler-scheduler-routes.js";
import { CapacityBoundaryError } from "./capacity-boundary-service.js";
import {
  registerCapacityBoundaryRoutes,
  type CapacityBoundaryRouteOptions,
} from "./capacity-boundary-routes.js";
import { SelectionJourneyError } from "./selection-journey-service.js";
import {
  registerSelectionJourneyRoutes,
...
export function buildApp(options: BuildAppOptions = {}): FastifyInstance {
  ...
  if (options.erpProductImport) registerErpProductImportRoutes(app, options.erpProductImport);
  ...
  if (options.notifications) registerNotificationRoutes(app, options.notifications);
  ...
  app.setErrorHandler(
    async (error: FastifyError | ApiError, request, reply): Promise<ErrorEnvelope> => {
      const requestId = request.headers["x-request-id"]!.toString();
      const traceId = request.headers["x-trace-id"]!.toString();
      const apiError = error instanceof ApiError ? (error as ApiError) : null;
      const authError = error instanceof AuthError ? (error as AuthError) : null;
      const tenancyError = error instanceof TenancyError ? (error as TenancyError) : null;
      const authorizationError =
        error instanceof AuthorizationError ? (error as AuthorizationError) : null;
      const resourceGrantError =
        error instanceof ResourceGrantError ? (error as ResourceGrantError) : null;
      const auditError = error instanceof AuditError ? (error as AuditError) : null;
      const preferenceError = error instanceof PreferenceError ? (error as PreferenceError) : null;
      const discoveryError = error instanceof DiscoveryError ? (error as DiscoveryError) : null;
      const providerError =
        error instanceof ProviderRegistryError ? (error as ProviderRegistryError) : null;
      const credentialError =
        error instanceof CredentialAssetError ? (error as CredentialAssetError) : null;
      const adapterError =
        error instanceof ProviderAdapterServiceError
          ? (error as ProviderAdapterServiceError)
          : null;
      const crawlerError =
        error instanceof CrawlerRuntimeError ? (error as CrawlerRuntimeError) : null;
      const collectionError =
        error instanceof CollectionTaskServiceError ? (error as CollectionTaskServiceError) : null;
      const dataQualityError =
        error instanceof DataQualityServiceError ? (error as DataQualityServiceError) : null;
      const sourceError =
        error instanceof ProviderSourceServiceError ? (error as ProviderSourceServiceError) : null;
      const validation = "validation" in error && Boolean(error.validation);
      const businessTaskError =
        error instanceof BusinessTaskError ? (error as BusinessTaskError) : null;
      const approvalError =
        error instanceof ApprovalServiceError ? (error as ApprovalServiceError) : null;
      const notificationError =
        error instanceof NotificationServiceError ? (error as NotificationServiceError) : null;
      const realtimeError = error instanceof RealtimeServiceError ? error : null;
      const automationError = error instanceof AutomationServiceError ? error : null;
      ...
      const trendError = error instanceof TrendServiceError ? (error as TrendServiceError) : null;
      const opportunityError =
        error instanceof OpportunityServiceError ? (error as OpportunityServiceError) : null;
      const selectionJourneyError = error instanceof SelectionJourneyError ? error : null;
      const scoringError =
        error instanceof ScoringServiceError ? (error as ScoringServiceError) : null;
      const profitError =
        error instanceof ProfitServiceError ? (error as ProfitServiceError) : null;
      const competitorError =
        error instanceof CompetitorServiceError ? (error as CompetitorServiceError) : null;
      const sourcingError =
        error instanceof SourcingServiceError ? (error as SourcingServiceError) : null;
      const aiAnalysisError =
        error instanceof AiAnalysisServiceError ? (error as AiAnalysisServiceError) : null;
      const statusCode =
        apiError?.statusCode ??
      ...
      return {
        error: {
          ...
          action_hint:
            ...
            (validation ? "按 OpenAPI 修正字段后重试。" : "携带 request_id 联系管理员。"),
        },
        request_id: requestId,
        trace_id: traceId,
      };
