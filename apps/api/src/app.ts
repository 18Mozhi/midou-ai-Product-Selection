...
import { registerProviderAdapterRoutes, type ProviderAdapterRouteOptions } from './provider-adapter-routes.js';
import { CrawlerRuntimeError } from './crawler-runtime-service.js';
import { registerCrawlerRuntimeRoutes, type CrawlerRuntimeRouteOptions } from './crawler-runtime-routes.js';
import { CollectionTaskServiceError } from './collection-task-service.js';
import { registerCollectionTaskRoutes, type CollectionTaskRouteOptions } from './collection-task-routes.js';
import { DataQualityServiceError } from './data-quality-service.js';
import { registerDataQualityRoutes, type DataQualityRouteOptions } from './data-quality-routes.js';
import { ProviderSourceServiceError } from './provider-source-service.js';
import { registerProviderSourceRoutes, type ProviderSourceRouteOptions } from './provider-source-routes.js';
...
import { registerOpportunityRoutes, type OpportunityRouteOptions } from './opportunity-routes.js';
import { ScoringServiceError } from './scoring-service.js';
import { registerScoringRoutes, type ScoringRouteOptions } from './scoring-routes.js';
import { ProfitServiceError } from './profit-service.js';
import { registerProfitRoutes, type ProfitRouteOptions } from './profit-routes.js';
import { CompetitorServiceError } from './competitor-service.js';
import { registerCompetitorRoutes, type CompetitorRouteOptions } from './competitor-routes.js';
import { SourcingServiceError } from './sourcing-service.js';
import { registerSourcingRoutes, type SourcingRouteOptions } from './sourcing-routes.js';
import { AiAnalysisServiceError } from './ai-analysis-service.js';
import { registerAiAnalysisRoutes, type AiAnalysisRouteOptions } from './ai-analysis-routes.js';
import { BusinessTaskError } from './business-task-service.js';
import { registerBusinessTaskRoutes, type BusinessTaskRouteOptions } from './business-task-routes.js';
...
export function buildApp(options: BuildAppOptions = {}): FastifyInstance {
  ...
  if (options.providerAdapters) registerProviderAdapterRoutes(app, options.providerAdapters);
  if (options.crawlerRuntime) registerCrawlerRuntimeRoutes(app, options.crawlerRuntime);
  if (options.collectionTasks) registerCollectionTaskRoutes(app, options.collectionTasks);
  if (options.dataQuality) registerDataQualityRoutes(app, options.dataQuality);
  if (options.providerSources) registerProviderSourceRoutes(app, options.providerSources);
  if (options.trends) registerTrendRoutes(app, options.trends);
  if (options.opportunities) registerOpportunityRoutes(app, options.opportunities);
  ...
  if (options.businessTasks) registerBusinessTaskRoutes(app, options.businessTasks);

  app.setErrorHandler(async (error: FastifyError | ApiError, request, reply): Promise<ErrorEnvelope> => {
    const requestId=request.headers['x-request-id']!.toString();const traceId=request.headers['x-trace-id']!.toString();
    const apiError=error instanceof ApiError?error as ApiError:null;const authError=error instanceof AuthError?error as AuthError:null;const tenancyError=error instanceof TenancyError?error as TenancyError:null;const authorizationError=error instanceof AuthorizationError?error as AuthorizationError:null;const resourceGrantError=error instanceof ResourceGrantError?error as ResourceGrantError:null;const auditError=error instanceof AuditError?error as AuditError:null;const preferenceError=error instanceof PreferenceError?error as PreferenceError:null;const discoveryError=error instanceof DiscoveryError?error as DiscoveryError:null;const providerError=error instanceof ProviderRegistryError?error as ProviderRegistryError:null;const credentialError=error instanceof CredentialAssetError?error as CredentialAssetError:null;const adapterError=error instanceof ProviderAdapterServiceError?error as ProviderAdapterServiceError:null;const crawlerError=error instanceof CrawlerRuntimeError?error as CrawlerRuntimeError:null;const collectionError=error instanceof CollectionTaskServiceError?error as CollectionTaskServiceError:null;const dataQualityError=error instanceof DataQualityServiceError?error as DataQualityServiceError:null;const sourceError=error instanceof ProviderSourceServiceError?error as ProviderSourceServiceError:null;const validation='validation' in error&&Boolean(error.validation);
    const businessTaskError=error instanceof BusinessTaskError?error as BusinessTaskError:null;const trendError=error instanceof TrendServiceError?error as TrendServiceError:null;const opportunityError=error instanceof OpportunityServiceError?error as OpportunityServiceError:null;const scoringError=error instanceof ScoringServiceError?error as ScoringServiceError:null;const profitError=error instanceof ProfitServiceError?error as ProfitServiceError:null;const competitorError=error instanceof CompetitorServiceError?error as CompetitorServiceError:null;const sourcingError=error instanceof SourcingServiceError?error as SourcingServiceError:null;const aiAnalysisError=error instanceof AiAnalysisServiceError?error as AiAnalysisServiceError:null;const statusCode=apiError?.statusCode??authError?.statusCode??tenancyError?.statusCode??authorizationError?.statusCode??resourceGrantError?.statusCode??auditError?.statusCode??preferenceError?.statusCode??discoveryError?.statusCode??providerError?.statusCode??credentialError?.statusCode??adapterError?.statusCode??crawlerError?.statusCode??collectionError?.statusCode??dataQualityError?.statusCode??sourceError?.statusCode??businessTaskError?.statusCode??trendError?.statusCode??opportunityError?.statusCode??scoringError?.statusCode??profitError?.statusCode??competitorError?.statusCode??sourcingError?.statusCode??aiAnalysisError?.statusCode??(validation?400:500);reply.code(statusCode);
    ...
    return{error:{code:apiError?.code??authError?.code??tenancyError?.code??authorizationError?.code??resourceGrantError?.code??auditError?.code??preferenceError?.code??discoveryError?.code??providerError?.code??credentialError?.code??adapterError?.code??crawlerError?.code??collectionError?.code??dataQualityError?.code??sourceError?.code??businessTaskError?.code??trendError?.code??opportunityError?.code??scoringError?.code??profitError?.code??competitorError?.code??sourcingError?.code??aiAnalysisError?.code??(validation?'schema_validation_failed':'internal_error'),message:apiError?.message??crawlerError?.message??collectionError?.message??dataQualityError?.message??businessTaskError?.message??trendError?.message??opportunityError?.message??scoringError?.message??profitError?.message??competitorError?.message??sourcingError?.message??aiAnalysisError?.message??(sourceError?'首批来源请求无法处理。':authError?authErrorMessage(authError.code):tenancyError?tenancyMessages[tenancyError.code]??'租户请求无法处理。':authorizationError?'权限检查未通过。':resourceGrantError?'资源授权请求无法处理。':auditError?'审计请求无法处理。':preferenceError?'主题偏好请求无法处理。':discoveryError?'搜索请求无法处理。':providerError?'来源配置请求无法处理。':credentialError?'凭证资产请求无法处理。':adapterError?'适配器运行请求无法处理。':validation?'请求字段不符合接口合同。':'服务暂时无法处理请求。'),action_hint:apiError?.actionHint??crawlerError?.actionHint??collectionError?.actionHint??dataQualityError?.actionHint??sourceError?.actionHint??businessTaskError?.actionHint??trendError?.actionHint??opportunityError?.actionHint??scoringError?.actionHint??profitError?.actionHint??competitorError?.actionHint??sourcingError?.actionHint??aiAnalysisError?.actionHint??authError?.actionHint??tenancyError?.actionHint??authorizationError?.actionHint??resourceGrantError?.actionHint??auditError?.actionHint??preferenceError?.actionHint??discoveryError?.actionHint??providerError?.actionHint??credentialError?.actionHint??adapterError?.actionHint??(validation?'按 OpenAPI 修正字段后重试。':'携带 request_id 联系管理员。')},request_id:requestId,trace_id:traceId};
  });
