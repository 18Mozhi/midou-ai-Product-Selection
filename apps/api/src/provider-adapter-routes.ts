...
export interface ProviderAdapterRouteOptions {
...
export function registerProviderAdapterRoutes(
  app: FastifyInstance,
  options: ProviderAdapterRouteOptions,
) {
  ...
  app.post("/api/v1/platform/provider-adapters/:providerId/health-check", async (request) => {
    ...
    return envelope(
      await options.service.probe(providerId, {
        actorId,
