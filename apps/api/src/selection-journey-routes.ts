...
export interface SelectionJourneyRouteOptions {
  service: SelectionJourneyService;
...
export function registerSelectionJourneyRoutes(
  app: FastifyInstance,
  o: SelectionJourneyRouteOptions,
) {
  ...
  app.post("/api/v1/selection-journeys", async (r, reply) => {
    const result = await o.service.create({
  ...
  app.get("/api/v1/selection-journeys/:id", async (r, reply) => {
    const result = await o.service.get({
      ...(await scope(r, "opportunity:read")),
      ...ids(r),
      journeyId: (r.params as { id: string }).id,
    });
    reply.header("cache-control", "private, no-store");
    return envelope(result, r);
  });
  app.post("/api/v1/selection-journeys/:id/decisions", async (r, reply) => {
    const result = await o.service.decide({
      ...(await write(r, "opportunity:decide")),
      journeyId: (r.params as { id: string }).id,
      value: r.body as any,
    });
    reply.code(201);
