...
export function registerErpProductImportRoutes(
  ...
  app.post("/api/v1/imports/erp-products", { bodyLimit: 25_000_000 }, async (request, reply) => {
    ...
    try {
      ...
      const code = error instanceof Error ? error.message : "erp_product_import_failed";
      const hints: Record<string, string> = {
        ...
        erp_product_provider_unavailable: "等待平台同步并启用米豆 ERP 商品库来源。",
