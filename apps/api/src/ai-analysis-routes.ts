...
export interface AiAnalysisRouteOptions {
...
export function registerAiAnalysisRoutes(app: FastifyInstance, o: AiAnalysisRouteOptions) {
  const scope = async (r: FastifyRequest, capability: Capability) => {
    ...
    write = async (r: FastifyRequest) => {
      ...
      return {
        ...(await scope(r, "opportunity:decide")),
        ...ids(r),
        idempotencyKey: requireIdempotencyKey(r),
      };
    };
  app.get("/api/v1/opportunities/:id/ai-analyses", async (r) =>
    env(
      await o.service.list({
  ...
  app.post("/api/v1/opportunities/:id/ai-analyses", async (r, reply) => {
    const x = await o.service.queue({
      ...(await write(r)),
      opportunityId: (r.params as any).id,
      value: r.body,
    });
  ...
  app.post("/api/v1/ai-analyses/:id/reviews", async (r, reply) => {
    const x = await o.service.review({
      ...(await write(r)),
      resultId: (r.params as any).id,
      value: r.body,
    });
    reply.code(201);
    return env(x, r);
