...
const requestedInternalView = new URLSearchParams(window.location.search).get('view');
...
const navigationShell = memberRoute
  ...
  : routePath === '/org-admin' || routePath.startsWith('/org-admin/')
    ? 'organization_admin'
    : routePath === '/platform-admin' || routePath.startsWith('/platform-admin/')
      ? 'platform_admin'
      : null;
const isUiStatesView = import.meta.env.DEV && (requestedInternalView === 'ui-states' || routePath === '/ui-states');
