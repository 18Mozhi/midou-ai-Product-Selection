    ...
    <template v-else-if="data">
      <section class="p67-conclusion" :data-verdict="state" aria-labelledby="p67-conclusion-title">
        <div>
          ...
          <p>
            服务返回 {{ data.findings.length }} 项发现；不等于恢复演练、所有任务或高可用已验证。
          </p>
        </div>
        <div>
      ...
      <div class="p67-workspace">
        <section class="p67-findings p67-section" aria-labelledby="p67-findings-title">
          ...
          <p v-else>
            当前返回未列出告警或阻断。不据此推定所有缓存、队列或实时消息功能均已实测可用。
          </p>
        </section>
          ...
          <template v-else
            ><div class="p67-resource-grid">
              ...
              >
                ...
                <p>
                  {{ compactBytes(data.memory.used_bytes) }} /
                  {{ compactBytes(data.memory.max_bytes) }}
                </p>
                ...
                <small v-if="data.memory.max_bytes <= 0"
                  >比例是服务占位值，不作为实际使用率。</small
                >
        ...
        <section class="p67-persistence p67-section" aria-labelledby="p67-persistence-title">
          ...
          <dl v-else class="p67-persistence-rows">
            <div>
              <dt>AOF</dt>
              <dd>
                {{ data.persistence.aof_enabled ? "已启用" : "未启用"
                }}<small>CONFIG GET 观测</small>
              </dd>
              <dd>
            ...
            <div>
              <dt>RDB</dt>
              <dd>
                {{ data.persistence.rdb_enabled ? "已启用" : "未启用"
                }}<small>CONFIG GET 观测</small>
              </dd>
        ...
        <section class="p67-sampling p67-section" aria-labelledby="p67-sampling-title">
          ...
          <dl class="p67-counts">
            <div>
              <dt>已扫描去重键</dt>
              <dd>{{ data.keyspace_sample.scanned_keys }}</dd>
            </div>
            <div>
              <dt>成功测量</dt>
              <dd>{{ data.keyspace_sample.measured_keys }}</dd>
            </div>
            <div>
              <dt>忽略 / 测量失败</dt>
              <dd>
                {{ data.keyspace_sample.ignored_keys }} /
                {{ data.keyspace_sample.failed_measurements }}
              </dd>
            </div>
            <div>
              <dt>成功测得字节</dt>
              <dd>{{ compactBytes(data.keyspace_sample.total_sampled_bytes) }}</dd>
            </div>
          </dl>
          <p>
            采样上限 {{ data.keyspace_sample.sample_limit }} 个键；{{
              data.keyspace_sample.truncated ? "已达到有界采样范围" : "本次未标记截断"
            }}。SCAN COUNT 32 为提示，最多 32 轮；测量每批最多 16。
          </p>
          <div v-if="data.keyspace_sample.hotspots.length" class="p67-sample-list">
            <article
            ...
            >
              ...
              <div>
                <b>{{ compactBytes(item.sampled_bytes) }}</b
                ><small>采样字节</small>
              </div>
              <div>
                <b>{{
                  data.keyspace_sample.total_sampled_bytes > 0
                    ? percent(item.sampled_share_basis_points)
                    : "无比例分母"
                }}</b
                ><small>成功测得字节的占比</small>
              </div>
              <div
                v-if="data.keyspace_sample.total_sampled_bytes > 0"
                class="p67-bar"
                aria-hidden="true"
              >
          ...
          <div v-else class="p67-sample-empty">
            <h3>{{ sampleStatusLabel[data.keyspace_sample.status] }}</h3>
            <p v-if="data.keyspace_sample.status === 'partial'">
              已扫描到受限键，但本次未完成有效内存测量；不能据此判断没有业务键。
            </p>
            <p v-else-if="data.keyspace_sample.unavailable_reason === 'command_unsupported'">
              当前客户端不支持受限 SCAN 与 MEMORY USAGE。
            </p>
            <p v-else-if="data.keyspace_sample.unavailable_reason === 'scan_failed'">
