...
<template>
  <section class="redis-resilience" :data-state="state">
    <header class="redis-resilience__hero"><div><p>SINGLE REDIS · M08-02</p><h2>Redis 单实例韧性</h2><span>当前惠州单机只运行一个宝塔 Redis；不启用 Sentinel、集群、副本或备用服务器。</span></div><button type="button" @click="load">刷新运行事实</button></header>
    ...
    <template v-else-if="data">
      ...
      <div class="redis-resilience__layout">
        ...
        <aside class="redis-resilience__panel"><header><div><p>BOUNDARY</p><h3>单机边界</h3></div></header><dl><div><dt>运行模式</dt><dd>单实例</dd></div><div><dt>Sentinel</dt><dd>{{data.sentinel_enabled?'已启用':'未启用'}}</dd></div><div><dt>Redis 集群</dt><dd>{{data.cluster_enabled?'已启用':'未启用'}}</dd></div><div><dt>容量结论</dt><dd>{{data.capacity_claim==='unverified'?'未验证':'—'}}</dd></div></dl></aside>
      </div>
      <section class="redis-resilience__panel redis-resilience__findings"><header><div><p>FAIL-CLOSED GATES</p><h3>告警与阻断项</h3></div><span>{{data.findings.length}} 项</span></header><div v-if="data.findings.length"><article v-for="(item,index) in data.findings" :key="item.code" :data-severity="item.severity"><span>{{String(index+1).padStart(2,'0')}}</span><code>{{item.code}}</code><p>{{item.action_hint}}</p></article></div><div v-else class="redis-resilience__clear"><b>当前无 Redis 韧性阻断</b><span>这不代表整机、磁盘、机房或容量受到保证。</span></div></section>
