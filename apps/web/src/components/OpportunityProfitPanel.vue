...
<template>
  <section class="opportunity-profit">
    ...
    >
      <article>
        <small>状态</small
        ><strong>{{ statusLabel(profit.latest_run.status) }}</strong
    ...
    >
      ...
      ><span
        >缺失：{{
          profit?.latest_run?.missing_fields.join("、") || "尚无利润计算运行"
        }}</span
      >
    </aside>
    ...
    <form class="profit-input" @submit.prevent="$emit('confirmCost')">
      <h5>确认成本输入</h5>
      <label
        >平台<input v-model="costForm.platform" required maxlength="80"
      /></label>
      <label
      ...
      /></label>
      <label
        >币种<input v-model="costForm.currency" required maxlength="3"
      /></label>
      <label
        >来源类型<input v-model="costForm.source_type" required maxlength="80"
      /></label>
      <label
        >来源标识<input
      ...
      /></label>
      <label
        >证据 ID<input v-model="costForm.evidence_id" required maxlength="36"
      /></label>
      <label
        >观测时间<input
      ...
      <footer>
        ...
        ><button type="button" :disabled="busy" @click="$emit('queueProfit')">
          重新计算
        </button>
      </footer>
    </form>
