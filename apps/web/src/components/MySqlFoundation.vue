...
const state=ref<PreviewState>(requested==='blocked'||requested==='rollback'?requested:'available');
const copy=computed(()=>({
  available:['AVAILABLE · 合同通过','MySQL 5.7 基座可用','真实门禁要求 product_scout 业务账号、utf8mb4、迁移幂等与事务回滚全部通过。'],
  blocked:['BLOCKED · 停止下游','数据库门禁未通过','版本、字符集、业务账号或连接任一不满足时停止依赖模块，不用静态检查冒充成功。'],
  rollback:['ROLLBACK · 逆序恢复','按迁移逆序回滚','先备份与导出迁移元数据，再按 down SQL 逆序执行并复核 checksum 与业务数据。'],
}[state.value]));
</script>
<template>
  ...
  <section class="verification-grid config-grid" aria-label="MySQL 基座合同">
    <article><span class="state-label state-label--passed">MYSQL · 5.7</span><h3>版本锁定</h3><p>拒绝 MySQL 8 专属语法；生产库与业务账号均为 product_scout。</p></article>
    <article><span class="state-label state-label--passed">CHARSET · UTF8MB4</span><h3>统一字符集</h3><p>表、连接和服务端使用 utf8mb4 / utf8mb4_unicode_ci。</p></article>
