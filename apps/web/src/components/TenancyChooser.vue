...
<template>
  <main class="tenancy-page" data-testid="tenancy">
    <header class="tenancy-header"><a href="/" class="identity-brand"><span>S</span>ai选品</a><div><span class="tenancy-step">01</span><i></i><span class="tenancy-step" :class="{'tenancy-step--active':selectedOrganization}">02</span><i></i><span class="tenancy-step">03</span></div><button type="button" class="tenancy-account">当前账号</button></header>
    <section class="tenancy-shell">
      ...
      <div v-else-if="state==='selected'&&selectedContext" class="tenancy-state tenancy-state--selected" aria-live="polite"><b>✓</b><strong>工作范围已就绪</strong><p>{{ selectedContext.organization.name }} · {{ selectedContext.workspace.name }}</p><a href="/onboarding">继续快速引导</a></div>
      <template v-else>
