...
const routePath=window.location.pathname.replace(/\/$/,'')||'/',isHome=computed(()=>props.shell==='member'&&routePath==='/home'),isTrends=computed(()=>props.shell==='member'&&routePath==='/trends'),isScoringRules=computed(()=>props.shell==='member'&&routePath==='/opportunities/scoring-rules'),isOpportunities=computed(()=>props.shell==='member'&&(routePath==='/opportunities'||routePath.startsWith('/opportunities/'))),isCompetitors=computed(()=>props.shell==='member'&&routePath.startsWith('/competitors')),isCostRules=computed(()=>props.shell==='member'&&routePath.startsWith('/sourcing')),opportunityId=computed(()=>{const match=routePath.match(/^\/opportunities\/([0-9a-f-]{36})$/i);return match?.[1]??'';});
const phaseLabel=computed(()=>isTrends.value||isOpportunities.value||isCompetitors.value||isCostRules.value?'P04':props.shell==='platform_admin'&&['/platform-admin/providers','/platform-admin/credentials','/platform-admin/collection','/platform-admin/data'].some(path=>routePath.startsWith(path))?'P03':'P02');
const pageSummary=computed(()=>isCompetitors.value?'竞品身份、来源快照、变化记录和阈值告警由真实 API 与 Worker 驱动。':isCostRules.value?'版本化费用、双审批、汇率来源与利润计算由真实 API 和 Worker 驱动。':isOpportunities.value?'机会、证据覆盖和人工决策由当前组织与工作区的真实 API 驱动。':isTrends.value?'趋势主题、证据、关注和监控规则均由当前组织与工作区的真实 API 驱动。':phaseLabel.value==='P03'?'来源、采集运行与证据数据均由对应模块的真实 API 和权限边界驱动。':'导航与权限壳层已就绪；业务数据由对应阶段的真实 API 接入。');
