...
const pageTitle=computed(()=>routePath==='/opportunities/scoring-rules'?'评分规则':routePath==='/sourcing/cost-rules'?'费用与利润规则':routePath==='/sourcing'?'供应链与利润':activeItem.value?.label??shellTitle.value);
const routePath=window.location.pathname.replace(/\/$/,'')||'/',isHome=computed(()=>props.shell==='member'&&routePath==='/home'),isTrends=computed(()=>props.shell==='member'&&routePath==='/trends'),isScoringRules=computed(()=>props.shell==='member'&&routePath==='/opportunities/scoring-rules'),isOpportunities=computed(()=>props.shell==='member'&&(routePath==='/opportunities'||routePath.startsWith('/opportunities/'))),isCompetitors=computed(()=>props.shell==='member'&&routePath.startsWith('/competitors')),isSourcing=computed(()=>props.shell==='member'&&routePath==='/sourcing'),isCostRules=computed(()=>props.shell==='member'&&routePath.startsWith('/sourcing/cost-rules')),opportunityId=computed(()=>{const match=routePath.match(/^\/opportunities\/([0-9a-f-]{36})$/i);return match?.[1]??'';});
const phaseLabel=computed(()=>isTrends.value||isOpportunities.value||isCompetitors.value||isSourcing.value||isCostRules.value?'P04':props.shell==='platform_admin'&&['/platform-admin/providers','/platform-admin/credentials','/platform-admin/collection','/platform-admin/data'].some(path=>routePath.startsWith(path))?'P03':'P02');
const pageSummary=computed(()=>isSourcing.value?'供应链候选、版本化报价、最多五家对比和采购任务均保留来源与缺失项。':isCompetitors.value?'竞品身份、来源快照、变化记录和阈值告警由真实 API 与 Worker 驱动。':isCostRules.value?'版本化费用、双审批、汇率来源与利润计算由真实 API 和 Worker 驱动。':isOpportunities.value?'机会、证据覆盖和人工决策由当前组织与工作区的真实 API 驱动。':isTrends.value?'趋势主题、证据、关注和监控规则均由当前组织与工作区的真实 API 驱动。':phaseLabel.value==='P03'?'来源、采集运行与证据数据均由对应模块的真实 API 和权限边界驱动。':'导航与权限壳层已就绪；业务数据由对应阶段的真实 API 接入。');
...
<template>
  <main class="role-shell" :data-shell="shell" :data-state="state">
    <header class="role-topbar">
      <a class="role-brand" :href="shell==='member'?'/home':shell==='organization_admin'?'/org-admin':'/platform-admin'"><span>S</span><b>ScoutOps</b></a>
      <button class="role-menu-toggle" type="button" aria-label="打开导航菜单" :aria-expanded="menuOpen" aria-controls="role-navigation" @click="menuOpen=!menuOpen">☰ <span>菜单</span></button>
    ...
    <aside id="role-navigation" class="role-sidebar" :class="{'is-open':menuOpen}" :aria-label="`${shellTitle}导航`">
      <div class="role-sidebar-head"><strong>{{shellTitle}}</strong><small>{{shell==='member'?'当前组织业务范围':shell==='organization_admin'?'仅当前组织':'平台角色授权范围'}}</small></div>
      <nav v-if="state==='ready'">
        <a v-for="item in items" :key="item.path" :href="item.path" :aria-current="activeItem?.path===item.path?'page':undefined" @click="menuOpen=false"><i>{{item.icon}}</i><span>{{item.label}}</span></a>
      </nav>
      <div class="role-sidebar-foot"><span aria-hidden="true">●</span><p><strong>权限由服务端裁决</strong><small>前端菜单不是安全边界</small></p></div>
    </aside>
    <section class="role-content">
      <section v-if="state!=='ready'" class="role-gate-state" aria-live="polite">
        <span class="role-state-mark" aria-hidden="true">{{state==='loading'?'···':state==='forbidden'?'×':'!'}}</span><p>ROUTE GUARD</p><h1>{{stateCopy[0]}}</h1><p>{{stateCopy[1]}}</p><small v-if="actionHint">{{actionHint}}</small><code v-if="requestId">request_id: {{requestId}}</code>
      ...
      <template v-else>
        ...
        <section v-else-if="shell==='platform_admin'&&routePath.startsWith('/platform-admin/collection')" class="provider-runtime-surface">
          <nav class="provider-runtime-tabs" aria-label="采集控制台视图"><a href="/platform-admin/collection" :aria-current="routePath==='/platform-admin/collection'?'page':undefined">任务状态机</a><a href="/platform-admin/collection/browser-runtime" :aria-current="routePath==='/platform-admin/collection/browser-runtime'?'page':undefined">浏览器运行时</a></nav>
        ...
        <section v-else class="role-ready-panel">
          ...
          <div class="role-scope-note"><strong>权限说明</strong><p>{{shell==='platform_admin'?'平台角色与组织角色严格分离；平台壳层不依赖组织上下文。':shell==='organization_admin'?'仅 organization_admin 可进入；所有后续组织 API 仍需服务端能力与范围 Guard。':'活动成员可进入；菜单按服务端 capabilities 过滤，资源 API 仍执行数据范围 Guard。'}}</p></div>
        </section>
      </template>
    </section>
