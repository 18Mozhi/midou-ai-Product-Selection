...
import PlatformLogWorkspace from "./PlatformLogWorkspace.vue";
import type { OperationalLog, PlatformLogTraceChain } from "./platform-log-types";
...
const traceChains = computed<PlatformLogTraceChain[]>(() => {
...
<template>
  <section class="platform-log-center platform-log-center--c" aria-live="polite">
    <header>
      <div>
        <p>P62 / 事件检索</p>
        <h1>链路日志</h1>
    ...
    <p class="p62-boundary-note">
      本次结果最多返回 200 条，可能只包含部分调用链；事件与关联对象均以已返回记录为准。
    </p>
    ...
    <template v-else>
      ...
      <PlatformLogWorkspace :chains="traceChains">
        <template #chain="{ chain }">
          <section class="platform-log-chain" :data-exception="chain.exceptionCount > 0">
            <header>
              <div>
              ...
              <div class="platform-log-chain__sources">
                <i
                  v-for="chainSource in chain.sources"
                  :key="chainSource"
                  :data-source="chainSource"
                >
            ...
            >
                      ...
                      <td class="platform-log-actions">
                        <template v-if="isException(item)">
                          ...
                          <RouterLink v-if="providerLink(item)" :to="providerLink(item)"
                            >查看关联来源</RouterLink
                          >
                          <small v-if="!taskLink(item) && !providerLink(item)"
                            >暂无可定位对象</small
                          >
                        </template>
                        <span v-else>—</span>
              ...
              <template #detail="{ row }">
                ...
                <div v-if="isException(row)" class="platform-log-actions">
                  ...
                  <RouterLink v-if="providerLink(row)" :to="providerLink(row)"
                    >查看关联来源</RouterLink
                  >
...
<style src="../platform-log-center-c.css"></style>
