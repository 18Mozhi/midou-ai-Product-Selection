...
const state = ref<State>("loading"),
  ...
  roles = ref<RoleCapabilitySummary[]>([]),
  selected = ref<RoleCapabilitySummary | null>(null),
  query = ref(""),
  requestId = ref("");
const filtered = computed(() =>
  roles.value.filter(
    (role) =>
      !query.value.trim() ||
      `${role.name} ${role.code} ${role.description} ${role.capabilities.join(" ")}`
        .toLowerCase()
  ...
  <main class="authz-page" data-testid="authorization">
    ...
    <section class="authz-main">
      ...
      <div v-else-if="state === 'empty'" class="authz-state">
        <b>○</b><strong>暂无活动角色</strong>
        <p>角色目录未初始化，管理动作保持默认拒绝。</p>
        <button type="button" @click="load">重新加载</button>
      </div>
      <template v-else-if="current"
        ><section class="authz-summary">
          <div>
            <span>当前角色</span><strong>{{ current.roles.length }}</strong
        ...
        <div class="authz-toolbar">
          <label
            >搜索角色或能力<input
              v-model="query"
              type="search"
              placeholder="例如：审批机会" /></label
          ><span>{{ filtered.length }} 个角色</span>
        </div>
        <div class="authz-layout">
          <div class="role-list">
            <button
              ...
              :key="role.code"
              type="button"
              :class="{ selected: selected?.code === role.code }"
              @click="selected = role"
            >
              <span>{{ role.name.slice(0, 1) }}</span>
              <div>
                <strong>{{ role.name }}</strong
                ><small>角色编号：{{ role.code }}</small>
