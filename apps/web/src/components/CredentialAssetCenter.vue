...
const props = defineProps<{ apiBaseUrl: string }>(),
  ...
  requestId = ref(""),
  message = ref(""),
  editor = ref<"asset" | "rotate" | "profile" | null>(null),
...
<template>
  <section class="credential-center">
    <header>
      <div>
        <p>CREDENTIAL VAULT / PLATFORM SECURITY</p>
        <h2>凭证与浏览器档案</h2>
        <span>页面只显示指纹、版本与状态；秘密载荷不会回显。</span>
      </div>
      <div>
        <button type="button" @click="openProfile">＋ 浏览器档案</button
    ...
    <section v-else>
      <div class="credential-metrics">
        ...
        <article>
          <small>CRAWLER PROFILES</small><strong>{{ profiles.length }}</strong
          ><span>授权任务才可临时解密</span>
        </article>
        <article>
          <small>PLAINTEXT EXPOSURE</small><strong>0</strong
          ><span>API 不提供读取接口</span>
        </article>
      </div>
      <section v-if="!assets.length" class="credential-empty">
        <h3>还没有平台凭证资产</h3>
        <p>先选择已登记 Provider，再写入受 AES-256-GCM 保护的秘密载荷。</p>
      ...
      <section class="profile-list">
        ...
        <article v-for="profile in profiles" :key="profile.id">
          ...
          ><span>{{ profile.locale }} · {{ profile.timezone }}</span
          ><b>{{ profile.status }}</b>
        </article>
    ...
    >
      <header>
        <div>
          <p>
            {{ editor === "rotate" ? "ROTATE SECRET" : "NEW ENCRYPTED ASSET" }}
          </p>
          <h3>
            {{
      ...
      <div class="credential-fields">
        <label v-if="editor === 'asset'"
          >Provider<select v-model="assetForm.provider_id" required>
        ...
        ><label
          >编码<select v-model="assetForm.encoding">
            <option>utf8</option>
    ...
    >
      ...
      <div class="credential-fields">
        ...
        ><label>Locale<input v-model="profileForm.locale" required /></label
        ><label>Timezone<input v-model="profileForm.timezone" required /></label
        ><label
      ...
      <p v-if="!browserAssets.length">
        需要先创建类型为 browser_profile 的 active 凭证资产。
      </p>
      <footer>
