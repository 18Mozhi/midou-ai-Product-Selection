...
代码提交 `3bf11b89c808d0e626ace4900cd67cde75143a3d` 已推送并通过固定 `python scripts/deploy-baota.py` 部署，生产 build SHA 一致。脚本公开复验 ready/available/version、`/login` 200、具体未知路由404及浏览器助手资源均通过，并报告临时发布产物已清理。按固定部署流程执行了既有迁移白名单与宝塔 Node 停启；本批未增加迁移或运行配置。正式 M07-03、真实 RBAC/批量业务写入与服务端幂等仍需独立证据。
