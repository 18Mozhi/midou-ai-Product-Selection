...
2026-09-10字段细化：[202张字段/组合图](design/roles-fields-direction-c/README.md)。16字段对应14子模型、受控类型和共享原因，92单端状态；9组新增组合使四容器上下文引用从11增至20。全部复验源身份和PNG，不将离线输入演示称为真实授权。创建/延期原因上限500，共享原因前端无maxlength而API上限500；延期必须晚于原到期时间来自现有后端，新稿就地提示但生产不变。用户已通过四项控件视觉，字段/组合尚未批准。

2026-09-10控件细化：[新378图](design/roles-controls-direction-c/gallery.html)把18代表动作76状态和24附加变体113状态绑定精确selector；余44代表槽未映射，不补造单页可翻页、原生select弹层或原因窗提交busy。源动作仍20；下文120未映射为上一批源登记记录。字段/完整组合、真实C与具体审核仍待，未部署。
