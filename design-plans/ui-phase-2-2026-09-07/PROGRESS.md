...
真实凭证路由的长文本矩阵发现不换行来源显示名把390px移动卡片撑至2138px。通过容器收缩与来源名/指纹断行修复，仅涉及展示样式。178字符来源名、288字符指纹在390/760/1024/1440及3主题×2密度属性组合下，桌面与390px手机定向E2E各1/1通过（每端24组合）；属性由测试直接设置，不验证偏好设置流程。完整套件待本轮回归；真实读屏与生产验收不在证据内。
