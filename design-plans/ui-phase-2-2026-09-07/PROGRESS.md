...
SC50-02 定向回归发现刷新失败虽保留旧快照并提示错误，却未把失败请求编号带入刷新提示，还覆盖了快照请求编号。实际 Vue 改为单独保存本次读失败 request_id：首次错误显示在 UiStatePanel，保留快照时显示在就地刷新提示，成功快照编号不被失败请求替换。补充三项 GET 各自部分失败/401/403 组合，以及创建、轮换、档案引用的就地失败 action_hint/request_id；现有撤销和登录失败路径继续覆盖。新增E2E双端4/4通过；完整 M03-02 页回归桌面 Chromium 与390px手机各28/28，Web类型和生产构建通过。实施与边界见[P50错误追踪实施记录](P50-CREDENTIAL-ERROR-TRACE-IMPLEMENTATION.md)。只变更前端反馈状态和测试/规格，不改 API、权限、字段、业务合同或运行配置；本地拦截证据不代表真实后端/授权验收，也未部署。
