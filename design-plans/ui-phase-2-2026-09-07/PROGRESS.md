...
已将用户通过的 C 方向接入真实 `/security/mfa` 身份安全页：明确状态未知/读取失败与重读、分步绑定和恢复码结果、独立停用区；共享的首次安全设置也使用原生必填校验与单飞保护。保留现有四个 MFA API 方法、字段与会话撤销边界。桌面 Chromium 7/7、390px 手机 7/7、身份来源 18 项及 P02–P07 路由映射检查通过；类型、构建及项目发布质量门与线上证据详见[P07实施记录](P07-PAGE-COMPOSITION-IMPLEMENTATION.md)。本地隔离样例不证明真实 MFA、Cookie 撤销、RBAC 或正式 M07-03；全 73 页继续。
