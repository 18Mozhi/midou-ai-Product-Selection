...
保留新建/轮换资产与档案引用表单现有 `required`/`pattern` 约束和请求行为；捕获原生 invalid 状态，将浏览器本地化错误文本显示在对应字段旁并通过 `aria-invalid`、`aria-describedby` 关联，输入修正后清除。P50完整E2E桌面与390px手机各30/30通过；定向双端字段回归2/2、Web类型检查通过；验证表单错误不会提交写请求。证据仅为本地浏览器约束，不声称服务端字段规则、屏幕阅读器或生产验收。未改API/OpenAPI、业务字段、权限、数据或配置；未部署/重启。SC50-06仍有长文本、主题密度与真实读屏覆盖待验。
