...
P50缓存停用清理（2026-09-12，main/de9ec65起点）：生产CredentialAssetCenter增加onDeactivated/onActivated和读取代次；KeepAlive离页关闭编辑/撤销、清除登录材料与敏感字段、失效化助手/材料回执并abort三项资料GET，返回只恢复被中断读取，普通缓存返回复用成功快照。新增3个真实App壳层E2E，先在旧代码复现3类失败，修复后本批双端6/6、M03-02桌面/手机28/28；2状态×4宽度共8次128检查/8PNG/172源，全部本地GET夹具、零写入/外链/真实材料。C审核CSS未进入生产，API/OpenAPI、权限、数据库、加密、env、依赖均未改，未部署；两种返回空白态待审，通用资产/轮换/档案/撤销在途写、深链重复激活政策与真实链路继续。

P50登录材料生命周期（2026-09-12，main/164b3aab起点）：生产CredentialAssetCenter增加编辑器/材料代次、助手AbortSignal、来源/方式切换清空、两段保存阶段与当前会话保护；保存中关闭/取消/来源/方式锁定，资产结果未知、档案结果未知和档案明确失败分开反馈并禁直接重提。新增5个E2E；叠加缓存批次后M03-02桌面/手机28/28通过。7状态×4宽度共28次500检查/28PNG/172源，合成材料不回显/持久化，0/0/0/1/1/2/2次POST只到本地拦截层且带body/幂等键。C审核CSS只补禁用主按钮灰态，六状态/六边界/页面证据随当前源码重建。未改API/权限/数据库/加密/env/依赖，未部署；七状态待审，KeepAlive停用清理由独立批次覆盖，真实助手/后端/生产及P50其余交互继续。
