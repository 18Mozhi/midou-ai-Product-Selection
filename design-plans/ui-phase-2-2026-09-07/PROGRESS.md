...
P50登录材料生命周期（2026-09-12，main/164b3aab起点）：生产CredentialAssetCenter增加编辑器/材料代次、助手AbortSignal、来源/方式切换清空、两段保存阶段与当前会话保护；保存中关闭/取消/来源/方式锁定，资产结果未知、档案结果未知和档案明确失败分开反馈并禁直接重提。新增5个E2E，M03-02桌面/手机22/22通过；7状态×4宽度共28次500检查/28PNG/172源，合成材料不回显/持久化，0/0/0/1/1/2/2次POST只到本地拦截层且带body/幂等键。C审核CSS只补禁用主按钮灰态，六状态/六边界/页面证据随当前源码重建。未改API/权限/数据库/加密/env/依赖，未部署；七状态待审，KeepAlive停用清理、真实助手/后端/生产及P50其余交互继续。
