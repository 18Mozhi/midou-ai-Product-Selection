...
(() => {
  ...
  function renderModal() {
    ...
    $("#modal").innerHTML =
      `<form id="modal-form" aria-busy="${busy}"><header><div><p class="eyebrow">供应链 / ${modalKind === "delete" ? "危险操作" : "明确输入"}</p><h2 id="dialog-title">${title}</h2></div>${action("SC-MODAL-CLOSE", "×", "", busy, `aria-label="关闭${title}" data-close-variant="close" ${busy ? 'aria-describedby="modal-submit-hint"' : ""}`)}</header>${body}${modalError ? `<div role="alert" class="notice error">${esc(modalError)}<br><code>synthetic-request-21</code></div>` : ""}<footer><p id="modal-submit-hint" class="submit-hint" role="status" tabindex="-1"></p>${action("SC-MODAL-CLOSE", "取消", "", busy, `data-close-variant="cancel" ${busy ? 'aria-describedby="modal-submit-hint"' : ""}`)}<button type="submit" data-action="SC-${modalKind.toUpperCase()}-SUBMIT" aria-describedby="modal-submit-hint" aria-busy="${busy}" class="primary ${modalKind === "delete" ? "danger" : ""}" ${busy ? "disabled" : ""}>${busy ? { search: "正在提交找货", quote: "正在确认报价", purchase: "正在创建任务", delete: "正在删除记录" }[modalKind] : submit}</button></footer></form>`;
    syncModalSubmit();
  }
