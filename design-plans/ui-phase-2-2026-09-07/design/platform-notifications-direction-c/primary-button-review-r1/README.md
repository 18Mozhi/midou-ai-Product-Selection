...
2026-09-15 复核：16 张 PNG 与原 manifest 哈希全部一致；20 份捕获来源中，当前 `apps/web/src/platform-notifications.css` 已发生变化，其余 19 份仍匹配。因此本包继续作为历史组件隔离审核图，不覆盖图片、不重写捕获指纹，也不把原 432 项结果当作当前代码的验证。此次重新展示手机发布的默认、悬停＋键盘焦点、原因不足禁用、提交中禁用四张操作栏，等待该区域的用户审核；手机取消四组合的既有通过不扩大到发布、保存或完整弹窗。

本次另以当前工作树运行 `node scripts/verify-platform-notification-button-states.mjs --primary`，72 环境、432 状态通过、0 失败；这是独立的新回归结果，不更新旧 manifest 或宣称新截图。临时本地服务为 `127.0.0.1:5173`（验证进程 PID 36264），命令正常退出，浏览器与服务由 finally 关闭，未写出临时文件、未调用业务接口。本次仅补充审核说明，无产品代码、配置或运行契约改动，无需构建或重启，未部署。
