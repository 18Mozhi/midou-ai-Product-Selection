...
状态：C方向已选，具体稿待审。独立工作面，不是实际Vue、完整NavigationShell或生产验收。
...
| 收起/再展开 | 不清输入；创建成功关表单但保留输入。busy时局部禁用与空关键词关联返焦是待审改进，不声称源码已修复。 |
...
证据记录为known-gap-not-fixed。这里没有实际Vue渲染修复，未注销UI2-A12或其它验收项。新恢复链接、未知值保护、busy/焦点均待审。审核场景切换的世代清理仅用于测试工具，不等于真实页面缓存、跨范围、迟到响应或并发提交已安全。
