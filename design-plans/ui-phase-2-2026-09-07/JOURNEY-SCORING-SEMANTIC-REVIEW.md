...
后续P16 r2已按新规则扩展338图，当前十处/8组源码映射不变；40个代表适用状态槽已有引用、8导航禁用/忙碌不适用，恢复主次分支已核对。用户只批准整体布局。见[控件审核](P16-CONTROL-STATE-REVIEW.md)、[图册](design/journey-direction-c/README.md)、[批准范围](P16-C-R2-LAYOUT-APPROVAL.md)与合同第9节。本页下方P16旧状态为历史，P17未改。 新增[字段审核](P16-FIELD-STATE-REVIEW.md)，5个源绑定的8控件变体已有状态图与隔离交互验证，生产未改。 另补44张外观/放大图，总382图，见[外观审核](P16-APPEARANCE-REVIEW.md)；仅代表场景，不证明所有主题状态或生产验收。

最新决定：用户已明确“统一五项质量门”，见[J07决定记录](JOURNEY-ADOPTION-DECISION.md)。P16 前后端本地实现和定向/双端隔离回归已通过；下文为 `a8cbf10f` 改前语义快照，旧冲突与错误态缺口不能当作当前源码事实。旧图与来源哈希保持历史证据，采纳成功/缺门新图及源映射仍需刷新，不再等待重复规则选择。P17 代码未改。
