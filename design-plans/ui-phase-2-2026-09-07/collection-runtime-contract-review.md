...
O:743的checkbox另外由:checked/:disabled及@change转发到toggleDeadLetter；状态非open/batchBusy禁用，不能把这个输入漏在10个v-model之外。三个ConfirmDialog还拥有共享影响勾选与签认文本，是调用场景输入，不在局部v-model数量内。
