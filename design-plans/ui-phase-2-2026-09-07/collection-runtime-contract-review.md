...
| ID | 源码事实 / 待验场景 | 退出证据 |
| --- | --- | --- |
| CL-G01 | T同路由page/status及列表读取代次已局部修复；三页KeepAlive离页、O/R快速查询与组织/角色变化仍待 | 继续可控迟到读/写、跨页返回和权限范围变化；当前CL51不外推到O/R |
| CL-G02 | T重放关闭/换任务归属已局部修复；O批原因未冻结、过滤可变；R回收再刷新提示覆盖 | 继续精确body/目标/幂等/调用次数，离开与部分成功不误导；先复现不猜新业务规则 |
| CL-G03 | T三态标题/关闭、可见焦点过滤及嵌套确认inert已局部修复；共享移动抽屉及二级转详情焦点仍待 | 继续真实键盘、焦点返回、错误关联与读屏可达，不能靠CSS或脚本改DOM绕过 |
| CL-G04 | T已补automatically_replayed与空结果筛选保留；O ready未计attempts仍待 | 继续核对真实可达数据/过滤合同与总览呈现；不改任务状态机 |
