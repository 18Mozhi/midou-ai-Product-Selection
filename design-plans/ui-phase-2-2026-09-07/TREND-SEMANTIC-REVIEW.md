...
2026-09-26 修复并补实浏览器回归：`follow()` 在发请求前捕获主题对象和ID，成功回执只更新该对象及同ID列表项。新增延迟回执用例先确认旧行为失败，再验证桌面 Chromium/390px 手机切换到B后，B仍未关注、A列表更新为已关注。该证据覆盖本地受控Vue响应，不证明真实数据库/审计或其他迟到读写场景。
