...
最新实现增量：[P38 时间窗同步](P38-WINDOW-SYNC-REVIEW.md)。当前挂载源已修复返回/前进同步与在途窗口归属，模板/CSS 不变；新增 2 张样例结果图，累计 42 图/32 组正向浏览器检查。下面“源组件未改/返回未修/40 图”为此前批次记录，不再代表当前脚本；复制拒绝及模态隔离等剩余问题仍适用。
