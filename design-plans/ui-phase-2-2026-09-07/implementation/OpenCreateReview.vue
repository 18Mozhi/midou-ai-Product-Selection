...
// Local review only. The slot owns the original fields, validation and request preparation.
...
watch(
  ...
  async (confirming) => {
    if (confirming && dialog.value?.open) {
      ...
      // Synchronous close precedes ConfirmDialog's focus capture; only one modal remains visible.
      close();
