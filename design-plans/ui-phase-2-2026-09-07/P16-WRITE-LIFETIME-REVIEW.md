...
NavigationShell的reset_on_scope使用KeepAlive键；换键不一定销毁原实例，因此本批**不宣称修复所有跨范围或缓存实例之间的全局活动ID竞争**。多标签/账号/组织切换期间的跨作用域标记争用、等待期间编辑、reset保留决策草稿和全部异步读写归属仍待专项处理，不能以销毁保护或本地存储异常隔离替代它们。没有冻结输入、清理用户未提交草稿或修改范围权限。
...
## 2026-09-25 · 本地进度存储失败隔离

`localStorage` 的读取、设置或删除可能因浏览器隐私策略、配额或安全上下文而抛错。现在这些异常只影响本地恢复标记：已由服务端确认的创建、状态读取或决策仍显示服务端结果，不再落入“服务失败”分支；恢复读取失败不会产生未处理异常；重置仍清空当前页面草稿。页面通过现有 `role="status"` 提示本地恢复标记无法同步，且说明服务器状态未受影响。

存储键和格式不变，不自动重试写入，不变更 API、数据库或业务门槛。`tests/unit/ui-phase2-journey-write-lifetime.test.mjs` 扩为 12 项，覆盖 create 写入、decide 清除、resume 读取和 reset 清除失败；`scripts/verify-ui-phase2-journey-write-lifetime.mjs` 扩为 8 个真实 Vue 挂载场景，并在真实 `Storage.prototype` 上模拟 SecurityError，验证服务端确认仍保留且不会重复 POST。这些是本地隔离验证，不代替真实隐私模式、后端或生产验收。全站多标签/组织范围活动标记仲裁仍未解决。
