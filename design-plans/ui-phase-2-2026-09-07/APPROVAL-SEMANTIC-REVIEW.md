...
2026-09-10在途交互：[54图与规则](design/approval-lifecycle-direction-c/README.md)。五个原合同请求预览在提交时固定内容/原dialog渲染代次，全局等待不重复写入；关闭不取消，原窗失败保留/模拟成功只关原窗，重开或后来B不受旧结果影响。双端106检查含18关闭路径×两结果、重开草稿/旧token与A/B焦点/URL/提示归属，均离线、真实Vue读写与requestId风险未修。原busy条件补模板/发起/发布disabled三槽，103有图/20源码无状态/21待核对；非详情遮罩和发布关闭差异仍待审，未提升批准。

2026-09-10诊断增量：[12图与适用性说明](design/approval-diagnostics-direction-c/README.md)。AN-TECH-REQUEST四个代表槽明确映射，409提示/编号来源为UI2-AN04；source api/closeDetail隔离验证错误后关闭仍保留页面notice/ID，后到成功覆盖ID的既有风险未修。普通Error无编号回退不编造服务响应。10组20槽按当前源呈现无disabled/busy单列，累计100有图/20源码无状态/24待核对。24包含两组被直接展示替代的字段折叠12、含原生dialog事件的关闭8、写入disabled3与分页busy1，保守校验不绕过，不伪造状态或批准。生产不变；下文历史批次保留。

2026-09-10导航增量：[354图册与范围](design/approval-navigation-direction-c/README.md)。41变体双端原生四态/逻辑选中/分页边界及82点击，17代表组+19额外入口显式关联；5新阅读目录不当新增源动作。新增65代表槽，余48。仅新包修复目录被固定标题覆盖并验证章节起点不被遮挡；源码全部筛选URL、错误时队列可见性、字段折叠替代、发布关闭/在途归属等差异继续明示，不提升具体批准或生产验收。下文为历史批次。
