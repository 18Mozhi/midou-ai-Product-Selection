...
按 page-spec 第 9 节，将 P39 组织目录的桌面列设置、密度选择及展开后的列选项行最小高度从共享控件默认 36px 提升至 44px；账号二级导航链接由 42px 提升至 44px。规则限定在 P39 页面作用域，不改变其他 `ResponsiveDataView` 页面、表格数据或列选择行为。手机端继续使用现有移动记录预览，不显示桌面列工具。

验证：生产组合单测 4/4；M06-01.A07/A08/A15 实际 Vue E2E 桌面与 390px 手机各 1/1。双端当前可见的账号链接、按钮、输入/选择控件均实测不小于 44px；桌面还逐项测量列工具与展开列项，手机筛选窗控件、记录预览和路由切换回归通过。Web 类型检查与生产构建通过；此项仅调整前端静态样式，无 API、数据、权限或服务运行行为变更。页面视觉方向按用户全局授权通过；读屏器、真实 RBAC 和正式 M07-03 仍不由此证明。
