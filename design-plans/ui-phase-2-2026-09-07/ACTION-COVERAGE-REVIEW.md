...
[逐项机器清单](action-reviews/P59.json)：29个局部源位置 → 9组；0类写入，9组路由动作，0组转发/容器关联不重复计动作。已映射0/3个源码字段位置，0/5处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有54个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| SO59-WINDOW 切换安全事件与审计时间范围 / read | 1处；current-route-source-contract | ；其余见JSON | 范围控件不证明数据库事件完整或所有集合都按时间筛选。 |
| SO59-LOAD 刷新或重试当前安全运营读取 / read | 2处；current-route-source-contract | ；其余见JSON | 页面读取会由服务写入安全读取审计；静态映射不证明真实权限、HTTP或审计持久化。 |
| SO59-TECH 展开当前安全记录已有技术详情 / local | 11处；current-route-source-contract | ；其余见JSON | 披露不证明敏感字段服务端脱敏、移动模态完整读屏或焦点生命周期已全部验收。 |
| SO59-VIEW 切换事件、会话、访问凭证或审计调查视图 / navigation | 1处；current-route-source-contract | ；其余见JSON | 视图导航不证明目标数据存在或当前身份获准读取。 |
| SO59-FILTER 提交当前安全调查视图搜索与状态条件 / read | 2处；current-route-source-contract | ；其余见JSON | 提交筛选不证明服务端语义、记录总数或跨视图数据授权。 |
| SO59-RESET 清除当前调查视图筛选 / read | 1处；current-route-source-contract | ；其余见JSON | 重置只改变查询范围，不撤销安全对象或删除记录。 |
| SO59-PAGE 翻阅事件、会话、凭证或审计当前视图分页 / read | 8处；current-route-source-contract | ；其余见JSON | 本地翻页不证明服务端COUNT或历史集合完整。 |
| SO59-TOKEN-PAGE 翻阅组织令牌独立分页 / read | 2处；current-route-source-contract | ；其余见JSON | 独立控件不证明report:read授权或令牌集合已完整读取。 |
| SO59-MANAGE 前往独立授权的凭证与档案管理页 / navigation | 1处；current-route-source-contract | ；其余见JSON | 入口不授予platform:operate，也不代表凭证已查看、修改或撤销。 |
...
[逐项机器清单](action-reviews/P61.json)：6个局部源位置 → 6组；0类写入，6组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/1处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有36个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| P61-TOPOLOGY 查看实时拓扑 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不证明生产依赖状态或目标页权限。 |
| P61-TOPOLOGY-NODE 打开单个依赖拓扑节点详情 / navigation | 1处；current-route-source-contract | ；其余见JSON | 目标链接存在不证明服务真实健康、故障传播或生产观测完整。 |
| P61-PROPAGATION 进入当前异常传播核查项处理页 / navigation | 1处；current-route-source-contract | ；其余见JSON | 核查入口不证明传播推断或后续处置成功。 |
| P61-COLLECTION 前往采集任务概览 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不代表采集Worker在线或任务已运行。 |
| P61-PROVIDER 前往来源配置目录 / navigation | 1处；current-route-source-contract | ；其余见JSON | 链接可见不证明来源权限、凭证状态或平台可用。 |
| P61-WORKSPACE 切换状态页四个本地信息分区 / local | 1处；current-route-source-contract | ；其余见JSON | 本地分区切换不刷新数据，也不证明全局系统健康。 |
...
[逐项机器清单](action-reviews/P63.json)：2个局部源位置 → 2组；0类写入，2组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/0处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有12个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| P63-OP-CURRENT-EVIDENCE 展开当前 method/path 操作的五维验证证据 / local | 1处；current-route-source-contract | ；其余见JSON | 报告可关联或证据状态显示通过，不等于当前构建、真实生产报告、SQL/RBAC或正式验收通过。 |
| P63-OP-CURRENT-TRACE 展开当前 method/path 操作的请求与追踪编号 / local | 1处；current-route-source-contract | ；其余见JSON | 编号存在不证明关联请求成功，也不代表真实运行链路、敏感信息审查或生产权限已验收。 |
...
[逐项机器清单](action-reviews/P64.json)：10个局部源位置 → 5组；0类写入，5组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/2处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有30个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| BR64-LOAD 读取或刷新当前备份与恢复只读事实 / read | 1处；current-route-source-contract | ；其余见JSON | 页面刷新不是备份、恢复、演练、下载密文或数据库零写入证明。 |
| BR64-RETRY 在失败状态下重新核验备份恢复快照 / read | 2处；current-route-source-contract | ；其余见JSON | 重读通过不证明真实授权、资产完整性或恢复可用。 |
| BR64-LOGIN 会话过期时重新登录 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不证明重新认证成功或具有platform:operate权限。 |
| BR64-TECH 展开资产或阻断事实的技术详情 / local | 3处；current-route-source-contract | ；其余见JSON | 本页局部披露不证明真实MySQL审计或复制剪贴板权限/读屏验收。 |
| BR64-DIRECTORY-NAV 跳转到本页目标、恢复证据或备份资产分区 / navigation | 3处；current-route-source-contract | ；其余见JSON | 页内导航不表示所选资产具备完整性、可解密性或恢复资格。 |
...
[逐项机器清单](action-reviews/P65.json)：12个局部源位置 → 7组；0类写入，7组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/2处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有42个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| RL65-LOAD 读取或刷新当前发布事实 / read | 1处；current-route-source-contract | ；其余见JSON | 本地读取不证明当前页面与生产构建一致，也不证明整套发布门已验收。 |
| RL65-COVERAGE 由超管打开接口覆盖证据页 / navigation | 1处；current-route-source-contract | ；其余见JSON | 入口可见不证明目标路由授权、报告真实或接口覆盖验收通过。 |
| RL65-RETRY 在发布事实读取失败后重新核验 / read | 2处；current-route-source-contract | ；其余见JSON | 重新核验不等于重新部署、重放迁移或自动回滚。 |
| RL65-LOGIN 会话过期时重新登录 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不证明重新认证、platform:operate或超管权限已取得。 |
| RL65-NAV 跳转运行身份、历史观察及动作历史分区 / navigation | 3处；current-route-source-contract | ；其余见JSON | 页内导航仅浏览已加载事实，不证明历史动作可执行。 |
| RL65-CONFIG-DETAIL 展开部署配置指纹详情 / local | 1处；current-route-source-contract | ；其余见JSON | 指纹披露不等于配置来源独立核验或部署成功。 |
| RL65-GATE-DETAIL 展开桌面/移动发布门指标及阻断原因技术详情 / local | 3处；current-route-source-contract | ；其余见JSON | 展示门结果不证明真实生产运行、审计事务或正式发布签收。 |
...
[逐项机器清单](action-reviews/P66.json)：16个局部源位置 → 12组；0类写入，12组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/1处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有72个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| RT66-LOAD 读取或刷新服务拓扑与运行事实 / read | 1处；current-route-source-contract | ；其余见JSON | 页面查看仍可能写入既有平台审计，不等于数据库零写入或实时外部探测。 |
| RT66-NAV 跳转节点进程、健康探测、队列调度及告警区段 / navigation | 4处；current-route-source-contract | ；其余见JSON | 页内导航不是重启、修复或调度执行动作。 |
| RT66-RETRY 在拓扑事实读取失败后重新核验 / read | 2处；current-route-source-contract | ；其余见JSON | 重读成功不证明节点、Worker、队列或上游实际健康。 |
| RT66-LOGIN 会话过期时重新登录 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不证明重新认证、platform:operate或审计权限已取得。 |
| RT66-PROCESS 展开进程最近失败文本 / local | 1处；current-route-source-contract | ；其余见JSON | 失败说明不保证进程完整日志、根因或当前状态仍相同。 |
| RT66-RESTART 展开API/Worker重启观测摘要 / local | 1处；current-route-source-contract | ；其余见JSON | 采样摘要不是OS进程列表、自动修复结果或完整重启审计。 |
| RT66-QUEUES 切换全部队列与运行/异常队列本地视图 / local | 1处；current-route-source-contract | ；其余见JSON | 筛选后的列表不代表隐藏队列不存在或调度已暂停。 |
| RT66-POLICY 展开单个队列调度策略参数 / local | 1处；current-route-source-contract | ；其余见JSON | 策略摘要不是队列已执行或已达到调度效果的证明。 |
| RT66-SNAPSHOT 披露状态文件异常 / local | 1处；current-route-source-contract | ；其余见JSON | 计数为零仅是当前响应事实，不证明所有状态文件均有效。 |
| RT66-OBJECT 打开响应中提供真实href的关联业务对象 / navigation | 1处；current-route-source-contract | ；其余见JSON | 链接存在不证明对象仍存在或用户有权查看。 |
| RT66-ALERT 展开告警代码、根因与关联业务ID / local | 1处；current-route-source-contract | ；其余见JSON | 详情展开不证明根因、告警完整性或当前影响已解除。 |
| RT66-BLOCKER 展开阻断项技术代码 / local | 1处；current-route-source-contract | ；其余见JSON | 阻断码展示不等于阻断已解决或服务已恢复。 |
...
[逐项机器清单](action-reviews/P67.json)：4个局部源位置 → 3组；0类写入，3组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/1处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有18个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| RD67-LOAD 读取或刷新 Redis 运行事实 / read | 1处；current-route-source-contract | ；其余见JSON | 读取成功/失败及保存的运行观测不等于真实恢复、MySQL零写入或全量Redis配置验收。 |
| RD67-RETRY 在 Redis 读取失败后重新核验 / read | 2处；current-route-source-contract | ；其余见JSON | 重新核验不证明探针、MySQL观测写入、权限或运行策略状态已恢复。 |
| RD67-LOGIN 会话过期时重新登录 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不证明重新登录、platform:operate或审计权限已取得。 |
...
[逐项机器清单](action-reviews/P68.json)：4个局部源位置 → 3组；0类写入，3组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/2处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有18个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| MY68-LOAD 读取或刷新 MySQL 运行与恢复事实 / read | 1处；current-route-source-contract | ；其余见JSON | 查看GET本身仍通过既有仓储写入观测/审计；页面不等于数据库零写入或实库资格验收。 |
| MY68-RETRY 在 MySQL 读取失败后重新核验 / read | 2处；current-route-source-contract | ；其余见JSON | 重读成功不证明MySQL配置、RPO/RTO或恢复记录已满足生产发布门。 |
| MY68-LOGIN 会话过期时重新登录 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不证明重新认证、platform:operate或数据库审计权限已取得。 |
...
[逐项机器清单](action-reviews/P69.json)：4个局部源位置 → 3组；0类写入，3组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/1处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有18个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| FL69-LOAD 读取或刷新本机文件存储事实 / read | 1处；current-route-source-contract | ；其余见JSON | 页面读取仍按服务合同检查三根并记录运行观测/审计，不代表真实文件完整性或生产恢复通过。 |
| FL69-RETRY 在文件观测读取失败后重新核验 / read | 2处；current-route-source-contract | ；其余见JSON | 重读不证明所有文件被抽检、真实权限ACL或第三根目录恢复资格。 |
| FL69-LOGIN 会话过期时重新登录 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航不证明重新认证、platform:operate或实际文件系统访问权限。 |
...
[逐项机器清单](action-reviews/P70.json)：16个局部源位置 → 9组；2类写入，9组路由动作，0组转发/容器关联不重复计动作。已映射0/2个源码字段位置，0/3处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有54个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| SC70-LOAD 读取或刷新采集调度运行事实 / read | 1处；current-route-source-contract | ；其余见JSON | 读取事实不等于执行任务、查询所有历史或验证真实Linux/Worker/Python运行身份。 |
| SC70-RETRY 在调度事实读取失败后重新核验 / read | 2处；current-route-source-contract | ；其余见JSON | 重读不证明当前来源健康、实际Worker并发或回执完整。 |
| SC70-FILTER 按来源代码和运行范围筛选当前来源列表 / local | 2处；current-route-source-contract | ；其余见JSON | 本地筛选不重新读取、不改变服务端范围或任务状态。 |
| SC70-PAGE 在当前已筛选来源结果中前后分页 / local | 2处；current-route-source-contract | ；其余见JSON | 本地分页不表示数据源总数、队列任务数或跨来源全局去重数。 |
| SC70-EXPIRED 确认回收已过期调度租约 / write | 3处；current-route-source-contract | ；其余见JSON | 失败/网络未知不证明未执行；操作不等同于删除任务、释放档案租约或重放采集。真实数据库锁、审计和权限仍待验。 |
| SC70-PROVIDER-RECOVER 确认解除指定来源熔断 / write | 3处；current-route-source-contract | ；其余见JSON | recovered=false、409、权限失败或网络未知不代表恢复已完成；对象code/ID在现有弹窗中的核对缺口保持待办，不改后端确认规则。 |
| SC70-HEALTH 导航到指定来源适配器健康页 / navigation | 1处；current-route-source-contract | ；其余见JSON | 链接可见不证明健康已执行、来源当前可用或用户有权读取目标。 |
| SC70-DETAIL 展开来源最近失败文本 / local | 1处；current-route-source-contract | ；其余见JSON | 失败文本不是完整日志，也不保证当前状态仍相同。 |
| SC70-LEASE-TECH 展开当前活动租约技术标识 / local | 1处；current-route-source-contract | ；其余见JSON | 标识披露不表示任务仍运行、操作系统进程完整或可安全重放。 |
...
[逐项机器清单](action-reviews/P71.json)：7个局部源位置 → 3组；1类写入，3组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/2处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有18个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| SC71-LOAD 读取或刷新容量边界实测事实 / read | 3处；current-route-source-contract | ；其余见JSON | 读取成功不证明同提交签名、实际并发容量、生产权限或隔离恢复通过。 |
| SC71-ATTEST 确认签认归档与隔离恢复演练 / write | 3处；current-route-source-contract | ；其余见JSON | 失败或网络未知不证明未签认；服务与仓储当前未证明观测新鲜度、同提交绑定、真实权限/审计或并发幂等验收。 |
| SC71-FINDING-TECH 展开当前容量发现代码与责任角色 / local | 1处；current-route-source-contract | ；其余见JSON | 角色标签不是具体责任人；发现文本不能替代原始采样、构建身份或权限证明。 |
...
[逐项机器清单](action-reviews/P72.json)：8个局部源位置 → 4组；0类写入，4组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/2处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有24个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| ST72-HOME 返回 ScoutOps 首页 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航本身不证明首页授权或任何业务操作已发生。 |
| ST72-SELECT 选择一个开发态界面状态 / local | 1处；current-route-source-contract | ；其余见JSON | 状态是演示选择，不代表真实用户/服务权限或故障事实；非法/重复query回退策略不在按钮动作中重写。 |
| ST72-ACTIONS 触发当前状态卡片的演示主/次动作 / local | 3处；current-route-source-contract | ；其余见JSON | “重新尝试/申请权限”等演示文案不代表真实重试、权限申请、网络或业务写入。 |
| ST72-CONFIRM-DEMO 打开并完成本地高影响确认演示 / local | 3处；current-route-source-contract | ；其余见JSON | 共享ConfirmDialog键盘/焦点/遮罩行为仍由共享合同负责；本示例不得推断其他调用方或真实授权流程。 |
...
[逐项机器清单](action-reviews/P73.json)：3个局部源位置 → 3组；0类写入，3组路由动作，0组转发/容器关联不重复计动作。已映射0/0个源码字段位置，0/1处调用/内嵌容器，0个明确变体。共享源页面记录允许显式标注局部子集，不表示其余字段或容器已审阅；此处不是全页共享源的去重分母，原静态导入关联数不与本数相减当缺失按钮。

尚有18个视觉状态槽未映射；已登记状态见逐项JSON，仍须判断所有变体适用性。有场景关联不等于每个按钮六态已验收，也不表示缺少同数量图片。

| 合同组 / 性质 | 源位置 / 动态变体 | 已有场景入口 | 剩余核对 |
| --- | --- | --- | --- |
| NF-BRAND 从404恢复页返回主页 / navigation | 1处；current-route-source-contract | ；其余见JSON | 离开404页后目标页面的请求/会话权限不属于本页零API结论。 |
| NF-RETURN 返回最近有效的已登记页面 / navigation | 1处；current-route-source-contract | ；其余见JSON | 导航记忆检查不等于完整URL安全、跨账号隔离、目标会话有效或HTTP状态为404。 |
| NF-HOME 最近目标不是主页时提供独立主页入口 / navigation | 1处；current-route-source-contract | ；其余见JSON | 条件入口不证明目的页面已登录、获授权或目标请求成功。 |
