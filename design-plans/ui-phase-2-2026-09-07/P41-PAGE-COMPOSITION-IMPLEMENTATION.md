...
- BaoTa 部署成功，部署脚本确认临时发布包已删除；最终部署后线上 `/api/v1/health/live` 返回的 `BUILD_SHA` 与本次发布 HEAD 完全一致。线上 `/api/v1/health/live`、`/api/v1/health/ready`、P41 深链接及向导/父页面 JS、CSS 资源均返回 200；ready 报告 MySQL、Redis、supervisor 可用。仅为只读运行核验，不执行真实组织创建，也不等于正式 M07-03 验收。
