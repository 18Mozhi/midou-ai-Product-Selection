...
2026-09-10写入补充：[已销毁实例回写保护](P16-WRITE-LIFETIME-REVIEW.md)。create/decide成功、失败及finally忽略onUnmounted后的回执，不能覆盖/清理新实例活动ID；onDeactivated继续保留原续办语义。8源测试/6真实Vue宿主场景通过，不是全范围缓存/多标签竞争修复。模板与事件签名不变，下方旧源码行号按当时快照保留；现行行号由动作审计扫描。
