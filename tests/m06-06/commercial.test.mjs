...
const repository = new Proxy(
  ...
  { get: (target, key) => target[key] ?? (async (input) => input) },
);
...
test("M06-06.A01/A02/A04/A09 validates quota-only commercial boundary", async () => {
  assert.throws(
    () =>
      service.createPlan({
        value: { code: "Bad Code", name: "x", quotas: { price: 1 }, reason: "x" },
      }),
...
test("M06-06.A03/A05/A10/A11/A12/A14/A16 persistence and audit", async () => {
  const all = (
    await Promise.all(
      [
        "database/migrations/0024_commercial_operations_m06_06.up.sql",
        "database/migrations/0024_commercial_operations_m06_06.down.sql",
