...
test("M07-02.A01-A05 scope is finite and runtime persistence surfaces are explicitly not applicable", () => {
  ...
  assert.deepEqual(
    Object.keys(policy.notApplicable).sort(),
    ["asyncRuntime", "databaseMigration", "frontendPage", "runtimeApi"].sort(),
  );
});
...
test("M07-02.A06-A13 gate covers every required release security family and fails high or critical dependency findings", () => {
  ...
  for (const id of [
    ...
    "browser-dangerous-sinks",
    "browser-sensitive-storage",
    "external-link-opener",
    "parameterized-sql-contract",
    "csrf-origin-contract",
    "ssrf-webhook-contract",
    "upload-surface-contract",
    "authorization-live",
    "sanitized-log-live",
    "baota-edge-headers",
  ])
    assert.ok(policy.checks.includes(id), id);
});
