...
test("M01-04.A03/A13 MySQL57 migrations persist roles scopes decisions and reverse order", async () => {
  const names = [
    "0011a_roles_m01_04",
    "0011b_capabilities_m01_04",
    "0011c_role_capabilities_m01_04",
    "0011d_membership_roles_m01_04",
    "0011e_membership_scopes_m01_04",
    "0011f_platform_roles_m01_04",
    "0011g_authorization_decisions_m01_04",
    "0011h_seed_roles_m01_04",
    "0011i_seed_capabilities_m01_04",
    "0011j_seed_role_capabilities_m01_04",
  ];
  for (const name of names) {
    ...
    assert.doesNotMatch(
      await read(`database/migrations/${name}.up.sql`),
      /JSON_TABLE|WITH\s+RECURSIVE|CHECK\s*\(/i,
    );
  }
  const scopes = await read("database/migrations/0011e_membership_scopes_m01_04.up.sql");
  for (const value of ["own", "team", "workspace", "organization"])
    assert.match(scopes, new RegExp(`'${value}'`));
  const decisions = await read("database/migrations/0011g_authorization_decisions_m01_04.up.sql");
...
test("M01-04.A06/A13 OpenAPI DTO and API use server session without client actor fields", async () => {
  const [openapi, contracts, routes] = await Promise.all([
    read("docs/openapi.yaml"),
    read("packages/contracts/src/index.ts"),
    read("apps/api/src/authorization-routes.ts"),
  ]);
  assert.match(openapi, /\/me\/authorization:/);
  assert.match(openapi, /\/org\/\{organizationId\}\/roles:/);
  assert.match(openapi, /\/platform\/roles:/);
  for (const dto of [
...
test("M01-04.A01/A10/A11/A17 docs maps and env lock exact boundaries", async () => {
  const [architecture, runbook, map, env, blueprint, gate, registry] = await Promise.all([
    read("docs/architecture/m01-04-rbac-data-scope.md"),
    read("docs/runbooks/m01-04-rbac-data-scope.md"),
    read("docs/feature-map.json"),
    read("config/env.example"),
    read("new-product-enterprise-blueprint.md"),
    read("scripts/verify-docs.mjs"),
    read("verification/modules/M01-04.json"),
  ]);
  assert.match(architecture, /动作族/);
  assert.match(architecture, /M01-05/);
  assert.match(runbook, /0011j.*0011i.*0011h.*0011g.*0011f.*0011e.*0011d.*0011c.*0011b.*0011a/s);
