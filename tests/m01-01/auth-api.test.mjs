...
import { CaptureAuthDelivery, InMemoryAuthRepository, LocalAuthService, createArgon2PasswordHasher } from '../../packages/auth/dist/index.js';
...
test('M01-01.A06/A09/A13 API sets HttpOnly session and never returns opaque token', async () => {
  ...
  const registration=await app.inject({method:'POST',url:'/api/v1/auth/register',headers:headers('register-a'),payload:{email:'user@example.com',password:'Correct-Horse-42'}});
...
test('M01-01.A08/A09/A16 API rejects foreign origin, missing idempotency and replayed action token', async () => {
  ...
  const forbidden=await app.inject({method:'POST',url:'/api/v1/auth/register',headers:{origin:'https://evil.example','idempotency-key':'bad-origin'},payload:{email:'user@example.com',password:'Correct-Horse-42'}});assert.equal(forbidden.statusCode,403);assert.equal(forbidden.json().error.code,'origin_not_allowed');
  const missing=await app.inject({method:'POST',url:'/api/v1/auth/register',headers:{origin:'http://127.0.0.1:5173'},payload:{email:'user@example.com',password:'Correct-Horse-42'}});assert.equal(missing.statusCode,400);assert.equal(missing.json().error.code,'idempotency_key_required');
  await app.inject({method:'POST',url:'/api/v1/auth/register',headers:headers('register-b'),payload:{email:'user@example.com',password:'Correct-Horse-42'}});
  const verify=()=>app.inject({method:'POST',url:'/api/v1/auth/email-verification/confirm',headers:headers('verify-b'),payload:{token:delivery.messages[0].token}});
  assert.equal((await verify()).statusCode,200);const replay=await verify();assert.equal(replay.statusCode,200);
  const login=await app.inject({method:'POST',url:'/api/v1/auth/login',headers:{origin:'http://127.0.0.1:5173'},payload:{email:'user@example.com',password:'Correct-Horse-42'}});assert.match(login.headers['set-cookie'],/__Host-scoutops_session=.*Secure/);
  await app.close();
});
