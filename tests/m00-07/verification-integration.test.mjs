...
test('M00-07.A14/M00-07.A16 nonzero command fails and writes a redacted report', async () => {
  await withWorkspace(async (root) => {
    ...
    await writeFile(
      ...
      JSON.stringify(registry(moduleId, ['node -e "console.error(\'password=hunter2\'); process.exit(7)"'])),
    );
    await assert.rejects(
      () => runModule(moduleId, { root, env: { VERIFY_REPORT_DIR: '.artifacts/verification', VERIFY_COMMAND_TIMEOUT_MS: '10000' } }),
      (error) => error instanceof VerificationError && error.code === 'command_failed',
    );
    const report = await readFile(join(root, '.artifacts', 'verification', 'module-M00-99.json'), 'utf8');
...
test('M00-07.A05 phase runner reads every module in plan order', async () => {
  await withWorkspace(async (root) => {
    await writeFile(join(root, 'plans', 'phase-00-foundation.md'), [
      '# P00', '### M00-01 原子任务索引', '### M00-07 原子任务索引',
    ].join('\n'));
