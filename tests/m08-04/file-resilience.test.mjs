...
const required = [
  "database/migrations/0033_file_resilience_m08_04.up.sql",
  "database/migrations/0033_file_resilience_m08_04.down.sql",
  ...
  "verification/file-resilience-production-evidence.schema.json",
  "scripts/verify-file-resilience-production.mjs",
  "scripts/verify-file-resilience-live.mjs",
  "docs/architecture/m08-04-file-resilience.md",
  "docs/runbooks/m08-04-file-resilience.md",
  "tests/e2e/m08-04-file-resilience.spec.ts",
  "verification/modules/M08-04.json",
];
test("M08-04.A01-A17 deliver a BaoTa-only local-file resilience boundary", async () => {
  const all = (await Promise.all(required.map((path) => readFile(path, "utf8")))).join("\n");
  for (const token of [
    "M08-04",
    "local_managed_directories",
    "checksum",
    "organization",
    "platform:operate",
    "request_id",
  ...
  assert.doesNotMatch(all, /NFS|Ceph|MinIO|systemctl\s|pm2\s+start|docker(?:-|\s)compose\s+up/i);
});

test("M08-04.A02/A04/A12 evaluates roots integrity capacity and explicit recovery facts", async () => {
  const { evaluateFileResilience } = await import("../../packages/storage/dist/index.js");
  const policy = {
    usageWarningBasisPoints: 7500,
    usageStopBasisPoints: 9000,
    maximumRecoveryDrillAgeDays: 90,
  };
  const root = (kind, availableBytes = 800) => ({
    kind,
    available: true,
    writable: true,
    totalBytes: 1000,
  ...
  const base = {
    ...
    encryptedSameHostCopy: true,
    isolatedRestoreVerified: true,
    recoveryDrillAgeDays: 1,
    publicDirectoryExposed: false,
  ...
  assert.ok(warning.findings.some((item) => item.code === "file_capacity_warning"));
  const blocked = evaluateFileResilience(
    { ...base, checksumMismatchFiles: 1, encryptedSameHostCopy: false },
    policy,
  );
  assert.equal(blocked.state, "blocked");
  assert.ok(blocked.findings.some((item) => item.code === "file_checksum_mismatch"));
  assert.ok(blocked.findings.some((item) => item.code === "file_recovery_unverified"));
  assert.throws(
    () => evaluateFileResilience(base, { ...policy, usageWarningBasisPoints: 9000 }),
    /warning threshold/,
  );
});

test("M08-04.A04/A06/A09 service and operations route preserve explicit recovery truth and authorization", async () => {
  const { FileResilienceService } = await import("../../apps/api/dist/file-resilience-service.js");
  const snapshot = {
    roots: [
      {
        kind: "evidence",
  ...
  const recorded = [];
  const service = new FileResilienceService(
    { snapshot: async () => snapshot },
    { record: async (input) => recorded.push(input) },
    { usageWarningBasisPoints: 7500, usageStopBasisPoints: 9000, maximumRecoveryDrillAgeDays: 90 },
    () => new Date("2026-08-15T08:00:00.000Z"),
  );
  const dto = await service.read({ actorId: "actor", requestId: "request", traceId: "trace" });
  ...
  const auth = {
    authenticate: async () => ({
      user: { id: "00000000-0000-4000-8000-000000000804" },
      session: { id: "session" },
    }),
  };
  const app = buildApp({
    fileResilience: { service: routeService, authorization, auth, secureCookie: false },
  });
...
test("M08-04.A03/A05/A10/A13 migration config worker and manifests stay MySQL57 and backend-only", async () => {
  ...
  assert.match(up, /content_sha256/);
  assert.doesNotMatch(up, /CHECK\s*\(|utf8mb4_0900|INVISIBLE\s+INDEX/i);
  assert.match(down, /DROP COLUMN `content_sha256`/);
  assert.match(worker, /createHash\("sha256"\)/);
  assert.match(rollout, /0033_file_resilience_m08_04\.up\.sql/);
  const config = loadRuntimeConfig(
    {
      NODE_ENV: "test",
      FILE_STORAGE_WARNING_PERCENT: "70",
  ...
  assert.equal(config.fileResilience.usageWarningBasisPoints, 7000);
  ...
  assert.throws(
    () =>
      loadRuntimeConfig(
        { FILE_STORAGE_WARNING_PERCENT: "90", FILE_STORAGE_STOP_PERCENT: "80" },
        "api",
      ),
    /FILE_STORAGE/,
  );
  for (const key of [
    ...
    "FILE_STORAGE_STOP_PERCENT",
    "FILE_STORAGE_CHECKSUM_SAMPLE_LIMIT",
    "FILE_STORAGE_RECOVERY_DRILL_MAX_AGE_DAYS",
    "FILE_STORAGE_PRODUCTION_EVIDENCE_FILE",
    "FILE_STORAGE_EVIDENCE_MAX_AGE_MINUTES",
  ]) {
...
test("M08-04.A04/A09/A14 persists observation view and audit in one transaction", async () => {
  const { FileResilienceRepository } =
    await import("../../apps/api/dist/file-resilience-repository.js");
  const calls = [];
  const connection = {
  ...
  const repository = new FileResilienceRepository({ getConnection: async () => connection });
  const root = (kind) => ({
    kind,
    available: true,
    writable: true,
  ...
  await repository.record({
    ...
    evaluation: { state: "ready", maximumUsageBasisPoints: 1000, findings: [] },
  });
  assert.deepEqual(
...
test("M08-04.A07/A08/A15/A16 UI evidence and rollback cover full states and image contract", async () => {
  ...
  assert.match(verifier, /sharedStorageEnabled===false/);
