...
test("M00-04 key always scopes organization and optional workspace", () => {
  const org = buildScopedRedisKey({
    purpose: "cache",
    organization_id: "org-a",
    resource: "trend",
    identifiers: ["same"],
  });
  const workspace = buildScopedRedisKey({
    purpose: "cache",
    organization_id: "org-a",
    workspace_id: "ws-a",
    resource: "trend",
  ...
  const otherOrg = buildScopedRedisKey({
    purpose: "cache",
...
test("M00-04 rejects missing scope and unsafe or empty segments", () => {
  assert.throws(
    () => buildScopedRedisKey({ purpose: "queue", organization_id: "", resource: "job" }),
    /organization_id/,
  );
  assert.throws(
    () => buildScopedRedisKey({ purpose: "queue", organization_id: "org-a", resource: "" }),
    /resource/,
  );
});
...
test("M00-04 queue lease only releases the matching token", async () => {
  ...
  const client = {
    ...
    incr: async () => 1,
    expire: async () => true,
    eval: async (_script, options) => (options.arguments[0] === "owner-a" ? 1 : 0),
  };
  const store = new ScopedRedisStore(client);
  const input = {
    organization_id: "org-a",
  ...
  assert.equal(await store.acquireLease(input, "owner-a", 30), true);
  assert.deepEqual(calls[0][2], { EX: 30, NX: true });
  assert.equal(await store.releaseLease(input, "owner-b"), false);
  assert.equal(await store.releaseLease(input, "owner-a"), true);
});
...
test("M00-04 health maps dependency failure without leaking connection details", async () => {
  ...
  assert.deepEqual(
    { status: status.status, request_id: status.request_id, trace_id: status.trace_id },
    { status: "unavailable", request_id: "request-1", trace_id: "trace-1" },
  );
...
test("M00-04 migration is MySQL 5.7 compatible and reversible", async () => {
  const up = await readFile(
    "database/migrations/0004_m00_04_redis_namespace_catalog.up.sql",
    "utf8",
  );
  const down = await readFile(
    "database/migrations/0004_m00_04_redis_namespace_catalog.down.sql",
    "utf8",
  );
  assert.match(up, /utf8mb4/);
  assert.match(up, /cache.*queue.*rate.*sse/s);
...
test("M00-04 config timeout validates range and remains backend-only", async () => {
  assert.equal(
    loadRuntimeConfig({ NODE_ENV: "test", REDIS_CONNECT_TIMEOUT_MS: "4500" }).redis
      .connectTimeoutMs,
    4500,
  );
  assert.throws(
    () => loadRuntimeConfig({ NODE_ENV: "test", REDIS_CONNECT_TIMEOUT_MS: "99" }),
    /REDIS_CONNECT_TIMEOUT_MS/,
  );
  const env = await readFile("config/env.example", "utf8");
  const schema = await readFile("config/schema.json", "utf8");
  assert.match(env, /REDIS_CONNECT_TIMEOUT_MS=3000/);
...
test("M00-04 OpenAPI, Feature Map, UI, docs and atomic evidence are synchronized", async () => {
  const [api, map, ui, architecture, runbook, registry] = await Promise.all(
    [
      ...
      "apps/web/src/components/RedisFoundation.vue",
  ...
  assert.match(api, /x-scoutops-redis:[\s\S]*RedisDependencyStatus/);
  assert.match(map, /scoutops:v1:<purpose>:org:<organization_id>/);
  for (const state of ["available", "unavailable", "recovering"])
    assert.match(ui, new RegExp(state));
  for (const label of ["可用 · 可连接", "不可用 · 已阻塞", "恢复中 · 恢复检查"])
    assert.match(ui, new RegExp(label));
  assert.doesNotMatch(ui, />\s*(?:AVAILABLE|UNAVAILABLE|RECOVERING)\s*</);
  ...
  assert.deepEqual(
    ...
    Array.from({ length: 17 }, (_, i) => `M00-04.A${String(i + 1).padStart(2, "0")}`),
  );
});
