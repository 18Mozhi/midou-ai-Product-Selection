...
test("M08-03.A01-A17 deliver a BaoTa-only MySQL57 single-primary resilience boundary", async () => {
  ...
  assert.doesNotMatch(
    all,
    /CHANGE\s+MASTER|START\s+SLAVE|read[_ -]?replica|systemctl\s|pm2\s+start|docker(?:-|\s)compose\s+up/i,
  );
});
...
test("M08-03.A02/A04/A12 evaluates durability I/O slow query capacity and recovery facts", async () => {
  ...
  const blocked = evaluateMySqlResilience(
    { ...base, logBinEnabled: false, backupStatus: "stale" },
    policy,
  );
...
test("M08-03.A06/A09/A11/A13 operations route is authorized audited and sanitized", async () => {
  ...
  const service = {
    read: async (input) => {
      ...
      return {
        ...
        connections: { connected: 2, running: 1, maximum: 512, usage_basis_points: 39 },
        storage: { used_bytes: 400, total_bytes: 1000, usage_basis_points: 4000 },
        io: {
          buffer_pool_bytes: 1000,
          buffer_pool_data_bytes: 500,
  ...
  const response = await app.inject({
    ...
    headers: {
      ...
      "x-request-id": "mysql-request",
      "x-trace-id": "mysql-trace",
    },
  });
  ...
  assert.doesNotMatch(
    response.body,
    /DB_PASSWORD|mysql:\/\/|127\.0\.0\.1|3306|\/www\/server|product_scout@/i,
  );
  await app.close();
});
...
test("M08-03.A03/A10/A14 migration and configuration remain MySQL57 and backend-only", async () => {
  ...
  const config = loadRuntimeConfig(
    {
      NODE_ENV: "test",
      MYSQL_CONNECTION_WARNING_PERCENT: "70",
      MYSQL_CONNECTION_STOP_PERCENT: "85",
      MYSQL_DATA_WARNING_PERCENT: "65",
      MYSQL_DATA_STOP_PERCENT: "90",
      MYSQL_SLOW_QUERY_WARNING_PER_MINUTE: "4",
      MYSQL_SLOW_QUERY_STOP_PER_MINUTE: "16",
    },
    "api",
  );
  ...
  assert.throws(
    () =>
      loadRuntimeConfig({ MYSQL_DATA_WARNING_PERCENT: "90", MYSQL_DATA_STOP_PERCENT: "80" }, "api"),
    /MYSQL_DATA/,
  );
  for (const key of [
    "MYSQL_CONNECTION_WARNING_PERCENT",
    "MYSQL_CONNECTION_STOP_PERCENT",
    "MYSQL_DATA_WARNING_PERCENT",
    "MYSQL_DATA_STOP_PERCENT",
    "MYSQL_SLOW_QUERY_WARNING_PER_MINUTE",
    "MYSQL_SLOW_QUERY_STOP_PER_MINUTE",
...
test("M08-03.A04/A09/A14 persists observation view and audit in one transaction", async () => {
  const { MySqlResilienceRepository } =
    await import("../../apps/api/dist/mysql-resilience-repository.js");
  const calls = [];
  const connection = {
  ...
  await repository.record({
    actorId: "00000000-0000-4000-8000-000000000803",
    requestId: "request",
    traceId: "trace",
    observedAt: new Date("2026-08-14T13:00:00.000Z"),
    snapshot: {
      available: true,
      version: "5.7.44-log",
      readOnly: false,
      ...
      masterStatusAvailable: true,
      replicaConfigured: false,
      bufferPoolBytes: 1000,
      bufferPoolDataBytes: 500,
      bufferPoolHitRateBasisPoints: 9999,
      maxConnections: 512,
      threadsConnected: 2,
      threadsRunning: 1,
    ...
    evaluation: {
      state: "ready",
      connectionUsageBasisPoints: 39,
      dataUsageBasisPoints: 4000,
      findings: [],
      singlePrimary: true,
...
test("M08-03.A04/A14 probe follows the real M07-04 backup and isolated recovery schema", async () => {
  ...
  const statuses = {
    ...
    innodb_buffer_pool_bytes_data: "2147483648",
    innodb_log_waits: "0",
    innodb_row_lock_waits: "0",
  };
  const pool = {
    query: async (sql, values = []) => {
      if (sql.startsWith("SHOW GLOBAL VARIABLES"))
        return [
          values.map((name) => ({
            Variable_name: name,
            Value: variables[String(name).toLowerCase()] ?? "",
          })),
          [],
        ];
      if (sql.startsWith("SHOW GLOBAL STATUS"))
        return [
          values.map((name) => ({
            Variable_name: name,
            Value: statuses[String(name).toLowerCase()] ?? "0",
          })),
          [],
        ];
      ...
      if (sql.includes("backup_recovery_runs")) {
        assert.match(sql, /run_type/);
        assert.match(sql, /finished_at/);
        assert.doesNotMatch(sql, /\bkind\b|completed_at/);
        return [
          [
            {
              id: "backup",
              ...
              finished_at: "2026-08-14T12:00:00.000Z",
              isolated: 0,
              encrypted: 1,
              integrity_verified: 1,
              permission_boundary_verified: 0,
              audit_chain_verified: 0,
              evidence_hash_verified: 0,
            },
            {
              id: "drill",
      ...
      if (sql.includes("backup_recovery_assets")) {
        assert.equal(values[0], "backup");
        return [
          [
  ...
  const snapshot = await new MySqlResilienceProbe(
    pool,
    () => new Date("2026-08-14T13:00:00.000Z"),
  ).snapshot();
  assert.equal(snapshot.backupStatus, "verified");
  assert.equal(snapshot.actualRpoMinutes, 1);
  assert.equal(snapshot.actualRtoMinutes, 3);
  assert.equal(snapshot.recoveryDrillAgeDays, 0.04);
});

test("M08-03.A10/A13 upgrades the release gate to ROW binlog inclusion without changing canary thresholds", async () => {
  const [manifest, runner, verifier, schema, featureMap, plan, architecture, runbook] =
    await Promise.all(
      [
        "infra/baota/release-rollout-manifest.json",
  ...
  assert.equal(contract.mysqlDurability.binlogFormat, "ROW");
  assert.equal(contract.mysqlDurability.productScoutBinlogExcluded, false);
  assert.deepEqual(contract.canary.percentages, [5, 25, 100]);
  assert.equal(contract.canary.minimumObservationSeconds, 1800);
  assert.equal(contract.automaticStop.writeP95MsInclusive, 600);
  for (const source of Object.values(sources)) {
    assert.match(source, /binlogFormat|binlog_format|binlog-format/);
    assert.doesNotMatch(
      source,
      /binlog-ignore-db=product_scout|productScoutBinlogExcluded["']?\s*[:=]\s*true/,
    );
  }
});
...
test("M08-03.A10/A13 keeps historical probes immutable and writes new durability samples to a compact MySQL57 table", async () => {
  ...
  assert.match(up, /`sample_id` BINARY\(16\)/);
  assert.match(up, /`build_sha` BINARY\(20\)/);
  assert.match(up, /`nonce_hash` BINARY\(32\)/);
  assert.doesNotMatch(up, /PRIMARY KEY \(`id`\)/);
  assert.match(down, /DROP TABLE IF EXISTS `deployment_release_write_samples`/);
  assert.match(repository, /INSERT INTO deployment_release_write_samples/);
  assert.doesNotMatch(repository, /INSERT INTO deployment_release_write_probes/);
  assert.match(runner, /deployment_release_write_samples/);
  assert.match(runner, /0032a_compact_release_write_probe_m08_03\.up\.sql/);
  assert.match(verifier, /0033_file_resilience_m08_04\.up\.sql/);
  assert.match(schema, /0033_file_resilience_m08_04\.up\.sql/);
  assert.match(runbook, /历史探针|旧表/);
  assert.match(runbook, /只读|保留/);
});
...
test("M08-03.A07/A08/A15/A16 UI and production evidence cover full states and recovery", async () => {
  ...
  assert.doesNotMatch(
    `${ui}\n${e2e}\n${manifest}\n${runbook}`,
    /replica_enabled\s*[:=]\s*true|备用服务器已启用|异地灾备已启用/i,
  );
});
