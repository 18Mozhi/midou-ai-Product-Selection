...
import { ProfitServiceError, validateCostInput, validateCostRule } from '../../apps/api/dist/profit-service.js';
...
test('M04-04.A01/A02/A04/A12 requires every explicit fee and provenance',()=>{
  const rule=validateCostRule({market:'US',platform:'amazon',version_code:'v1',name:'规则一',effective_from:'2026-08-08',fee_lines:fees});
  assert.equal(rule.fee_lines.length,4);
  assert.throws(()=>validateCostRule({market:'US',platform:'amazon',version_code:'bad',name:'bad',effective_from:'2026-08-08',fee_lines:fees.slice(0,3)}),error=>error instanceof ProfitServiceError&&error.code==='cost_rule_fee_lines_invalid');
  const input=validateCostInput({platform:'amazon',input_type:'purchase_price',amount_value:40,currency:'CNY',source_type:'supplier_quote',source_ref_id:'quote-1',evidence_id:'00000000-0000-4000-8000-000000000404',observed_at:new Date().toISOString(),expected_version:1});
