...
for (const owner of ["retry", "notice"])
  test(`P66 ${owner} transfers its own focus without unnecessary scroll`, () => {
...
test("P66 retry never steals focus that the user moved elsewhere", () => {
...
for (const property of ["hidden", "disconnected", "inert"])
  for (const target of ["retry", "refresh"])
    test(`P66 ${property} ${target} prevents handoff`, () => {
      assert.deepEqual(fixture({ [property]: target }).calls, []);
    });

test("P66 unsuccessful focus does not scroll", () => {
...
for (const options of [{ top: -20 }, { top: 790 }, { covered: true }])
  test(`P66 focused but obscured target is scrolled into view ${JSON.stringify(options)}`, () => {
...
test("P66 single-flight guard precedes handoff and clearing the old retry region", () => {
