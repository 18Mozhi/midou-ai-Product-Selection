...
test("P34 first 500 presentation leaves parent business code and adjacent template intact", () => {
  const old = execFileSync("git", ["show", `${firstFailureBaseline}:${firstFailureParent}`], {
    encoding: "utf8",
  });
  const current = readFileSync(firstFailureParent, "utf8");
  ...
  const child = readFileSync(
    "apps/web/src/components/OrganizationApprovalFirstFailure.vue",
    "utf8",
  );
...
test("P34 actual first-failure evidence pins 4 breakpoints,52 unchanged views and8 real screenshots", () => {
