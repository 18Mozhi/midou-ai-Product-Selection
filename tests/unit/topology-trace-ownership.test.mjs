...
const file = readFileSync("apps/web/src/components/RuntimeTopologyCenter.vue", "utf8");
const source = parse(file).descriptor.scriptSetup.content;
const ast = ts.createSourceFile("topology.ts", source, ts.ScriptTarget.Latest, true);
    ...
    async seed(empty = false, id = "snapshot-original") {
      ...
      calls.at(-1).resolve({
        ...
        data: {
          ...
          nodes: empty ? [] : [{ node_id: "api-primary" }],
        },
      });
      await pending;
    },
...
for (const empty of [false, true])
  test(`P66 failed refresh preserves ${empty ? "empty" : "populated"} snapshot identity`, async () => {
    const h = harness();
    await h.seed(empty);
...
test("P66 first failure has no snapshot identity and later success clears failure", async () => {
  const h = harness();
...
test("P66 restarting read clears old failure without relabeling retained snapshot", async () => {
  const h = harness();
  await h.seed();
...
for (const seeded of [false, true])
  test(`P66 ${seeded ? "retained" : "initial"} timeout uses its own sent correlation ID`, async () => {
    const h = harness();
...
for (const kind of ["expired", "forbidden"])
  test(`P66 ${kind} clears snapshot ID with cleared facts`, async () => {
    const h = harness();
    await h.seed();
...
test("P66 unknown failure cannot reuse unrelated snapshot or failure identity", async () => {
  const h = harness();
  await h.seed();
...
test("P66 late result after unmount cannot replace accepted identities", async () => {
  const h = harness();
  await h.seed();
  const pending = h.subject.load();
  h.unmount();
...
test("P66 template labels snapshot and both failure consumers separately", () => {
