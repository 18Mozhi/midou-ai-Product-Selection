...
const source = readFileSync("apps/web/src/components/ApiCoverageDashboard.vue", "utf8"),
  ...
  operationCard = readFileSync("apps/web/src/components/ApiCoverageOperationCard.vue", "utf8");
test("P63 C review preserves dashboard script", () => {
  ...
  assert.match(preview.template.content, /class="api-coverage api-coverage--c"/);
});
test("P63 C review separates catalog state and five-dimensional operation evidence", () => {
  ...
  assert.match(template, /报告本身不等于全量生产验收/);
  assert.match(operationCard, /查看五维证据/);
  assert.match(operationCard, /item\.test_id/);
  assert.match(operationCard, /item\.latest_result/);
});
test("P63 production dashboard and operation card compile as typed Vue SFCs", () => {
  for (const [filename, content] of [
    ["apps/web/src/components/ApiCoverageDashboard.vue", source],
    ["apps/web/src/components/ApiCoverageOperationCard.vue", operationCard],
  ]) {
