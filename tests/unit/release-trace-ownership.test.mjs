    ...
    async seed(empty = false, id = "snapshot-original") {
      ...
      calls.at(-1).resolve({
        ...
        data: {
          ...
          gates: empty ? [] : [{ gate_kind: "canary_5" }],
        },
      });
      await pending;
    },
...
for (const empty of [false, true])
  test(`P65 failed refresh preserves ${empty ? "empty" : "populated"} snapshot identity`, async () => {
    const h = harness();
    await h.seed(empty);
...
test("P65 first failure has no snapshot identity and later success clears failure", async () => {
  const h = harness();
...
test("P65 restarting read clears old failure without relabeling retained snapshot", async () => {
  const h = harness();
  await h.seed();
...
for (const seeded of [false, true])
  test(`P65 ${seeded ? "retained" : "initial"} timeout uses its own sent correlation ID`, async () => {
    const h = harness();
...
for (const kind of ["expired", "forbidden"])
  test(`P65 ${kind} clears snapshot ID with cleared facts`, async () => {
    const h = harness();
    await h.seed();
...
test("P65 unknown failure cannot reuse unrelated snapshot or failure identity", async () => {
  const h = harness();
  await h.seed();
...
test("P65 late result after unmount cannot replace accepted identities", async () => {
  const h = harness();
  await h.seed();
  const pending = h.subject.load();
  h.unmount();
...
test("P65 template labels snapshot and both failure consumers separately", () => {
