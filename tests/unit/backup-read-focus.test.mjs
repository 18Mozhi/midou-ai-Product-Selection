...
const source = readFileSync("apps/web/src/components/BackupRecoveryCenter.vue", "utf8");
const script = parse(source).descriptor.scriptSetup.content;
const ast = ts.createSourceFile("backup.ts", script, ts.ScriptTarget.Latest, true);
...
for (const owner of ["retry", "notice"])
  test(`P64 ${owner} transfers its own focus without unnecessary scroll`, () => {
...
test("P64 retry never steals focus that the user moved elsewhere", () => {
...
for (const property of ["hidden", "disconnected", "inert"])
  for (const target of ["retry", "refresh"])
    test(`P64 ${property} ${target} prevents handoff`, () => {
      assert.deepEqual(fixture({ [property]: target }).calls, []);
    });

test("P64 unsuccessful focus does not scroll", () => {
...
for (const options of [{ top: -20 }, { top: 790 }, { covered: true }])
  test(`P64 focused but obscured target is scrolled into view ${JSON.stringify(options)}`, () => {
...
test("P64 single-flight guard precedes handoff and clearing the old retry region", () => {
