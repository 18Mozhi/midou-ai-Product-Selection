...
test("P43 current replay keeps raw source hashes, original checks, images and request evidence separate", () => {
  const e = JSON.parse(read(`${userLifecycleReviewReplayRoot}/evidence.json`));
  ...
  for (const item of e.summaries) {
    ...
    assert.equal(
      current.transformedHashes[child],
      hash(
        item.stage === "create-current"
          ? userCreatePreview(userPasswordPreview(read(child)))
          : userPasswordPreview(read(child)),
      ),
    );
    assert.deepEqual(
