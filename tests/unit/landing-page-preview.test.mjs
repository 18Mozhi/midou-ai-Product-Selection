...
test("P01 C review preserves landing resolution script", async () => {
  ...
  assert.match(reviewed, /@primary="resolveLanding"/);
  assert.match(reviewed, /request-id="requestId"/);
});
test("P01 C review separates loading and retry boundaries", async () => {
  const reviewed = previewLandingPage(await readFile(file, "utf8"));
  assert.match(reviewed, /state === 'loading' \? 'loading' : 'blocked'/);
  assert.match(reviewed, /加载时不展示可重复动作/);
});
