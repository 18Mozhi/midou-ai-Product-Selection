...
const source = readFileSync("apps/web/src/components/ReleaseRolloutCenter.vue", "utf8");
const script = parse(source).descriptor.scriptSetup.content;
const ast = ts.createSourceFile("release.ts", script, ts.ScriptTarget.Latest, true);
...
for (const owner of ["retry", "notice"])
  test(`P65 ${owner} transfers its own focus without unnecessary scroll`, () => {
...
test("P65 retry never steals focus that the user moved elsewhere", () => {
...
for (const property of ["hidden", "disconnected", "inert"])
  for (const target of ["retry", "refresh"])
    test(`P65 ${property} ${target} prevents handoff`, () => {
      assert.deepEqual(fixture({ [property]: target }).calls, []);
    });

test("P65 unsuccessful focus does not scroll", () => {
...
for (const options of [{ top: -20 }, { top: 790 }, { covered: true }])
  test(`P65 focused but obscured target is scrolled into view ${JSON.stringify(options)}`, () => {
...
test("P65 single-flight guard precedes handoff and clearing the old retry region", () => {
