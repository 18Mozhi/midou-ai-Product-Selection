...
import {
  ...
  validateQuote,
  validateSearch,
} from "../../apps/api/dist/sourcing-service.js";
test("M04-06.A01/A02/A04/A12 validates supported inputs complete quotes and two-to-five comparison", () => {
  assert.equal(
    validateSearch({ input_type: "keyword", input_ref: "净水杯" }).input_type,
    "keyword",
  );
  assert.equal(
    validateComparison({
      name: "对比",
      quote_ids: ["00000000-0000-4000-8000-000000000602", "00000000-0000-4000-8000-000000000603"],
    }).quote_ids.length,
    2,
  );
  assert.throws(
    () =>
      validateComparison({
        name: "过多",
        quote_ids: Array.from({ length: 6 }, (_, i) => `00000000-0000-4000-8000-00000000060${i}`),
      }),
    (e) => e instanceof SourcingServiceError && e.code === "sourcing_comparison_size_invalid",
  );
  assert.equal(
    validateQuote({
      candidate_id: "00000000-0000-4000-8000-000000000604",
      specification: "500ml 蓝色",
      moq: 100,
      lead_time_days: 7,
      location: "浙江",
      confidence_value: 85,
      ...
      evidence_id: "00000000-0000-4000-8000-000000000605",
    }).location,
    "浙江",
  );
});
test("M04-06.A03/A05-A11/A13-A17 delivery evidence exists", async () => {
  const paths = [
      "database/migrations/0017f_sourcing_m04_06.up.sql",
      "database/migrations/0017f_sourcing_m04_06.down.sql",
  ...
  assert.match(
    up,
    /sourcing_searches[\s\S]*sourcing_candidates[\s\S]*supplier_quotes[\s\S]*sourcing_comparisons[\s\S]*sourcing_purchase_tasks/,
  );
  assert.match(down, /DROP TABLE IF EXISTS `sourcing_searches`/);
  assert.match(service, /validateQuote[\s\S]*validateComparison/);
  ...
  assert.match(
    live,
    /INSERT INTO collection_task_evidence_links[\s\S]*VALUES \(\?,\?,\?,\?,\?,\?,\?,\?,'captured',\?,\?,\?,\?\)/,
  );
  assert.match(
    live,
    /SELECT status,last_error_code FROM sourcing_projection_jobs WHERE id=\? AND organization_id=\? AND workspace_id=\?/,
  );
  assert.doesNotMatch(live, /new MySqlSourcingProjectionWorker/);
...
test("M04-06 saved comparisons align specification MOQ price and lead time without inventing login renewal", async () => {
  ...
  assert.match(feature, /sourcingDiscovery[\s\S]*pageFlow/);
  assert.match(runbook, /四阶段导航[\s\S]*390px/);
  assert.match(e2e, /600ml \/ 绿色 \/ 双滤芯[\s\S]*12 天/);
});
