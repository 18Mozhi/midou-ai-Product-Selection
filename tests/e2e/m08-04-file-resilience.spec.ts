    ...
    await route.fulfill({
      ...
      json: {
        error: {
          ...
          message: "本机文件事实暂不可用。",
          action_hint: "在宝塔检查 Node API、受控目录与 MySQL 后重新核验。",
        },
        request_id: "m08-04-refresh-failure",
        trace_id: "m08-04-refresh-failure",
      },
    });
  });
  await page.goto("/platform-admin/files");
  await expect(page.getByText("本机文件韧性门已满足")).toBeVisible();
  const refresh = page.getByRole("button", { name: "刷新文件事实" });
  await refresh.click();
