...
test('M00-04.A07/A15 Redis foundation is visually stable', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Redis 基座' })).toBeVisible();
...
test('M00-04.A08/A16 dependency failure and recovery are explicit at 390px', async ({ page }) => {
  ...
  await expect(page.getByText('UNAVAILABLE · 已阻塞')).toBeVisible();
  await page.getByRole('button', { name: '恢复中' }).click();
  await expect(page.getByText('RECOVERING · 恢复检查')).toBeVisible();
  if (page.viewportSize()?.width === 390) await expect(page).toHaveScreenshot('m00-04-redis-390.png', { fullPage: true });
});
