...
async function base(page: any) {
  await page.route("**/api/v1/me/navigation?**", (route: any) =>
    route.fulfill({
      ...
      body: JSON.stringify({ data: navigation, request_id: "m02-05-nav", trace_id: "m02-05-nav" }),
    }),
  );
  await page.route("**/api/v1/me/home-dashboard", (route: any) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        request_id: "m02-05-home",
        trace_id: "m02-05-home",
      }),
    }),
  );
}
...
test("M02-05.A07/A08/A09/A15 mobile quick create shows authorized entries only", async ({
  page,
}) => {
  ...
  await page.route("**/api/v1/me/quick-actions?**", (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        request_id: "m02-05-actions",
        trace_id: "m02-05-actions",
      }),
    }),
  );
  await page.goto("/home");
  ...
  await expect(dialog.getByRole("link", { name: /发起找货/ })).toHaveAttribute(
    "href",
    "/sourcing?create=1",
  );
      ...
      status === 401
        ? {
            ...
            body: JSON.stringify({
              ...
              request_id: "m02-05-expired",
              trace_id: "m02-05-expired",
            }),
          }
        : {
            status: 200,
            ...
            body: JSON.stringify({
              ...
              request_id: "m02-05-empty",
              trace_id: "m02-05-empty",
            }),
          },
    ),
  );
