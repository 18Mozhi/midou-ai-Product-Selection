...
test('M00-07.A15 verification dashboard is visually stable', async ({ page }) => {
  ...
  await expect(page.getByText('PASSED · 通过')).toBeVisible();
  await expect(page.getByText('FAILED · 失败')).toBeVisible();
  await expect(page.getByText('BLOCKED · 前置未满足')).toBeVisible();
