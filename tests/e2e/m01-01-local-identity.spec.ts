...
test('M01-01 regression: successful administrator login enters the operation panel instead of device sessions', async ({ page }) => {
  await page.route('**/api/v1/auth/login', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { user: { id: 'admin' }, session: { id: 'session' }, security_setup: { required: false } }, request_id: 'login-admin', trace_id: 'login-admin' }) }));
  await page.route('**/api/v1/me/landing', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { shell: 'platform_admin', route: '/platform-admin', reason: 'landing_platform_admin' }, request_id: 'landing-admin', trace_id: 'landing-admin' }) }));
  await page.goto('/login');
