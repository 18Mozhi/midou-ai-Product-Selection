...
test("M07-06.A08/A12/A16 validates input and refuses deadline relaxation", async () => {
  const { SelectionJourneyService } =
    await import("../../apps/api/dist/selection-journey-service.js");
  const repository = {
  ...
  assert.throws(() => new SelectionJourneyService(repository, 180001), {
    code: "selection_deadline_contract_invalid",
  });
  const service = new SelectionJourneyService(
    repository,
    180000,
  ...
  assert.throws(
    () => service.create({ ...context, value: { input_kind: "asin", input_value: "short" } }),
    { code: "selection_asin_invalid" },
  );
  assert.throws(
    () =>
      service.create({
        ...context,
        value: { input_kind: "product_url", input_value: "http://example.com/product" },
      }),
...
test("M07-06.A04/A08 keeps a journey running until the collection task is terminal", async () => {
  ...
  assert.ok(
    resultGate > taskGate,
    "persisted evidence cannot expose result_ready while the task is retrying",
  );
  assert.match(
    repository,
...
test("M07-06.A09/A16 production runner selects tenant context before member guard", async () => {
  ...
  assert.ok(
    ...
    "production runner must resolve a workspace from the selected organization",
  );
  assert.ok(
    context > workspaces,
    "production runner must bind the new login session to organization/workspace context",
  );
  assert.ok(
    guard > context,
...
test("M07-06.A04/A05/A14 links deduplicated evidence to every scoped collection task", async () => {
  const { MySqlEvidencePersistence } =
  ...
  const connection = {
    ...
    query: async (sql) => {
      ...
      if (sql.includes("FROM raw_evidence e"))
        return [
          [
            {
    ...
    provenance: [
      {
        fieldPath: "title",
        sourcePath: "$.title",
        transformVersion: "parser-v1",
        sourceValueHash: contentHash,
      },
    ],
  ...
  assert.ok(
    statements.some(
      (sql) =>
        typeof sql === "string" && sql.includes("INSERT INTO collection_task_evidence_links"),
    ),
    "deduplicated evidence must be linked to the current task",
  );
  assert.ok(
    statements.some((sql) => typeof sql === "string" && sql.includes("FROM collection_tasks t")),
    "task, subquery, provider and tenant scope must be validated before linking",
  );
  ...
  const linkedEvent = changedContentStatements.find(
    (entry) =>
      typeof entry === "object" &&
      entry.sql.includes("INSERT INTO evidence_data_events") &&
      entry.values?.[3] === "evidence.linked",
  );
  ...
  await assert.rejects(
    () =>
      changedFieldsPersistence.persist({
        ...evidenceInput,
  ...
  const rejectedPersistence = new MySqlEvidencePersistence(
    { getConnection: async () => rejectedConnection },
    "unused",
    1024,
  );
  await assert.rejects(() => rejectedPersistence.persist(evidenceInput), {
    code: "evidence_scope_or_task_invalid",
  });
  assert.ok(
    !rejectedStatements.some(
      (sql) =>
        typeof sql === "string" && sql.includes("INSERT INTO collection_task_evidence_links"),
    ),
    "invalid task scope must never create an evidence association",
  );
...
test("M03-07 automatic feeds skip conflicting already-seen items while M07-06 required journeys stay explicit", async () => {
  ...
  const evidence = {
    persist: async (input) => {
      ...
      if (allRecordsAlreadySeen || input.dedupeKey === "conflict")
        throw new EvidencePersistenceError("evidence_dedupe_conflict");
      return {
        evidence_id: input.dedupeKey,
        normalized_record_id: input.dedupeKey,
        deduplicated: false,
      };
    },
  };
  ...
  assert.deepEqual(
    ...
    "a conflicting record must not discard later independent records",
  );
  ...
  assert.deepEqual(
    replayUpdates[0]?.slice(0, 3),
    ["succeeded", 2, null],
    "automatic non-required feeds must not retry an entire source batch",
  );
