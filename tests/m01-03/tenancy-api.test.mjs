...
async function fixture() {
  ...
  const auth = {
    authenticate: async (token) => {
      if (token === "token-a")
        return {
          user: { id: userA, status: "active" },
          session: { id: "00000000-0000-4000-8000-000000000313" },
        };
      if (token === "token-b")
        return {
  ...
  const context = { requestId: "setup", traceId: "setup" };
  const first = await service.provisionOrganization(
    {
      ...
      defaultWorkspaceName: "新品工作区",
      defaultWorkspaceSlug: "new-products",
    },
    { ...context, actorId: userA },
  );
  const second = await service.provisionOrganization(
    {
      ...
      defaultWorkspaceSlug: "isolated",
    },
    { ...context, actorId: userB },
  );
  const seen = new Map();
  const execute = async (input, work) => {
    const key = `${input.route}:${input.scope}:${input.key}`;
  ...
  return { app, repository, first, second };
}
const cookie = { cookie: "scoutops_session=token-a" };

test("M01-03.A06/A09 membership-backed lists use only current HttpOnly session identity", async () => {
  ...
  assert.equal(organizations.json().data.length, 1);
  assert.equal(organizations.json().data[0].id, f.first.organization.id);
  const workspaces = await f.app.inject({
    method: "GET",
    url: `/api/v1/org/${f.first.organization.id}/workspaces`,
    headers: cookie,
  });
  assert.equal(workspaces.statusCode, 200);
  assert.equal(workspaces.json().data[0].id, f.first.workspace.id);
  const teams = await f.app.inject({
    method: "GET",
    url: `/api/v1/org/${f.first.organization.id}/teams`,
    headers: cookie,
  });
  assert.equal(teams.statusCode, 200);
  assert.deepEqual(teams.json().data, []);
  await f.app.close();
});
...
test("M01-03.A08/A09/A16 foreign organization access is forbidden with stable envelope", async () => {
  ...
  const response = await f.app.inject({
    method: "GET",
    url: `/api/v1/org/${f.second.organization.id}/workspaces`,
    headers: cookie,
  });
  assert.equal(response.statusCode, 403);
  assert.equal(response.json().error.code, "organization_forbidden");
  assert.ok(response.json().request_id);
  assert.ok(response.json().trace_id);
  await f.app.close();
});

test("M01-03.A06/A11 context selection requires origin/idempotency and replays without duplicate audit", async () => {
  const f = await fixture();
  const payload = { organization_id: f.first.organization.id, workspace_id: f.first.workspace.id };
  ...
  const headers = {
    ...
    "idempotency-key": "select-context-1",
  };
  ...
  assert.equal(first.json().data.workspace.id, f.first.workspace.id);
