...
test('M03-05.A03/A07/A10/A11/A14-A17 delivery surfaces are complete and Baota bounded',async()=>{
  const paths=['database/migrations/0016e_collection_tasks_m03_05.up.sql','database/migrations/0016e_collection_tasks_m03_05.down.sql','packages/collection-tasks/src/index.ts','apps/worker/src/collection-task-worker.ts','apps/api/src/mysql-collection-task-repository.ts','apps/api/src/collection-task-routes.ts','apps/web/src/components/CollectionTaskCenter.vue','apps/web/src/collection-tasks.css','docs/openapi.yaml','config/env.example','config/schema.json','docs/architecture/m03-05-collection-task-state-machine.md','docs/runbooks/m03-05-collection-task-state-machine.md','docs/feature-map.json','tests/e2e/m03-05-collection-tasks.spec.ts','new-product-enterprise-blueprint.md'];
  const values=await Promise.all(paths.map(path=>readFile(path,'utf8'))),[up,down,domain,worker,repo,routes,web,css,openapi,env,schema,architecture,runbook,feature,e2e,blueprint]=values;
  ...
  assert.match(down,/DROP TABLE IF EXISTS `collection_tasks`/);assert.match(domain,/retryAvailableAt/);assert.match(worker,/FOR UPDATE/);assert.match(repo,/manually_replayed/);assert.match(routes,/collection:replay/);assert.match(web,/loading.*ready.*empty.*error.*expired.*forbidden.*blocked/);assert.match(css,/@media\(max-width:760px\)/);assert.match(openapi,/\/platform\/collection\/tasks:/);assert.match(env,/COLLECTION_TASK_LEASE_SECONDS=120/);assert.match(schema,/collectionTasks/);assert.match(architecture,/M03-06/);assert.match(runbook,/宝塔.*Node Worker/s);assert.match(feature,/collectionTaskStateMachine/);assert.match(e2e,/toHaveScreenshot/);assert.match(blueprint,/M03-05/);
});
