...
- A07/A08/A15 重跑全部页面场景，并在 desktop-chromium 与 mobile-390 两个项目上执行；其中真实 API 截图用浏览器经 Vite 访问实际 Fastify，不允许路由拦截。带路由拦截文件的截图调用占比必须低于 50%，只能承担视觉回归，不能独立证明功能链路。一次性生产验收另补四张不拦截请求的核心链路截图，先证明 MySQL 与 Redis 均为 `available`，再覆盖“趋势→建议采纳机会→人工采纳→任务”、任务移动端次要操作、“任务审批决策台”和“找货候选→报价确认→成本/利润”；当前机器统计为 19/39，即 48.72%。成员与平台壳层复用移动遮挡 helper，末端内容与固定底栏重叠必须为 0；M07-01 没有单独业务页面。
