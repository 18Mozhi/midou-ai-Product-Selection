...
验证输出必须包含 `run_id` 与 `trace_id`。失败立即返回非零；测试数据由各 live 脚本按组织隔离创建并在结束时清理。浏览器矩阵固定 4 worker，避免 46 个页面场景同时争抢本地渲染资源造成时钟与动画噪声；这不是生产并发配置。配置只复用 `VERIFY_COMMAND_TIMEOUT_MS` 和 `VERIFY_REPORT_DIR`，没有新增环境变量。
