...
前端按成员、角色和审批治理拆分展示边界：`OrganizationAdminCenter.vue` 保留路由、API、审计写操作与跨分区编排；`OrganizationMemberPanel.vue` 展示邀请、成员筛选和成员动作；`OrganizationRolePanel.vue` 展示固定角色与能力矩阵；`OrganizationApprovalPanel.vue` 展示模板版本差异和审批记录。子组件不直接请求接口，也不改变成员、角色或审批合同。
