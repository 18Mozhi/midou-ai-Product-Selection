...
采购成员以 `supplier_quote:manage` 补充 MOQ、规格、交期、所在地、可信度、稳定性、风险和证据，产生不可变 `supplier_quotes` 版本；旧版本不覆盖。报价确认使用当前搜索范围内的真实证据编号可搜索选择器，现行报价卡固定展示该版本的 MOQ 与 `observed_at` 新鲜度。对比只接受当前工作区至少两家、最多五家现行完整报价，并在每个已保存对比中使用相同字段顺序展开供应商、规格、MOQ、报价和交期，不自动评选“最优”。采购创建使用结构化表单展示并锁定报价版本、MOQ 和证据编号，数量不得低于已确认 MOQ。搜索可重新采集或审计软删除，不删除候选证据。
