...
页面把真实流程固定为搜索货源、确认报价、对比供应商、创建采购任务四阶段，并保留列表与详情主从布局。搜索条件和当前找货记录分别使用 `q` 与 `record` 查询参数恢复。候选卡优先显示报价、MOQ、交期和“待费用规则计算”的到岸价，不在浏览器估算费用；缺失字段逐项指向报价确认动作。已选报价数以固定栏持续显示，2–5 家时才允许保存对比。费用规则入口携带已校验的 `from` 路径，规则页可返回当前找货记录。移动端候选使用两列定义列表，外部原页明确标注新窗口并展示真实 `observed_at`。
