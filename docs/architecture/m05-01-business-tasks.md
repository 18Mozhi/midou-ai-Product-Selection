...
任务列表使用 `/tasks`，任务详情使用可直接分享和恢复的 `/tasks/{taskId}` 独立前端路由；详情仍消费既有 `GET /api/v1/tasks/{taskId}`，不新增 API 或持久化字段。列表状态与分页分别同步到 `status`、`page` 查询参数，详情链接携带校验后的 `from` 并返回今日工作或原筛选列表。列表行和详情内的删除入口都收进命名操作菜单，避免与打开/运行任务的主操作竞争；进度主表达使用待开始、执行中、已暂停、已完成或已结束阶段，既有百分比只保留为辅助进度条事实。SLA 同时展示期限、等级和下一步，事件与评论按真实时间合并为任务活动；审批只有进入任务事实后才会出现在该活动流，不额外拼造审批记录。
