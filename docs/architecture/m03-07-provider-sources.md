...
`/platform-admin/providers/sources/1688-acceptance` 是 1688 专用验收页，只聚合真实数据库事实：有效且未过期的浏览器档案、最近一次登录态浏览器运行是否成功/是否被验证码阻断、当前解析器版本固定样本回放，以及代码中已登记的负责人和逐项待配置原因。页面还读取最新一条真实浏览器作业形成搜索、商品详情和翻页覆盖矩阵：搜索与详情必须分别通过 `1688.search.v1`、`1688.offer-detail.v1` 实际解析才标记覆盖；翻页对照执行计划的 `max_pages` 与持久化 `page_count`，当前上限为 1 时明确显示“未演练”，不得宣称已覆盖分页。矩阵只提供验收事实，不替代或放宽登录、验证码和当前解析器固定样本三项启用门。页面与 API 不返回 Cookie、凭证密文、档案 UUID、租约、浏览器输入或原始快照。1688 从停用切为启用时必须同时满足三项门，旧的解析回放或仅存在档案均不能单独放行。
...
固定样本只能从已成功且同时具有结构化快照、截图、DOM 片段和单一解析版本的真实浏览器作业生成；前端不能上传任意 JSON 冒充生产样本。服务端保存不可变输入、规范化基线及哈希，回放时用当前解析器重新处理并保存最多 200 条逐路径差异。样本初始为待审批，必须由另一个具有 `provider:configure` 的管理员给出通过或驳回结论，创建人不能自审。驳回样本继续可见并保留审计，修正必须从新的真实作业创建不可变样本。1688 启用门禁只接受审批通过且当前解析版本最近一次回放为通过的活动样本；后续差异或失败会立即使该样本不再满足门禁。
