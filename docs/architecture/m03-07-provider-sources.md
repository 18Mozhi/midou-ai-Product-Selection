...
`/platform-admin/providers/sources/1688-acceptance` 是仅供 `platform:superadmin` 使用的 1688 启用检查页，不进入主导航，只聚合真实数据库事实：有效且未过期的浏览器档案、最近一次登录态浏览器运行是否成功/是否被验证码阻断、当前解析器版本固定样本回放，以及代码中已登记的负责人和逐项待配置原因。页面可从当前账号仍为活动成员的组织和活动工作区中选择范围、填写关键词并人工发起一次登录验收运行。该写入仍走 `collection:replay`、同源与幂等保护；只有持久化 Provider 代码严格等于 `1688_search` 且目标包含字面量 `acceptance_run=true` 时，仓储、Worker 和 Crawler 租约才允许停用来源完成这一次作业，浏览器作业同时记录 `purpose=acceptance`。这条例外不会启用来源，自动调度器也不会生成该标记。页面还读取最新一条真实浏览器作业形成搜索、商品详情和翻页覆盖矩阵：搜索与详情必须分别通过 `1688.search.v1`、`1688.offer-detail.v1` 实际解析才标记覆盖；翻页对照执行计划的 `max_pages` 与持久化 `page_count`，当前上限为 1 时明确显示“未演练”，不得宣称已覆盖分页。矩阵只提供运行证据，不替代或放宽登录、验证码和当前解析器固定样本三项启用门。页面把状态枚举转换为业务中文，原始枚举只保留在折叠的技术详情中；刷新具有 12 秒前端超时和重复触发保护，失败或超时会保留上一次成功读取的启用条件并允许恢复后重试。凭证入口携带准确 Provider 标识并直接打开登录档案表单，固定样本入口携带 `provider_id` 将来源目录收敛到 1688。页面与 API 不返回 Cookie、凭证密文、档案 UUID、租约、浏览器输入或原始快照。1688 从停用切为启用时必须同时满足三项门，旧的解析回放或仅存在档案均不能单独放行。
