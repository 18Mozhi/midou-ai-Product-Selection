...
列表查询以当前 HttpOnly 会话解析出的 user_id 为唯一身份来源。组织列表同时过滤活动成员资格与活动组织，已归档组织不会先显示、进入时再返回 403；工作区和团队路由仍再次校验成员资格与组织状态。上下文选择还验证工作区属于该组织且未归档，并以 MySQL 5.7 `ON DUPLICATE KEY UPDATE` 保存单一会话上下文。外部请求不能提交 actor_id 或 session_id。
...
M01-03 只确定当前会话的组织与工作区范围；角色、动作与数据范围授权继续由 M01-04 在服务端判定，租户选择本身不授予额外业务权限。
