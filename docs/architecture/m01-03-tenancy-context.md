...
页面覆盖加载、空组织、自助创建中、空工作区、接口错误、登录失效、无权、归档工作区、选择中和选择成功；组织选择阶段只展示组织搜索与组织卡片，工作区和团队摘要仅在明确选中组织后出现。错误保留 request_id 供排查。审计事件只保存范围、动作、资源、actor、request_id/trace_id 和时间，不保存 Cookie。
