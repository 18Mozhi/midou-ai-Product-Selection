...
组织创建仅由 `TenancyService.provisionOrganization` 作为内部事务能力提供，为后续平台管理模块复用；M01-03 不暴露普通用户创建 API。团队在本模块仅为组织级实体和只读摘要，不猜测工作区归属或团队成员关系。角色、动作和本人/团队/工作区/组织/平台数据范围由 M01-04 定义。

## 数据与同步真相

`0010a`–`0010g` 分步创建组织、工作区、团队、成员资格、会话上下文、租户审计及组织默认工作区外键。拆分默认工作区外键是为避免组织和默认工作区的循环建表依赖；创建事务先插入默认工作区为空的组织，再插入工作区、回填默认工作区、创建创建者成员资格并写审计，任一步失败全部回滚。组织 slug 全局唯一，工作区 slug 在组织内唯一。
...
页面覆盖加载、空组织、空工作区、接口错误、登录失效、无权、归档工作区、选择中和选择成功；错误保留 request_id 供排查。审计事件只保存范围、动作、资源、actor、request_id/trace_id 和时间，不保存 Cookie。
