...
工具栏“重置”和过滤空态“清除筛选”共用既有resetFilters：query清空、接入模式/来源状态/登记状态/健康状态恢复all、排序恢复attention；watch使页码回1。两入口都只重排当前已读取目录，不触发新GET、健康探针或持久化偏好。UI2-PR47四类隔离Vue用例覆盖搜索、来源状态、登记和组合条件的空结果恢复；这不证明全部模态、异步读取或真实授权通过。

第二阶段P46/P47事实规格及动作/弹窗/输入合同位于`design-plans/ui-phase-2-2026-09-07/provider-definition-contract-review.md`。ProviderAdapterCenter当前仅声明compatibility_matrix类型，没有兼容矩阵渲染或打开按钮；下述矩阵UI属于热点来源入口。P46来源定义编辑器与P47健康运行页不可按一个配置表单设计，正式新布局与全站验收仍待交付。
