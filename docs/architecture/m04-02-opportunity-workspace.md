...
创建事务同时写 `opportunities`、刷新任务、事件、Outbox 和幂等操作。趋势来源必须是同一组织、工作区的活动 `trend_topic`。宝塔管理的 Node Worker 以 MySQL 5.7 租约刷新证据链接，只引用已存在的 `trend_signals` 与 `raw_evidence`，不复制或删除原始证据。
