...
API Client 创建、轮换和撤销在写入前展示令牌权限风险预览。预览只解释当前真实 `status:read` scope、组织范围、分钟配额和操作后果：创建说明只读边界以及服务端决定的配额/到期时间，轮换说明权限与配额不变且旧密钥立即失效，撤销说明访问立即终止且不可恢复。页面不虚构未开放 scope，也不使用未经配置的高/中/低风险分级；服务端校验、版本锁和审计仍是最终边界。
