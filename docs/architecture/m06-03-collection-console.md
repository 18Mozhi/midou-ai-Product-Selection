...
最近尝试和死信分别使用 `attempt_page`、`dead_letter_page` 独立分页，页大小沿用 `COLLECTION_CONSOLE_RECENT_LIMIT`（10–200），接口同时返回两组精确总数、页码和总页数。筛选、精确根因和页码写入 URL，因此刷新、返回和共享链接保持同一事实范围；切换范围或根因时两组页码回到第一页。页面刷新具有 15 秒上限并阻止重复请求；已有数据时失败或超时只显示告警并保留上一份已验证事实。
...
来源与健康先按受阻、严重、降级、警告或连续失败，其次正常，最后尚未检查的顺序排列，默认显示前 8 个；标题保留完整来源总数，操作者可在当前区块查看全部并再次收起。来源筛选下拉仍使用完整 `source_options`，渐进展示不改变服务端过滤、健康事实、来源配置或任务关联。
...
确认后对话框立即关闭且同批次只允许一个执行循环，防止重复点击再次发起整批请求。达到 20 条后，第 21 个复选框会立即恢复未选状态；分页时只保留当前页仍可见且开放的选择。失败条目逐条给出可执行提示，成功条目从选择中移除。
