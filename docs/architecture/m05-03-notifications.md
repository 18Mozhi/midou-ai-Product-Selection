...
任务、审批、竞品、系统分类，处理状态、仅未读和分页分别同步到 `category`、`status`、`unread`、`page`，当前通知使用 `notification` 深链恢复；升级前已产生的 `notification_id` 会在读取成功后规范化为 `notification`。列表和详情同时展示阅读状态与处理状态，同根因组明确标注合并数量；任务通知直达 `/tasks/:taskId`，`approval` 和 `approval_request` 直达审批详情，其他受支持资源链接附带校验后的通知 `from` 路径。没有可验证资源路由的系统通知不显示来源链接，不会把成员导向无权的平台页。详情使用模态对话框、支持 Escape 关闭和焦点返回，已读、处理状态和全部已读操作在请求期间禁用重复提交。当前持久化处理状态不包含 ignored，页面不会把关闭伪装成忽略。
补采触发的机会评分完成后使用现有全局 Outbox 投影站内提醒：事件为 `task.evidence_completion.redecision_ready`，资源类型固定 `opportunity`，收件人由评分事务根据机会负责人或补采任务执行人确定。通知正文只声明补采与重新评分已经完成并要求人工重新决策，不把评分结果自动解释为采纳结论；同一事件和收件人仍受既有唯一键去重，入口沿资源路由直达机会详情。
