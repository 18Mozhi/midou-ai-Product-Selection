...
页面创建入口统一为“添加竞品监控”，并按商品链接、市场信息、确认采集三步展示；步骤只组织既有 `market`、`product_url`、`title` 和可选 `opportunity_id`，不改变 API 合同。详情把价格与库存快照合并为带采集时间的时间轴，首次采集明确显示排队、采集中或失败并提供重新采集。每条真实变化事件可直接创建高优先级人工验证任务，任务说明锁定竞品、变化事件、字段前后值、证据编号和采集时间，不覆盖原快照；创建后可直达任务详情。专属规则入口预选当前竞品；外部商品链接明确标注新窗口，移动端暂停与删除进入“更多操作”。列表搜索和当前竞品分别使用 `q`、`competitor` 查询参数恢复。
