...
每次状态变化在同一事务写 `collection_task_events` 和 `collection_task_outbox`，保留 organization/workspace、request_id/trace_id、操作主体和脱敏元数据。死信重放只允许 `collection:replay`，要求同源 Origin、Idempotency-Key 和 2–500 字原因；新任务复制内部子查询，原任务改为 `manually_replayed`，全部历史不覆盖。相同 actor、任务路由和 Idempotency-Key 的并发请求在任务锁之后再次执行当前读：若首个请求已经提交操作记录，后到请求返回同一个结果任务，而不是创建第二条任务或因原任务状态已经变化返回 409。
...
采集任务桌面端保留队列表格；390px 改为状态、覆盖与证据摘要卡片，详情抽屉保留子查询计数、缺失字段和进入完整任务详情的操作。列表使用服务端 50 条分页并显示总数、当前区间和页数，状态筛选覆盖全部持久化状态；文本框明确只筛选当前页，不能把浏览器局部匹配冒充全库搜索。刷新、离线、依赖失败和 15 秒读取超时保留已经验证的当前页事实并提供重试。`page`、`status` 和 `task` 写入 URL，详情刷新可恢复、返回可关闭。

完整任务详情使用具名 `dialog`、遮罩、焦点进入与返回、Tab 约束和 Escape 关闭。业务标题只显示短任务标识，完整任务、组织、工作区、错误与请求标识仅归入“技术标识与关联信息”。详情读取开始时先清除上一任务，不允许失败响应继续展示旧任务。详情逐条展示子查询的真实结果数、缺失字段及 `started_at` 到 `finished_at` 的耗时，并把 RSS 空成功、无新内容和解析失败按审计分类显示；尚未完成的子查询只显示开始时间，不用当前浏览器时间推断耗时。每个子查询明确展示是否允许自动重试；进入 `retry_scheduled` 或 `rate_limited` 后，所有可重试子查询展示任务事实源中的共同 `available_at`，并标明这是任务级调度时间，不伪造来源独立退避。失败或受阻任务提供与现有页面和权限一致的恢复入口：登录/验证码进入浏览器档案，robots/页面变化进入来源设置，终止失败或部分完成进入来源健康，死信进入原有确认重放。
