...
第二阶段UI局部修复：没有容量快照时也显示独立签认结果，不再隐藏具体失败提示。UI2-SC71验证取消不提交、409提示可读和同一幂等键重试；这些是隔离响应，不执行真实演练或生产签认。无新配置/迁移或当前重启要求，后续仍按固定 `python scripts/deploy-baota.py` 整体发布和既有停启预检，Web回退不能执行下方数据库down迁移。

证据边界补充：当前API仓储仅选择最新production_benchmark行，读取查询没有校验BUILD_SHA；签认服务检查归档/恢复标志，未执行完整容量评价。上文同提交/有效证据要求是运营测量与发布链要求，不能把页面可见或签认返回成功本身当作证明。后续需核对提交身份、时效及服务快照与事务选中记录的对应，未核验前不宣称这些保证已经成立，不在本次UI修复中改动安全或持久化契约。

- 页面刷新只允许一个在途 GET。15 秒客户端上限或 14 秒 API 上限触发后，保留最后成功快照并显示可重试告警；不要反复点击或把旧快照当作新测量。
- `capacity_boundary_dependency_unavailable` 表示依赖读取失败；通过 request_id/trace_id 在宝塔 Node 日志核对 MySQL 和容量证据，不从浏览器或日志复制连接密钥。
- 401/403 后页面必须清除容量事实；重新登录或补齐 `platform:operate` 后再读取，不能把权限失败降级为缓存展示。
- 签认按钮在请求期间禁用且关闭确认框；网络或 5xx 结果不确定时，页面复用原 Idempotency-Key。POST 成功但跟随 GET 失败时，成功消息、旧快照和刷新告警必须同时保留。
