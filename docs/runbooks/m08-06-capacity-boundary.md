...
第二阶段 UI 批68仅提供 P71 隔离实际 Vue C 审核组合：原读取、权限、快照、签认合同和确认调用保持不变，以本地只读样例检查默认、503保留、403清空与两个键盘焦点。资源用返回的绝对值呈现，避免将显示封顶误读为容量比例；审核预览不会打开或提交签认，也不构成真实测量、恢复、权限、审计或生产验收，且没有部署、重启或配置变更。
