...
用户名扩展回滚时，先回退 Web/API 并确认不再读取用户名字段；如允许删除用户名别名，再执行 `0067_usernames_login.down.sql`。基础身份模块回滚仍须先在宝塔停止接收新的注册/登录写请求并停止 Worker，导出受影响的身份表与审计证据；只有在确认允许删除 M01-01 数据时才按 `0008f`、`0008e`、`0008d`、`0008c`、`0008b`、`0008a` 的 down 文件逆序回滚。若需保留账号数据，只回退应用构建，不执行基础 down。最后在宝塔启动上一版本并验证 `/api/v1/health/live`、`/api/v1/health/ready`，不得使用面板外服务顶替。
