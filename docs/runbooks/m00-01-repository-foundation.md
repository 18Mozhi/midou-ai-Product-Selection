...
- Worker 启动失败：统一后端必须在健康检查中失败关闭并保留日志，修复后由宝塔重启“ai选品”。Python 心跳或桥接失败则查看 `ai选品-python` 日志并由宝塔重启该项目；不得另建生产项目绕过故障。
- 迁移失败：停止新入口，保留失败记录；确认没有未发布 Outbox 数据后执行 `0001_m00_01_foundation.down.sql`。审计或证据不得为掩盖失败而删除。

## 回滚

1. 在宝塔网站关闭新增入口或回退静态包。
2. 在本地切到目标提交后重新上传固定目录运行包，并在宝塔重启“ai选品”和“ai选品-python”。
