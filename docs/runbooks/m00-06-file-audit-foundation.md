...
生产文件根目录仅配置于宝塔受限环境，并由宝塔备份任务同步到深圳备用机房。目录必须可写但不可作为静态网站根直接暴露。运行 `npm run build`、`node --test tests/m00-06/file-audit-foundation.test.mjs`、`node scripts/verify-file-audit-live.mjs` 和 `npm run verify:module -- M00-06`；真实脚本使用 OS 临时目录，完成后删除文件和 MySQL 探针记录。
