...
文件页三个容量条提供“证据/导出/临时目录所在文件系统已用比例”的可访问名称，百分比文本随刷新更新。水位来自statfs文件系统总量与可用量，不是该目录递归占用；同盘多根不可相加。索引体积是独立统计，临时目录不建立持久索引。目录失败时0容量与100%水位是失败占位，不能作为真实满盘测量。

UI2-RS69在隔离API响应下验证三条名称、初始与刷新后0/87.5/100.0%和两个GET；不执行生产文件读写、删除或恢复，不等于完整读屏、正式新风格或生产第三目录已验收。该改动只有Web语义属性，无新配置、阈值或SQL，无当前重启要求；后续仍由固定宝塔部署器整体发布并预检既有迁移/停启，Web回退不执行下面的数据库down脚本。

P69 C 方向实际 Vue 组合审核只能使用隔离样例；审核通过前不得据此改写受控目录、校验样本、恢复、权限或审计链路，更不得将本地截图或 mock 当成生产文件系统证据。
