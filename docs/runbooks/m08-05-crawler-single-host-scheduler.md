...
1. 在候选发布迁移中以 `product_scout` 业务账号执行 `0034_crawler_scheduler_m08_05.up.sql` 与 `0055_provider_runtime_circuits.up.sql`；确认 MySQL 5.7、三张调度表及来源运行熔断表。
...
- “来源并发与排队”逐来源展示活动/有效并发、当前已到期可领取任务数、最长等待、等待 P50/P95、24 小时成功率、耗时 P95、样本量和独立熔断。排队只统计服务端时间已经到达 `available_at` 的任务；未来退避或限流任务不计入当前等待。若等待持续上升，先核对同页进程、租约、资源与告警，再通过任务中心下钻，不得直接调大单机并发。
