...
第二阶段 UI 批67仅提供 P70 隔离实际 Vue C 审核组合：原读取、筛选/分页、权限、快照和两个确认调用保持不变，以本地只读样例检查默认、503保留、403清空与两个键盘焦点。审核预览不会打开或提交回收，也不执行来源健康检查；它不构成真实调度、恢复、权限、审计或生产验收，且没有部署、重启或配置变更。

第二阶段UI局部修复：即使网址带 `?state=recovering`，页面仍会读取真实调度事实；只有实际提交回收才可进入回收中，网址不会启动恢复。UI2-SC70为隔离响应验证，不执行生产POST。全量新设计及其余恢复状态仍待验，本批无新配置/迁移或当前重启要求；以后通过固定 `python scripts/deploy-baota.py` 整体发布并预检既有迁移与停启窗口，不另建Web上传链，也不为Web回退执行down迁移。

来源等待分位的当前实现来自最多5000条子查询混合样本中的等待任务，不是完整的历史等待时间分布；完成样本数则仅计有duration的记录。最新待领取量与历史分位标签应分开核对，不能以本页指标单独确认容量或调度公平性。详细边界见第二阶段P70规格。
