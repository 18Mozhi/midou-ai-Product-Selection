...
390px 验证时，来源健康、全量数据和规则治理记录应显示摘要卡片且没有横向宽表；点“查看详情”后可查看完整事实。移动筛选应从抽屉打开，关闭后重开仍保留已选条件，状态选项显示中文。来源 UUID 和代码、数据/治理记录 ID、版本代码只在“技术详情”出现；告警主列表不应直接暴露组织/工作区 UUID，请求标识也只在页尾“技术详情”中展示。
