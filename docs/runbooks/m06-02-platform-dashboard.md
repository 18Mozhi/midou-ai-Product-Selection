...
3. 以平台运营管理员访问 `/platform-admin`，确认折线图展示成功/失败任务趋势；再检查内容、通知、邮件、系统状态、账号与组织、人员与权限等导航。通知和邮件可创建草稿、编辑、发布或取消；邮件发布先进入投递队列，未配置邮件服务时明确保持待配置状态。所有修改必须填写原因，并用关联编号在 `platform_audit_events` 定位记录。
