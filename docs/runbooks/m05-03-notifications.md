...
数据库迁移后在宝塔面板重启唯一的“ai选品”统一后端。内部 Worker 配置为 `NOTIFICATION_OUTBOX_POLL_MS`、`NOTIFICATION_OUTBOX_LEASE_SECONDS`、`NOTIFICATION_OUTBOX_RETRY_LIMIT`；邮件模式只能是 `NOTIFICATION_EMAIL_DELIVERY_MODE=placeholder`，其他值会阻止统一后端启动。修改后必须重启“ai选品”。
