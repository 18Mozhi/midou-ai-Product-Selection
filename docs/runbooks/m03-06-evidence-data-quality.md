...
在 390px 分别打开证据和质量问题卡片，确认无需横向滚动即可读取来源、状态、阈值与时间，并验证下载、完整溯源和记录解决操作仍可进入；完整 UUID、哈希和关联编号只应出现在“技术详情”。
...
- 批量归因、指派或关闭前先核对影响预览。指派只能选择全部所选问题共同组织中的活动成员；跨组织选择会失败关闭。409 表示至少一个问题版本或状态已变化，应刷新后重新选择，不能缩小校验范围强行部分提交。
