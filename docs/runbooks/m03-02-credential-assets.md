...
6. 浏览器助手 ZIP 只由 `npm run build:browser-helper` 从 `browser-helper/scoutops-browser-helper` 确定性生成；`build:web` 会先重建该文件。宝塔部署预检同时核对公开目录与 `dist` 内 ZIP 的文件清单和逐文件内容，发现源文件与发布包不一致时必须失败关闭。
7. 1688 登录读取会在用户点击后同时申请 `s.1688.com` 与 `*.1688.com` 权限，以读取实际随搜索页发送的父域登录 Cookie；不得扩成所有网站的常驻主机权限。读取到 0 条时页面必须要求先完成来源登录，不能把空数组显示为读取成功。
8. 使用无平台权限账号和无会话请求分别确认 403 与 401；本接口为平台全局资产，不接受组织或工作区参数。
