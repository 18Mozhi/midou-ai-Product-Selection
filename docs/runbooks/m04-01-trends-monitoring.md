...
同时核对筛选、排序、页码、当前主题和监控规则分区会同步到地址栏；复制后的视图链接应能恢复相同条件，`topic` 深链即使不在当前页也应读取对应详情。制造空结果后，“清除筛选并恢复”应回到默认活跃趋势。桌面调整列表宽度或打开包含大量时间点的主题时不得造成整页横向溢出；长时间线只能在自身卡片内滚动，390px 下主从栏应转为单栏且主要操作可触达。
...
- 首屏就绪状态不正确：核对启用规则数、主题总数及启用规则最近一次任务的失败来源；“市场质量门 · 证据就绪”只汇总工作区证据链，不代表任一商品已经通过市场质量门。
