...
此局部修复仅改变前端展示，不增加SQL、scope、有效期规则、配置项或后端重启需求。正式发布仍须遵循根AGENTS与 `python scripts/deploy-baota.py`：核对线上版本、既有迁移白名单执行记录、宝塔Node停启窗口和恢复材料，不能因纯UI变更承诺部署器不执行迁移步骤或零停机。以下旧模块建表/回滚步骤不是本修复再次执行0019迁移的指令。

先在隔离Vue用例验证：创建明文可见→离开到组织数据→返回，明文不恢复；创建在途离开再返回，迟到响应不展示；点击“我已安全保存”后写后刷新结束仍不显示。轮换/撤销需核对原API目标、版本、原因、幂等键和清除反馈，测试只用合成明文，不能写入生产Token或截图真实秘密。真实令牌若因离开或清除而未保存，按既有流程由授权操作者轮换，不能自动重试创建或从日志找回。局部回退只恢复已知稳定前端运行包，不执行数据库down migration；上线及真实生产复验在正式发布批进行。
