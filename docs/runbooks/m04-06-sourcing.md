...
- 对比页面：已保存的每一份对比必须按相同顺序显示供应商、规格、MOQ、报价与交期，并在对比区上方显示规格归一化提示。出现多种规格文本时应要求采购成员核对型号、容量、包装和计量单位；系统只识别格式差异，不自动换算或认定等价。当前两个来源均为公开页，不存在登录档案续期入口；不要把验证码或网络失败伪装成登录受阻。
