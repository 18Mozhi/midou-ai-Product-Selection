...
- 对比勾选上限：已选五家后点击第六家，提示“最多比较五家”且第六框保持未勾选；已选集合、视觉勾选和POST quote_ids必须一致。取消已有项后仍可选择其他报价，不改变2–5家业务规则。回归入口为 `m04-06-sourcing.spec.ts` 的UI2-SC06。
- 队列异常：在宝塔 Worker 日志搜索 `sourcing_projection`，再检查租约、尝试次数和 `last_error_code`。配置修改后必须重启 Worker。

2026-09-07复选框同步修复只更新Vue，无API、Worker、SQL或参数变化；修复本身无需后端重启。正式发布仍由 `python scripts/deploy-baota.py` 统一执行，必须按当前白名单核对迁移记录及宝塔停启窗口；本地隔离Vue通过不代表真实采集或生产采购消费已验收。
