...
人工验证：登录、选择组织和工作区，访问 `/?view=authorization`。有 `role:read` 与组织范围者可查看角色矩阵；普通成员仍可读取本人有效授权摘要，但角色目录返回 403 并明确联系管理员；未选择上下文返回 409；登录过期返回 401。禁止通过浏览器修改响应或显示菜单来判断 API 是否授权。
