...
1. 使用具备 `opportunity:decide` 与 `opportunity:approve` 的选品经理进入 `/opportunities/scoring-rules`，确认新建、预览、提交、批准、拒绝、启用和回滚入口按规则状态显示。
2. 使用仅具备 `opportunity:read` 与 `opportunity:decide` 的普通成员进入同一页面，确认可以创建和提交草稿，但看不到预览、批准、拒绝、启用和回滚入口；直接调用审批接口仍应返回 `permission_denied`。
3. 新建、预览和规则动作必须显示为浏览器原生模态框：背景不可操作，`Escape` 可关闭，关闭后焦点返回触发按钮；校验、冲突、离线及重试信息留在当前模态框内。
4. 预览结论使用业务中文展示，但接口仍保留原始枚举；桌面和 390px 视口不得出现结论与覆盖率文本重叠或页面级横向滚动。
