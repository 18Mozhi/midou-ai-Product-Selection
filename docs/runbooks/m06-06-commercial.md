...
4. 使用具有 `platform:operate` 的平台运营管理员访问 `/platform-admin/commercial`。导航和页面标题应显示“配额管理”，不应显示会员、续期或计费套餐入口。先显式创建配额方案草稿、编辑额度并启用，再按组织 UUID、方案和统计周期首次分配；已有分配可修改开始/结束时间后执行“确认调整”，也可暂停、恢复或结束。所有变更和人工调整都必须填写原因。可用 `?organization_id=<uuid>` 打开组织配额与用量深链接。
...
先运行 `node --test tests/m06-06/commercial.test.mjs`，再以 MySQL 5.7 环境运行 `node scripts/verify-commercial-live.mjs`，随后运行 Playwright 桌面与 390px 检查和 `npm run verify:module -- M06-06`。验收时确认平台导航、页面标题、确认文案均使用配额语义，页面明确说明当前不包含计费、价格或支付；现有 `/api/v1/platform/commercial/*` 路由和 `plans`、`assignments` 字段保持不变。生产冒烟只创建可删除的测试组织数据，结束后清理；不要使用真实客户数据做回归。
