...
- 1688 已有 `1688-browser-contract-v1` 搜索、商品详情和供应商输出合同及固定样本回放页面。先在专用验收页选择当前账号的活动组织与工作区、填写关键词并点击“发起登录验收运行”；该操作只提交一条带 `acceptance_run=true` 的受控人工任务，不启用来源，也不加入自动调度。搜索执行器通过页面自己的 GBK 表单提交中文关键词，识别当前 `offerId` 商品卡，并输出最多 15 条 `1688.search.v1` 快照、每条最多 15 KB DOM；标题、供应商和可见价格来自当前卡片，卡片未展示的 MOQ 与地区明确记为缺失。页面用最新真实浏览器作业展示搜索、详情、翻页覆盖矩阵；当前详情上限为 0、翻页上限为 1，因此两项必须显示“未演练”，不能把首屏搜索成功解释成详情或分页覆盖。只有带结构化快照、截图和 DOM 的成功真实作业才会成为候选；固定后执行差异回放和第二人审批。配置接口会以 `provider_source_setup_required` 阻止提前启用，不能只因合同测试或矩阵某一项通过就绕过门禁。
...
- 1688 搜索返回空：先检查页面标题和关键词是否乱码；执行请求必须从固定搜索页填写 `#alisearch-input` 并点击 `.input-button`，不得恢复为 UTF-8 `keywords` 查询串。再检查商品卡是否仍为带数字 `offerId` 的 `a.search-offer-wrapper`，以及标题、供应商和可见价格节点是否仍为当前合同选择器；结构变化按 `source_changed` 处理，不得扩大到旺旺、广告跳转或店铺链接。
