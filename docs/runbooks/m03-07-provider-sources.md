...
- 1688 已有 `1688-browser-contract-v1` 搜索、商品详情和供应商输出合同及固定样本回放页面。先在专用验收页选择当前账号的活动组织与工作区、填写关键词并点击“发起登录验收运行”；该操作只提交一条带 `acceptance_run=true` 的受控人工任务，不启用来源，也不加入自动调度。页面用最新真实浏览器作业展示搜索、详情、翻页覆盖矩阵；当前执行计划 `max_pages=1` 时翻页必须显示“未演练”，不能把首屏成功解释成分页覆盖。只有带结构化快照、截图和 DOM 的成功真实作业才会成为候选；固定后执行差异回放。若 Playwright 尚未产出结构化快照，则没有候选，来源仍显示“待配置”且保持禁用。配置接口会以 `provider_source_setup_required` 阻止手工启用，不能只因合同测试或矩阵某一项通过就绕过门禁。
