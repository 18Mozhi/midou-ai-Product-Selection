...
- 普通用户：进入“热点趋势”，系统会自动更新；需要立即更新时点击“立即刷新来源”。
...
- 1688 已有 `1688-browser-contract-v3` 搜索、商品详情和供应商输出合同及固定样本回放页面。先在专用验收页选择当前账号的活动组织与工作区、填写关键词并点击“发起登录验收运行”；该操作只提交一条带 `acceptance_run=true` 的受控人工任务，不启用来源，也不加入自动调度。搜索执行器通过页面自己的 GBK 表单提交中文关键词，只识别声明 `data-tracker="offer"` 且链接携带数字 `offerId` 的当前商品卡；当前计划最多两页、每页最多 15 条，按 `offerId` 去重合并，原始 HTTP 移动详情链接只用于提取商品 ID，持久化时统一生成 HTTPS 规范链接。任务再从规范 HTTPS 地址最多打开一个详情页，提取标题、供应商、店铺 ID、规格、价格、MOQ 与发货地；目的地 ETA 不作为供应商交期。页面用最新真实浏览器作业展示搜索、详情、翻页覆盖矩阵，两页必须实际完成且详情合同必须解析通过才能显示覆盖。只有带结构化快照、截图和 DOM 的成功真实作业才会成为候选；固定后执行差异回放和第二人审批。配置接口会以 `provider_source_setup_required` 阻止提前启用，不能只因合同测试或矩阵某一项通过就绕过门禁。
...
- `network_error` 且浏览器作业已成功：优先检查 MySQL 连接、死锁、锁等待或证据目录可用性。Worker 会对明确可重试的证据持久化异常执行最多三次短退避重试；耗尽后保留 `network_error`，不会再把依赖异常显示为“全部字段缺失”。
...
来源目录、1688 解析合同、配置版本回滚和凭证续期重放逻辑都在 Node 进程启动时加载。本次验收运行同时修改 Node 任务边界和 Python Crawler 使用的领取 SQL，但没有修改 Python 包；发布后通过宝塔重启统一 Node 后端“ai选品”，现有 `ai选品-python` 会在下一次领取请求自动使用新 SQL，无需重启。MySQL 与 Redis 不需要重启，也没有数据库迁移或新服务。`1688_search` 在真实登录固定样本字段提取回放和第二人审批均通过前继续保持停用。

## 网页登录与真实页面烟测

平台超级管理员从来源详情的“检查启用条件”二级入口进入“1688 启用检查”，依次核对登录态、验证码和字段解析；该入口不进入主导航，`platform:operate` 与 `provider:configure` 账号均不可访问。保存有效登录档案后，在“真实登录验收”选择当前账号具有活动成员关系的组织和活动工作区，填写 1–200 字符关键词并人工提交。后台只对 `1688_search + acceptance_run=true + purpose=acceptance` 放行停用来源作业，普通停用来源和自动任务仍拒绝；无需改库或先启用 1688。登录态需要当前有效档案完成一次成功运行；同一次成功运行作为未被验证码阻断的运行证据；字段解析需要当前解析器版本的固定样本回放为通过，并由另一名来源管理员审批样本基线。再核对搜索、商品详情与翻页矩阵：搜索和详情的“已覆盖”必须来自最新作业快照通过对应版本合同；翻页只有计划允许多页且实际 `page_count > 1` 才显示“已覆盖”，单页计划显示“未演练”。页面会显示负责人和准确待配置原因。点击“刷新检查结果”只重新读取数据库事实；按钮在读取期间禁用，12 秒超时、离线或接口失败时保留上一次成功读取的启用条件并显示可重试提示，恢复后再次刷新应显示最新读取时间；从 KeepAlive 缓存页返回时会提示正在重新读取启用检查和执行范围，提示在两路读取完成或本轮被再次离页时收起。“配置或续期登录档案”会带入 1688 并直接打开登录档案表单，“定位 1688 固定样本”会带 `provider_id` 将来源目录筛到唯一 1688 来源。矩阵不属于启用门，登录、验证码、回放与审批门禁未全部通过时，配置 API 仍以 `provider_source_setup_required` 拒绝启用，禁止直接改库绕过。
