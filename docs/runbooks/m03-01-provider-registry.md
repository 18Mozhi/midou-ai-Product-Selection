...
见 `design-plans/ui-phase-2-2026-09-07/P46-HISTORICAL-EVIDENCE-REPAIR.md`。
原结构/反馈比较绑定原提交四包344图；后续两包172图另行固定，不与旧基线混比较。
历史读取器不接入Vue/服务端；只读测试修复无需重启、迁移或环境配置，不构成发布验收。
