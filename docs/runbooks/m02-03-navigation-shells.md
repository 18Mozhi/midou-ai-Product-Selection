...
9. 直接打开未知地址，确认显示 404 且侧栏不高亮“今日行动”或“平台概览”；点击同源普通链接时确认页面无整页刷新，浏览器前进/后退仍可恢复路径。
10. 打开 `/me`，确认使用独立账号侧栏；平台管理员在没有组织上下文时仍能读取账号资料，组织相关的权限或资产分区失败只能显示局部提示。组织后台与平台后台默认使用浅色高密度主题。
