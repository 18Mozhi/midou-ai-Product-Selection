...
12. 抽查成员、组织和平台壳层：页面必须显示路由拥有的一级标题与说明；上下文轨只能展示服务端返回的当前组织/工作区、路由任务域和中文角色。没有页面级来源、新鲜度或可信度事实时不得显示占位结论。桌面角色切换入口位于侧栏底部，顶栏只保留主题、搜索或主操作、通知与账号；390px 上下文轨改为单列且不得横向溢出。
