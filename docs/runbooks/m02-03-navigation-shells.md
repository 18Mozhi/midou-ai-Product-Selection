...
3. 抽查 `/`、登录成功、MFA 成功、`/home`、`/org-admin`、`/platform-admin`：根路径和登录后必须按真实角色进入对应面板；有活动成员资格但未选上下文时进入 `/select-context`，完全无活动成员资格时进入 `/me`；普通成员不能进入组织或平台后台，组织管理员不能进入平台后台；平台管理员无组织上下文仍可进入平台后台。
...
11. 打开 `/me`，确认使用独立账号侧栏；平台管理员和无组织账号在没有组织上下文时仍能读取、更新账号资料，并能进入 `/security/mfa`。组织相关的权限、通知或资产分区失败只能显示局部提示。无组织账号从 `/select-context` 空状态也必须能进入个人中心和 MFA，但所有成员、选品、采集和报表 API 继续返回上下文错误。成员、组织、平台和个人中心必须继承同一主题；后台只使用紧凑密度，不能强制切回浅色。

本次账号级落地调整同时改变 Vue 静态资源和 Node API 的落地/个人资料授权逻辑，不新增数据库迁移或环境变量。发布 `frontend` 与 `backend` 后必须在宝塔重启 Node 项目“ai选品”；Python、MySQL 和 Redis 无需重启。若宝塔网站启用了静态缓存，按现有网站配置刷新静态缓存。
