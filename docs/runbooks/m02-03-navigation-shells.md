...
12. 抽查成员、组织和平台壳层：页面必须显示账页序号、路由一级标题与说明；上下文轨只能展示服务端返回的当前组织/工作区、路由任务域、固定的连接状态文案和中文角色。桌面壳层切换入口位于模块索引末端，顶栏只保留主题、搜索或主操作、通知与账号；840px 及以下改为抽屉和五列底栏，上下文轨改为单列且不得横向溢出。
