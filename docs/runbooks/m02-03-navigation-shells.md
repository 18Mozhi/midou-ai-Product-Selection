...
11. 打开 `/me`，确认使用独立账号侧栏；平台管理员和无组织账号在没有组织上下文时仍能读取、更新账号资料，并能进入 `/security/mfa`。组织相关的权限、通知或资产分区失败只能显示局部提示。无组织账号从 `/select-context` 可创建个人选品空间；创建前所有成员、选品、采集和报表 API 仍返回上下文错误，创建成功并选中当前会话后才开放组织内能力。成员、组织、平台和个人中心必须继承同一主题；后台只使用紧凑密度，不能强制切回浅色。
