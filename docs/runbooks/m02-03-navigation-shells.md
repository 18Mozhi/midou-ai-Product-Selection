...
8. 在同一个单页会话内抽查任意成员列表筛选地址，进入平台后台后点击“选择范围并返回工作台”：必须先显示组织/工作区选择，选择成功后返回原完整地址；刷新浏览器后允许回退到 `/home`。手工传入 `return_to=//example.com` 时必须回退到快速引导，不能跳往站外；浏览器存储中不得新增返回路径或最近组织记录。
...
10. 在来源管理页确认凭证入口只出现在来源二级导航；在采集和系统运维页面确认二级导航保持当前项，并且平台告警不会跳进成员 `/automations` 页面。
11. 打开 `/me`，确认使用独立账号侧栏；平台管理员在没有组织上下文时仍能读取账号资料，组织相关的权限或资产分区失败只能显示局部提示。组织后台与平台后台默认使用浅色高密度主题。
