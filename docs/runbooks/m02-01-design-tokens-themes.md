...
即使本模块仅变更 Vue/CSS，也使用固定的 `python scripts/deploy-baota.py` 发布流程，不做静态文件热拷贝。该仓库部署器会按既定流程停止/启动宝塔 Node 项目、执行固定迁移 allowlist 检查、测试并重载 Nginx，并重新申明 Python 项目配置；不要另行手工重启或绕过宝塔。只有确认部署器实际未重启/重载对应对象且线上探针不健康时，才按项目约束检查宝塔对象。Worker/Python 业务代码没有变化，但 Python 项目配置会由部署器重申。
