...
4. 用平台安全管理员访问 `/platform-admin/open-platform`。创建、轮换或撤销 API Client 前先核对“令牌权限风险预览”的组织、授权能力和访问后果；当前只能显示 `status:read`，若出现未开放 scope 应停止操作并检查发布版本。创建/轮换密钥后立即保存一次性明文；日志和工单只记录 Client 前缀或 Webhook 指纹。
...
390px 页面验收时，Client、Webhook 和投递记录应显示摘要卡片且没有横向宽表；详情抽屉保留测试、轮换、撤销和重放动作，令牌权限风险预览应改为单列且完整显示权限和后果。Client 前缀、Webhook 指纹、原始错误代码、各类 ID 与 request_id 默认不出现在主界面，展开“技术详情”后可用于日志关联。
