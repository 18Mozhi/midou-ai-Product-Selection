...
8. 页面刷新期间按钮应显示忙态并禁用；同一页面只能有一个在途请求。API 事实读取最多 14 秒，浏览器保留 15 秒兜底。已有成功快照时，429、503 或前端超时必须保留旧事实并显示本次失败；401/403 必须清空快照并要求重新登录或申请权限。超时事务必须回滚，锁释放后不得补写成功记录。
9. 停止隔离或测试 MySQL 后，operations GET 应返回 503 `mysql_resilience_dependency_unavailable`，且响应不得包含 SQL、驱动错误、主机、端口、账号或凭证。MySQL 恢复后通过“重新核验”重新读取，不复用故障期间的迟到响应。
10. UI C 方向的页面组合审核只允许读取隔离样例；审核通过前不得据此改写 MySQL 核验、恢复、权限或审计链路，更不得把截图或本地 mock 当成生产 MySQL 证据。
...
- 不得为了通过门禁放宽阈值或缩短规定观察。
- 只有首次无快照读取才显示整页加载/错误；已有快照刷新失败不得把最后一次已验证状态渲染为空白或伪装成新的健康结论。
