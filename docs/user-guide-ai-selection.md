...
平台管理员可从顶部横向模块索引进入来源注册、凭证档案、采集控制台、全量数据、安全审计、监控运维和商业运营。点击任务域展开具体页面；顶部朱砂色主动作只展示当前账号有权执行的操作。
...
在首页直接填写市场、商品关键词、排除词、采集周期和“形成规则候选所需独立来源数”，保存后系统立即开始持续采集公开页面。真实商品证据命中规则但尚未达到来源门槛时显示“采集中”；达到用户设置的来源门槛后显示“规则命中候选”。只有评分、市场、竞争、成本和风险五项质量门全部通过后，商品才进入“待我采纳”并显示“建议采纳”。你只需打开这些建议核对依据并执行最终采纳；系统不会自动替你采纳，也不会为候选编造分数、利润或风险。

自动质量评估不会把“抓到页面”直接当成可采纳商品。当前自动成本只对费用规则明确选择的 Amazon 手机壳生效；其他品类仍需人工复核成本。Amazon 商品必须有真实售价和竞品快照；1688 采购端至少需要三条同商品族、同型号且版本不冲突的高置信报价，系统按偏保守的 75 分位价计算。费用规则要明确平台费、支付手续费、税费、履约成本、入仓物流，以及跨币种时的汇率和官方来源。匹配不足或费用依据缺失时，商品继续停在“规则命中候选”，不会强行推荐；已有人工当前成本时，自动证据不会覆盖它。
...
脚本直接解析 `apps/web/src/route-catalog.ts`，按每个账号从 `/api/v1/me/navigation` 取得的真实能力计算全部授权路由，不再只遍历可见模块索引。每个静态路由都必须打开；机会详情和任务详情必须从验收组织的真实列表取得一条已持久化记录后打开。它同时阻断角色叠加、越权能力、Route Catalog 漏测、API 4xx/5xx、控制台错误、阶段占位页、会话丢失以及失效的快捷创建入口。生产报告只保存角色、能力、路由与错误事实，不保存邮箱、密码或 Cookie。
