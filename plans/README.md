...
当前执行状态：按用户最新上线标准重新验收中。完成门改为：宝塔只展示一个名为 `ai选品` 的统一 Node 后端，服务器项目根目录为 `/www/wwwroot/ai选品`，根路由展示真实业务工作台，后端可由宝塔稳定启动/停止/重启，P00–P08 功能与真实浏览器流程全部通过。旧运行 ID、容量窗口和硬件观测不再作为当前完成依据；磁盘、容量、负载均衡、多节点、PVE、备用服务器及其他项目均不在本目标范围。
...
6. 软件完成门不依赖磁盘、容量压测或生产资源证据；这些结果只能作为独立运营观测，不能覆盖真实功能、启动稳定性和浏览器验收。
