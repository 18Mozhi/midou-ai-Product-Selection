...
const output = args.length ? path.resolve(`output/playwright/p65-read-focus-${args[1]}`) : null;
if (output) await mkdir(output);
const { fixture, nav: navigationFixture } = await releaseReviewFixtures(),
  probe = reservePort();
const envelope = (data) => ({
  data,
...
const sources = new Set([
    ...
    "scripts/verify-release-read-focus.mjs",
...
try {
  ...
  console.log(`P65 read focus ${origin}`);
    ...
    [1440, 390].flatMap((width) =>
      ["reduce", "no-preference"].map(async (motion) => {
        ...
        try {
          ...
          await state.getByRole("heading", { name: "发布事实暂不可用", exact: true }).waitFor();
          ...
          await release();
          await surface.locator(".verdict").waitFor();
          ...
          const nav = surface.getByRole("link", { name: "历史观察", exact: true });
          mode = "hold";
          await activate(refresh);
          await header.getByRole("button", { name: "正在刷新…", exact: true }).waitFor();
          await nav.focus();
          await release();
          await header.getByRole("button", { name: "刷新发布事实", exact: true }).waitFor();
  ...
  if (output) {
      ...
      JSON.stringify(
        {
          ...
          scope: "actual Vue local focus checks; no real release/rollback or permission acceptance",
    ...
    await writeFile(
      ...
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>P65读取焦点审核</title><style>body{font:16px/1.7 Micr' +
        "osoft YaHei;margin:24px}img{max-width:100%;display:block;border:1px solid #c7d3e4}article{margin:28px 0}</styl" +
        "e><h1>P65读取焦点局部待审</h1><p>本地样例；不代表真实权限、恢复或全页验收。</p>" +
        images
          .map(
            (i) =>
