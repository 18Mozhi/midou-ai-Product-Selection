...
const reviewRoot = path.resolve(
  "design-plans/ui-phase-2-2026-09-07/design/platform-notifications-direction-c/dialog-review-r2",
);
        ...
        try {
          ...
          if (scenario.startsWith("editor") || scenario.startsWith("action")) {
            ...
            if (editing || cancelling) {
              ...
              if (cancelling) {
                const sizing = await dialog.getByRole("alert").evaluate((element) => {
                ...
                assert.ok(
                  sizing.actual <= sizing.intrinsic,
                  "Error panel must fit its text instead of stretching into the spacer row",
                );
              }
              await trap(page, dialog);
