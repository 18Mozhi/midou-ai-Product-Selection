...
async function validate() {
  try {
    await verifyRuntimeDocumentation({ root });
  } catch (error) {
    fail(
      `runtime documentation consistency: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  if (matrix.schemaVersion !== 1 || matrix.moduleId !== "M07-01")
    fail("matrix schema or module id is invalid");
  const dimensions = [
    "roleScope",
    "sourceCollection",
    "taskQueue",
  ...
  for (const name of dimensions)
    if (!Array.isArray(matrix.dimensions[name]) || matrix.dimensions[name].length === 0)
      fail(`dimension ${name} is empty`);
  const targets = {
  ...
  for (const [name, value] of Object.entries(targets))
    if (matrix.performanceTargets[name] !== value) fail(`performance target ${name} drifted`);
  ...
  const actual = (await filesUnder(resolve(root, "tests/e2e"), ".spec.ts"))
    .map((path) => path.split("/").at(-1))
    .filter((name) => /^m0[0-6]-/.test(name))
    .sort();
  if (JSON.stringify(expected) !== JSON.stringify(actual))
    fail(`browser matrix drift: expected ${expected.length}, repository ${actual.length}`);
}
...
function run(program, args, id) {
  ...
  const commandArgs =
    program === "npx"
      ? [resolve(root, "node_modules/@playwright/test/cli.js"), ...args.slice(1)]
      : args;
...
console.log(
  JSON.stringify({ module_id: "M07-01", mode, status: "passed", run_id: runId, trace_id: traceId }),
);
