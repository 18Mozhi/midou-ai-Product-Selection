...
import {
  personalCenterPagePlugin,
  personalCenterReviewCss,
} from "./lib/personal-center-page-preview.mjs";
...
const widths = process.env.P11_VIEWPORT
  ? [Number(process.env.P11_VIEWPORT)]
  : smoke
    ? [390]
    : [1440, 390];
const motions = process.env.P11_MOTION
  ? [process.env.P11_MOTION]
  : smoke
    ? ["reduce"]
    : ["reduce", "no-preference"];
const output = args.length
  ? path.resolve(`output/playwright/p11-page-composition-${args[1]}`)
  : null;
...
const server = await createServer({
  ...
  plugins: [personalCenterPagePlugin()],
...
const authorization = {
  ...
  capabilities: ["task:read", "trend:read", "organization_token:manage"],
  data_scopes: [{ scope: "workspace", scope_key: "p11-workspace" }],
};
const sessions = [
  {
    id: "p11-session",
    device_label: "Windows · Edge",
    status: "active",
...
const assets = {
  followed_trends: [
    { id: "p11-trend", title: "户外收纳", market: "US", created_at: "2026-09-19T08:00:00.000Z" },
  ],
  decisions: [
    {
      id: "p11-decision",
      opportunity_id: "p11-opportunity",
      opportunity_name: "可折叠储物箱",
      action: "observe",
...
const envelope = (data, requestId = "p11-request") => ({
  data,
...
try {
      ...
      try {
        ...
        await context.route("**/*", async (route) => {
          ...
          if (key === "GET /api/v1/auth/session-status")
            return route.fulfill({ json: envelope({ authenticated: true }, "p11-session-status") });
          if (key === "GET /api/v1/me/profile")
            return scenario === "profile-failure"
              ? route.fulfill(failure(503, "profile_unavailable", "p11-profile-failure"))
              : route.fulfill({ json: envelope(profile, "p11-profile-read") });
          ...
          if (key === "PATCH /api/v1/me/profile") {
            ...
            return route.fulfill({
              json: envelope({ ...profile, ...body, version: 5 }, "p11-profile-save"),
            });
          }
          if (key === "PUT /api/v1/me/notification-preferences") {
            const body = request.postDataJSON();
            writes.push({ key, body });
            return route.fulfill({
              json: envelope({ ...body, version: 7 }, "p11-preferences-save"),
            });
          }
          if (key === "DELETE /api/v1/me/sessions/p11-session") {
            writes.push({ key });
        ...
        const root = page.locator(".p11-account-review");
        await root
          .getByRole("heading", { name: "可保存的账号资料", exact: true })
          .waitFor({ timeout: 8000 });
        check(
          await root.getByText("邮箱只读，不会写入资料更新请求。", { exact: true }).count(),
          1,
          "profile write boundary visible",
        );
        const save = root.getByRole("button", { name: "保存资料", exact: true });
        await save.focus();
        check(
          await save.evaluate((node) => getComputedStyle(node).outlineWidth),
          "3px",
          "profile save focus visible",
        );
        if (output && motion === "reduce") await capture(page, "profile");
        await root.getByLabel("显示名称").fill("陈晓（本地审核）");
        await save.click();
        await root.getByText(/个人资料已保存/).waitFor();
        check(
          writes[0],
          {
            key: "PATCH /api/v1/me/profile",
            body: {
              username: "review.member",
              display_name: "陈晓（本地审核）",
              avatar_url: "https://assets.example.test/avatar.png",
              phone: "13800000000",
              locale: "zh-CN",
              timezone: "Asia/Shanghai",
          ...
          "profile PATCH is exact and excludes email",
        );
        ...
        for (const [section, heading] of [
          ...
          if (section === "security") {
            ...
            check(
              writes[1],
              { key: "DELETE /api/v1/me/sessions/p11-session" },
        ...
        check(
          writes[2],
          {
            key: "PUT /api/v1/me/notification-preferences",
            body: {
              expected_version: 6,
              in_app_enabled: true,
              email_enabled: false,
          ...
          "preferences PUT has only five server fields and version",
        );
        scenario = "profile-failure";
        const failed = await context.newPage();
        await failed.goto(`${origin}/me`, { waitUntil: "domcontentloaded" });
        const failedRoot = failed.locator(".p11-account-review");
        await failedRoot.getByText("当前无法读取个人资料", { exact: true }).waitFor();
        check(
  ...
  const sources = [
    ...
    "scripts/lib/personal-center-page-preview.mjs",
    "scripts/verify-personal-center-page-preview.mjs",
    personalCenterReviewCss,
  ];
      ...
      JSON.stringify(
        {
          ...
          scope: "local actual Vue C review; account responses are locally intercepted",
