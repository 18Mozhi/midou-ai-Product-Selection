...
} from "./lib/ui-phase2-rate-limit-retention.mjs";
...
const oldText = (file) =>
  execFileSync("git", ["show", `${rateLimitBaseline}:${file}`], {
    encoding: "utf8",
    maxBuffer: 4000000,
  });
...
const before = {},
  after = {};
for (const f of [rateLimitParent, rateLimitCard]) {
  before[f] = oldText(f);
  after[f] = await read(f);
}
assertRateLimitPresentationDelta(before, after);
const actualProof = "output/playwright/p34-parent-read-states/evidence.json";
const known = [rateLimitParent, rateLimitCard, actualProof];
const deltas = Object.fromEntries(
  await Promise.all(
    known.map(async (f) => [f, { before: hash(lf(oldText(f))), after: hash(lf(await read(f))) }]),
  ),
);
const mounted = JSON.parse(await read("output/playwright/p34-rate-limit-vue/evidence.json"));
assert.equal(mounted.baselineCommit, rateLimitBaseline);
assert.equal(mounted.checks.length, 176);
assert.equal(mounted.checks.filter((c) => c.name.endsWith("pixels unchanged")).length, 52);
for (const file of [actualProof, "output/playwright/p34-rate-limit-vue/evidence.json"]) {
  const proof = JSON.parse(await read(file));
  for (const [source, sha] of Object.entries(proof.sourceHashes))
    assert.equal(hash(lf(await read(source))), sha, source);
...
for (const folder of folders) {
  ...
  if (!e.sourceHashes?.[rateLimitParent]) continue;
