...
if (pythonProjects.length !== 1 || pythonProjects[0]?.name !== 'ai选品-python' || pythonProjects[0]?.workingDirectory !== '/www/wwwroot/ai选品/python' || pythonProjects[0]?.pythonVersion !== '3.12.13' || pythonProjects[0]?.startCommand !== 'python -m scoutops_crawler') fail('python_project_invalid', 'one fixed-directory BaoTa Python 3.12 crawler project is required');
...
if (![evidence.health?.live, evidence.health?.ready, evidence.health?.version, evidence.runtime?.backend, evidence.runtime?.worker, evidence.logging?.panelVisible, evidence.logging?.rotationConfigured, evidence.logging?.secretScanPassed].every(Boolean)) fail('production_health_incomplete', 'backend runtime, worker heartbeat or logging evidence is incomplete');
