...
if (manifest.schemaVersion !== 4 || manifest.stage !== 'production') fail('manifest_contract_invalid', 'production schemaVersion 4 is required');
...
if (manifest.target?.deployRoot !== '/www/wwwroot/ai选品') fail('deploy_root_invalid', 'production deploy root must be /www/wwwroot/ai选品');
...
if (nodeProjects.length !== 1 || nodeProjects[0]?.name !== 'ai选品' || nodeProjects[0]?.processMode !== 'foreground' || nodeProjects[0]?.workingDirectory !== '/www/wwwroot/ai选品/backend' || !nodeProjects[0]?.startCommand.startsWith('node --env-file=/www/wwwroot/ai选品/config/product_scout.env ')) fail('single_backend_invalid', 'exactly one fixed-directory foreground ai选品 backend is required');
const pythonProjects = manifest.objects.filter((item) => item.kind === 'baota-python-project');
if (pythonProjects.length !== 1 || pythonProjects[0]?.name !== 'ai选品-python' || pythonProjects[0]?.workingDirectory !== '/www/wwwroot/ai选品/python' || pythonProjects[0]?.pythonVersion !== '3.12.13' || pythonProjects[0]?.startCommand !== 'python -m scoutops_crawler --env-file=/www/wwwroot/ai选品/config/product_scout.env') fail('python_project_invalid', 'one fixed-directory BaoTa Python 3.12 crawler project with restricted env loading is required');
...
if (!deployer.includes('shutil.chown(config, user="root", group="www")') || !deployer.includes('os.chmod(config, 0o750)') || !deployer.includes('shutil.chown(env_file, user="root", group="www")') || !deployer.includes('shutil.chown(release_file, user="root", group="www")')) fail('restricted_config_permission_contract_invalid', 'BaoTa restricted config must stay root:www so www-managed Node and Python projects can read 0640 environment files');
...
if (evidence.schemaVersion !== 2 || evidence.target?.host !== manifest.target.host || evidence.target?.domain !== manifest.target.domain || evidence.target?.deployRoot !== manifest.target.deployRoot) fail('production_evidence_target_invalid', 'production evidence target mismatch');
...
if (![evidence.health?.live, evidence.health?.ready, evidence.health?.version, evidence.runtime?.backend, evidence.runtime?.worker, evidence.runtime?.python, evidence.logging?.panelVisible, evidence.logging?.rotationConfigured, evidence.logging?.secretScanPassed].every(Boolean)) fail('production_health_incomplete', 'backend runtime, worker heartbeat, Python heartbeat or logging evidence is incomplete');
