...
const output = "output/playwright/p43-create-user-preview";
...
const fixtureCode = ["user", "org", "overview"]
  .map((name) => {
  ...
  .join("\n");
const data = JSON.parse(JSON.stringify(vm.runInNewContext(fixtureCode + "({overview})")));
...
const transformed = {
  [component]: userPagePreview(originals[component], "parent"),
  [child]: userCreatePreview(originals[child]),
};
...
const sources = new Set([
  ...
  "scripts/verify-ui-phase2-user-create-preview.mjs",
...
const entry = "/__p43_create_preview.js";
...
document.documentElement.dataset.design='signal-ledger';document.body.classList.add('p43-user-form-review','p43-page-preview');
const router=createRouter({history:createWebHistory(),routes:[{path:'/:pathMatch(.*)*',component:{render:()=>null}}]});
const app=createApp({render:()=>h('main',[h('p','P43 创建用户 · 实际 Vue 逻辑 + C 创建分区 · 测试数据 · 待审'),h(Current,{apiBaseUrl:'/api/v1',initialTab:'users',routePath:'/platform-admin/users'})])}).use(router);await router.isReady();app.mount('#app');`;
  ...
  plugins: [
    {
      ...
      configureServer(instance) {
        instance.middlewares.use((req, res, next) => {
          ...
          res.end(
            `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P43 创建弹窗</title><div id="app"></div><script type="module" src="${entry}"></script></html>`,
          );
        });
      },
    },
...
try {
  ...
  console.log(`p43_create_host ${origin}`);
  ...
  for (const width of [390, 760, 761, 1440]) {
    ...
    try {
      ...
      await page.route("**/*", async (route) => {
        ...
        if (
          ...
          return route.fulfill({
            json: {
              data: overview,
              request_id: "p43-create-fixture",
              trace_id: "p43-create-fixture",
            },
          });
        }
        if (
          ...
          return mode === "success"
            ? route.fulfill({
                json: {
                  ...
                  request_id: "p43-create-success",
                  trace_id: "p43-create-success",
                },
              })
            : route.fulfill({
                status: 400,
                json: {
                  ...
                  request_id: "p43-create-error",
                  trace_id: "p43-create-error",
                },
              });
        }
      ...
      const visit = async () => {
        await page.goto(`${origin}/platform-admin/users?keep=creation`);
      ...
      await snap("default", "普通用户 · 默认四字段");
      await snap("default-bottom", "普通用户 · 底部完整操作", true);
      await email.focus();
      ...
      await open();
      check(
        "reopen clears identity and role fields",
        await dialog.locator("input,select").evaluateAll((ns) => ns.map((n) => n.value)),
        ["", "", "", ""],
      );
      await snap("reopened", "重新打开 · 默认空表单");
      ...
      check(
        "default payload preserves null and no reason",
        requests.filter((r) => r.method === "POST").at(-1).body,
        {
          email: "preview@example.test",
          temporary_password: "[known synthetic value matched]",
      ...
      requestsByWidth.push({
        ...
        requests,
        limitations:
          "No backend/RBAC/database or real account creation. Current template/CSS creation review; full app shell/history and late write ownership excluded.",
      });
    } finally {
      await context.close();
    }
...
if (capture) {
    ...
    JSON.stringify(
      {
        ...
        approval: "pending",
        scope:
          "Actual current Vue parent and child scripts with review-only template composition and CSS. All GET/POST intercepted with test samples. Not production C, full shell, backend validation, native popup capture, real MFA/RBAC or account creation.",
  ...
  await writeFile(
    `${output}/index.html`,
    `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P43 创建用户弹窗</title><style>body{max-width:1000px;margin:24px auto;padding:20px;font-family:'Microsoft YaHei',sans-serif;background:#edf1f6;color:#202c3d}img{max-width:100%}section{margin:32px 0}p{line-height:1.6}</style><h1>P43 新建用户 · 待审核</h1><p>原Vue逻辑与字段，仅审核模板分区和CSS；请求均拦截为测试样例，未创建真实账号。不含原生校验气泡、真实权限、整页或生产验收。</p>${screenshots.map((s) => `<section><h2>${s.viewport.width}×${s.viewport.height} · ${s.label}</h2><img src="${s.file}" alt="${s.label}"></section>`).join("\n")}`,
  );
}
console.log(
  JSON.stringify({
