...
} from "./lib/ui-phase2-user-security-review-historical-capture.mjs";
import {
  userSecurityReviewReplayDriver,
  userSecurityReviewReplayRoot,
} from "./lib/ui-phase2-user-security-review-current-replay.mjs";
...
const resume = args.includes("--resume"),
  ...
  stages = args.includes("--smoke") ? ["password"] : Object.keys(userSecurityReviewCaptureStages);
const read = async (file) => (await readFile(file, "utf8")).replaceAll("\r\n", "\n");
const hash = (value) => createHash("sha256").update(value).digest("hex");
if (capture && (await exists(`${userSecurityReviewReplayRoot}/evidence.json`)))
  assert.fail("Completed P43 packet is immutable; do not capture or resume");
if (capture && !resume) await mkdir(userSecurityReviewReplayRoot); // Exclusive permanent packet. Do not overwrite earlier images.
if (resume) assert.ok((await stat(userSecurityReviewReplayRoot)).isDirectory());
...
const originalArgv = process.argv,
  ...
  sources = new Set([
    ...
    "scripts/lib/ui-phase2-user-security-review-historical-capture.mjs",
    "scripts/lib/ui-phase2-user-security-review-current-replay.mjs",
    "scripts/lib/ui-phase2-provider-historical-capture.mjs",
    "scripts/lib/ui-imported-style-sources.mjs",
    "scripts/verify-ui-phase2-user-security-review-current-replay.mjs",
  ]);
try {
  for (const stage of stages) {
    const entry = userSecurityReviewCaptureStages[stage],
      historical = userSecurityReviewHistoricalCapture(stage);
    const original = JSON.parse(historical.manifest);
    process.argv = [originalArgv[0], entry.driver, ...(capture ? ["--capture"] : [])];
    const code = userSecurityReviewReplayDriver(stage, await read(entry.driver));
    const recorded =
      resume && (await exists(`${userSecurityReviewReplayRoot}/${stage}/evidence.json`));
    if (!recorded) {
      if (capture)
        assert.equal(
          await exists(`${userSecurityReviewReplayRoot}/${stage}`),
          false,
    ...
    if (!capture) continue;
    const file = `${userSecurityReviewReplayRoot}/${stage}/evidence.json`,
    ...
    for (const shot of current.screenshots) {
      ...
      const previousBytes = await readFile(`${entry.folder}/${old.file}`),
        newBytes = await readFile(`${userSecurityReviewReplayRoot}/${stage}/${shot.file}`);
...
if (capture) {
  ...
  const evidence = {
    kind: "P43-security-current-replay-r1",
    approval: "pending",
    ...
    boundary:
      "Original pinned drivers and assertions; only static import resolution (including current P43 review helper) and output directory changed. Current Vue/CSS with review-only template composition, not the untransformed App, historical manifests pinned separately, original images untouched. Local fixture-only requests, not true password reset or role/session changes, full App shell, permission or production acceptance. Byte differences are observations, not approvals or pixel-equivalence claims.",
  };
  await writeFile(
    `${userSecurityReviewReplayRoot}/evidence.json`,
    JSON.stringify(evidence, null, 2) + "\n",
    { flag: "wx" },
  );
  await writeFile(
    `${userSecurityReviewReplayRoot}/index.html`,
    '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P43 改密与原因当前重放</title><h1>P43 改密与原因当前 Vue 重放</h1><p>旧图另存，当前测试样例；未获得新批准，不代表真实改密、授权、会话撤销或生产验收。</p>' +
      summaries
        .map(
          (item) =>
