...
const state = {
  ...
  approvalTemplateNodeId: randomUUID(),
  approvalRequestId: randomUUID(),
  approvalNodeRunId: randomUUID(),
...
async function seed() {
  ...
  await pool.query(
    "INSERT INTO approval_templates(id,organization_id,workspace_id,name,resource_type,status,current_version,revision,created_by,published_by,published_at,created_at,updated_at) VALUES(?,?,?,'生产验收审批模板','task','published',1,1,?,?,?,?,?)",
    [
      state.approvalTemplateId,
      state.organizationId,
      state.workspaceId,
      state.users.selection_manager,
      state.users.selection_manager,
  ...
  await pool.query(
    ...
    [
      state.approvalTemplateVersionId,
      state.approvalTemplateId,
      state.organizationId,
      state.workspaceId,
      state.users.selection_manager,
  ...
  await pool.query(
    "INSERT INTO approval_template_nodes(id,template_version_id,organization_id,workspace_id,ordinal,name,approver_id,sla_minutes,escalation_assignee_id) VALUES(?,?,?,?,1,'生产验收复核',?,60,?)",
    [
      state.approvalTemplateNodeId,
      state.approvalTemplateVersionId,
      state.organizationId,
      state.workspaceId,
      state.users.organization_admin,
  ...
  await pool.query(
    "INSERT INTO approval_requests(id,organization_id,workspace_id,template_id,template_version_id,resource_type,resource_id,title,status,current_node_ordinal,requested_by,completed_at,version,created_at,updated_at) VALUES(?,?,?,?,?,'task',?,'生产验收任务审批','pending',1,?,NULL,1,?,?)",
    [
...
async function cleanup() {
  ...
  try {
    ...
    const directIds = {
      ...
      approval_template_nodes: [state.approvalTemplateNodeId],
      approval_requests: [state.approvalRequestId],
      approval_node_runs: [state.approvalNodeRunId],
