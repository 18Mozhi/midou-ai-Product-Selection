...
const output = "output/playwright/p44-page-vue-preview";
...
const sources = new Set([
  ...
  "scripts/verify-ui-phase2-admin-page-preview.mjs",
]);
...
document.documentElement.dataset.design='signal-ledger';document.body.classList.add('p43-page-preview','p44-page-preview');
...
try {
  ...
  if (capture) {
    await writeFile(
      ...
      JSON.stringify(
        {
          kind: "P44-PAGE-VUE-PREVIEW-r1",
          approval: "pending-user-review",
          scope:
            "Actual parent/admin records/detail/role comparison Vue scripts. Review-only parent composition and C CSS. Original overview and roles plus explicit no-role row derived from the original user, and synthetic long/empty variants; global summary untouched. GET-only intercepted API, no real accounts or authorization. Comparison retains original layout and old matrix after role failure without explicit stale-matrix notice. Creation default/cancel only; no full App/KeepAlive, write, database, theme/density or whole-page acceptance.",
          sourceHashes,
    ...
    const gallery =
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P44 管理员目录审核</title><style>body{font:16px/1.6 "Microsoft YaHei",sans-serif;background:#eef2f7;color:#142a46;margin:24px}article{margin:24px 0;padding:20px;background:white}img{max-width:100%;height:auto}nav{display:flex;gap:12px;flex-wrap:wrap}</style><h1>P44 可授权账号目录 · 实际Vue C审核</h1><p>测试数据，未上线，待用户审核。详情及比较仅证明入口组合，不代表所有子区域C设计或权限通过。</p><nav>' +
