...
const output = args.length ? path.resolve(`output/playwright/p66-read-focus-${args[1]}`) : null;
if (output) await mkdir(output);
const { fixture, nav: navigationFixture } = await topologyReviewFixtures(),
  probe = reservePort();
const envelope = (data) => ({
  data,
  request_id: "p66-focus-fixture",
  trace_id: "p66-focus-fixture",
});
...
const sources = new Set([
    ...
    "scripts/verify-topology-read-focus.mjs",
...
try {
  ...
  console.log(`P66 read focus ${origin}`);
    ...
    [1440, 390].flatMap((width) =>
      ["reduce", "no-preference"].map(async (motion) => {
        ...
        try {
          ...
          await state.getByText("运行状态暂不可用", { exact: true }).waitFor();
          ...
          await page.waitForFunction(
            () => {
              const el = document.querySelector('.topology-read-action[aria-disabled="true"]');
              if (!el) return false;
              const style = getComputedStyle(el);
              return (
                style.backgroundColor === "rgb(237, 240, 245)" &&
                style.color === "rgb(102, 117, 138)"
              );
            },
            undefined,
            { timeout: 8000 },
          ...
          check(
            ...
            ["rgb(237, 240, 245)", "rgb(102, 117, 138)", "1", "rgb(23, 72, 160)", "3px"],
          ...
          const nav = surface.getByRole("link", { name: "告警与阻断", exact: true });
          mode = "hold";
          await activate(refresh);
          await header.getByRole("button", { name: "正在刷新…", exact: true }).waitFor();
          await nav.focus();
  ...
  if (output) {
    ...
    await writeFile(
      ...
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>P66读取焦点审核</title><style>body{font:16px/1.7 Micr' +
        "osoft YaHei;margin:24px}img{max-width:100%;display:block;border:1px solid #c7d3e4}article{margin:28px 0}</styl" +
        "e><h1>P66读取焦点局部待审</h1><p>本地样例；不代表真实权限、探针或全页验收。</p>" +
        images
          .map(
            (i) =>
