...
const args = process.argv.slice(2),
  smoke = process.env.P03_SMOKE === "1";
assert.ok(
  args.length === 0 ||
    (args.length === 2 && args[0] === "--capture-review" && /^r[1-9]\d*$/.test(args[1])),
);
const widths = process.env.P03_VIEWPORT
  ? [Number.parseInt(process.env.P03_VIEWPORT, 10)]
  : smoke
    ? [390]
    : [1440, 390];
const motions = process.env.P03_MOTION
  ? [process.env.P03_MOTION]
  : smoke
    ? ["reduce"]
...
const output = args.length
  ? path.resolve(`output/playwright/p03-page-composition-${args[1]}`)
  : null;
...
try {
      ...
      try {
        ...
        await context.route("**/*", async (route) => {
          ...
          return route.fulfill({
            ...
            json: {
              data: { accepted: true },
              request_id: "p03-register-review",
              trace_id: "p03-register-review",
            },
          });
        });
        ...
        check(
          await root
            .getByText("确认密码仅用于浏览器内一致性校验；服务端只接收邮箱和密码。", {
              exact: true,
            })
            .count(),
          1,
          "payload boundary shown",
        );
        check(
        ...
        check(
          await createButton.evaluate((node) => getComputedStyle(node).outlineWidth),
          "3px",
        ...
        check(
          await root.getAttribute("data-mode"),
          "verify",
          "successful registration is pending verification",
        );
        check(
          await page.getByRole("button", { name: "返回登录", exact: true }).count(),
          1,
          "verify keeps return action",
        );
        check(
          await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1),
          true,
          ...
          [
            {
              ...
              body: { email: "new.member@example.test", password: "Long-enough-password-123!" },
            },
          ],
          "registration payload omits confirmation",
        );
      ...
      JSON.stringify(
        {
          ...
          scope:
            "local actual Vue C review; registration POST is locally intercepted after a zero-POST mismatch",
  ...
  console.log(
    JSON.stringify({
      ...
      writes: allResults.reduce((sum, result) => sum + result.writes, 0),
