...
const sources = new Set([
  ...
  "scripts/verify-ui-phase2-account-create-preview.mjs",
...
const entry = "/__p39_create_preview.js";
...
document.documentElement.dataset.design='signal-ledger';document.body.classList.add('p39-create-preview');
const router=createRouter({history:createWebHistory(),routes:[{path:'/:pathMatch(.*)*',component:{render:()=>null}}]});
const app=createApp({render:()=>h('main',[h('p','P39 创建用户 · 原 Vue 模板/逻辑 + 独立 C 样式 · 测试数据 · 待审'),h(Current,{apiBaseUrl:'/api/v1',routePath:'/platform-admin/accounts'})])}).use(router);await router.isReady();app.mount('#app');`;
  ...
  plugins: [
    {
      ...
      configureServer(instance) {
        instance.middlewares.use((req, res, next) => {
          ...
          res.end(
            `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P39 创建弹窗</title><div id="app"></div><script type="module" src="${entry}"></script></html>`,
          );
        });
      },
    },
...
try {
  ...
  console.log(`p39_create_host ${origin}`);
  ...
  for (const width of [390, 760, 761, 1440]) {
    ...
    try {
      ...
      await page.route("**/*", async (route) => {
        ...
        if (
          ...
          return route.fulfill({
            json: {
              data: overview,
              request_id: "p39-create-fixture",
              trace_id: "p39-create-fixture",
            },
          });
        }
        if (
          ...
          return mode === "success"
            ? route.fulfill({
                json: {
                  ...
                  request_id: "p39-create-success",
                  trace_id: "p39-create-success",
                },
              })
            : route.fulfill({
                status: 400,
                json: {
                  ...
                  request_id: "p39-create-error",
                  trace_id: "p39-create-error",
                },
              });
        }
      ...
      const visit = async () => {
        await page.goto(`${origin}/platform-admin/accounts?keep=creation`);
      ...
      requestsByWidth.push({
        ...
        requests,
        limitations:
          "No backend/RBAC/database or real account creation. Current CSS-only dialog preview; full app shell/history and late write ownership excluded.",
      });
    } finally {
      await context.close();
    }
...
if (capture) {
    ...
    JSON.stringify(
      {
        ...
        approval: "pending",
        scope:
          "Original current Vue parent and child scripts/templates; CSS-only isolated creation dialog. All GET/POST intercepted with test samples. Not production C, full shell, backend validation, native popup capture, real MFA/RBAC or account creation.",
  ...
  await writeFile(
    `${output}/index.html`,
    `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P39 创建用户弹窗</title><style>body{max-width:1000px;margin:24px auto;padding:20px;font-family:'Microsoft YaHei',sans-serif;background:#edf1f6;color:#202c3d}img{max-width:100%}section{margin:32px 0}p{line-height:1.6}</style><h1>P39 新建用户 · 待审核</h1><p>原Vue模板与逻辑，仅审核CSS；请求均拦截为测试样例，未创建真实账号。不含原生校验气泡、真实权限、整页或生产验收。</p>${screenshots.map((s) => `<section><h2>${s.viewport.width}×${s.viewport.height} · ${s.label}</h2><img src="${s.file}" alt="${s.label}"></section>`).join("\n")}`,
  );
}
console.log(
  JSON.stringify({
