...
function parseTimingLines(source, candidatePort) {
  ...
  for (const line of source.split(/\r?\n/)) {
    ...
    if (method && path && Number.isFinite(status) && Number.isFinite(seconds) && upstream.split(/\s*,\s*/).some((item) => item.endsWith(`:${candidatePort}`))) records.push({ method, path, status, milliseconds: seconds * 1000, upstreamReached });
  }
  return records;
}
...
async function sendWriteProbeWithDeliveryRetry(baseUrl, runtime, releaseId, correlation, connectAddress = null) {
  ...
  throw lastError ?? fail("release_write_probe_delivery_failed", "write probe did not reach an upstream response");
}
...
async function writeProbeCount(pool, releaseId, buildSha) {
  const [[row]] = await pool.query("SELECT COUNT(*) sample_count FROM deployment_release_write_samples WHERE release_id=? AND build_sha=?", [releaseId, Buffer.from(buildSha, "hex")]);
...
if (argv.includes("--self-test")) {
  const records = parseTimingLines('127.0.0.1 - - [time] "GET /api/v1/health/live HTTP/1.1" status=200 bytes=1 request_time=0.100 upstream_addr="127.0.0.1:4103" upstream_status="200"\n127.0.0.1 - - [time] "POST /api/v1/platform/operations/releases/write-probe HTTP/1.1" status=202 bytes=1 request_time=0.200 upstream_addr="127.0.0.1:4103" upstream_status="202"\n127.0.0.1 - - [time] "POST /api/v1/platform/operations/releases/write-probe HTTP/1.1" status=499 bytes=0 request_time=0.000 upstream_addr="127.0.0.1:4103" upstream_status="-"\n127.0.0.1 - - [time] "GET /api/v1/health/live HTTP/1.1" status=500 bytes=1 request_time=9.000 upstream_addr="127.0.0.1:4101" upstream_status="500"', 4103);
  const reached = records.filter((item) => item.upstreamReached), transportAborts = records.filter((item) => !item.upstreamReached);
  const metrics = { errorRate: 0, readP95: percentile(reached.filter((item) => item.method === "GET").map((item) => item.milliseconds), .95), writeP95: percentile(reached.filter((item) => item.method === "POST").map((item) => item.milliseconds), .95), asyncLag: 2 };
  const thresholds = { errorRate: 1, readP95: 300, writeP95: 600, asyncLag: 60 };
  if (records.length !== 3 || reached.length !== 2 || transportAborts.length !== 1 || transportAborts[0].upstreamReached !== false || reached[1].path !== "/api/v1/platform/operations/releases/write-probe" || metrics.readP95 !== 100 || metrics.writeP95 !== 200 || writeProbeBreach([reached[1]], 1) !== null || writeProbeBreach([{ ...reached[1], status: 401 }], 0) !== "release_write_probe_rejected" || writeProbeBreach([reached[1]], 0) !== "release_write_probe_persistence_mismatch" || breachFor(metrics, 1, 1, 1, thresholds) !== null || breachFor(metrics, 0, 1, 1, thresholds) !== "insufficient_candidate_samples" || breachFor({ ...metrics, errorRate: 1 }, 1, 1, 1, thresholds) !== "error_rate_threshold_exceeded" || splitConfig(0, 4101, 4103).includes(":4103") || !siteConfig("server { location /api/ { proxy_pass http://127.0.0.1:4101; }\n}", "/www/wwwlogs/test.log").includes("$scoutops_release_upstream")) throw fail("release_self_test_failed", "release parser, write persistence, transport scope, thresholds, stable fallback or Nginx rendering drifted");
...
try {
  ...
  const mysqlDurability = { innodbFlushLogAtTrxCommit: Number(durabilityRow?.innodb_flush_log_at_trx_commit), syncBinlog: Number(durabilityRow?.sync_binlog), binlogFormat: String(durabilityRow?.binlog_format??"").toUpperCase(), productScoutBinlogExcluded: ignoredDatabases.includes(runtime.DB_NAME), osCrashDataLossWindowSeconds: 1 };
  if (mysqlDurability.innodbFlushLogAtTrxCommit !== 2 || mysqlDurability.syncBinlog !== 1 || mysqlDurability.binlogFormat !== "ROW" || mysqlDurability.productScoutBinlogExcluded !== false || runtime.DB_NAME !== "product_scout") throw fail("release_mysql_durability_contract_invalid", "MySQL must use the approved one-second redo flush window and include product_scout in ROW binlog");
  const mysqlResourceProfile = { innodbBufferPoolBytes: Number(durabilityRow?.innodb_buffer_pool_size), innodbBufferPoolInstances: Number(durabilityRow?.innodb_buffer_pool_instances), innodbIoCapacity: Number(durabilityRow?.innodb_io_capacity), innodbIoCapacityMax: Number(durabilityRow?.innodb_io_capacity_max), innodbFlushNeighbors: Number(durabilityRow?.innodb_flush_neighbors), innodbFlushMethod: String(durabilityRow?.innodb_flush_method ?? "").toUpperCase() };
  if (mysqlResourceProfile.innodbBufferPoolBytes !== 4294967296 || mysqlResourceProfile.innodbBufferPoolInstances !== 4 || mysqlResourceProfile.innodbIoCapacity !== 1000 || mysqlResourceProfile.innodbIoCapacityMax !== 4000 || mysqlResourceProfile.innodbFlushNeighbors !== 0 || mysqlResourceProfile.innodbFlushMethod !== "O_DIRECT") throw fail("release_mysql_resource_profile_invalid", "MySQL runtime resource profile must match the BaoTa-approved M07-05 single-host configuration");
  ...
  const migrations = [];
  for (const name of ["0026_release_rollout_m07_05.up.sql", "0027_release_write_probe_m07_05.up.sql", "0027a_release_rollout_attempts_m07_05.up.sql", "0032_mysql_resilience_m08_03.up.sql", "0032a_compact_release_write_probe_m08_03.up.sql", "0033_file_resilience_m08_04.up.sql", "0034_crawler_scheduler_m08_05.up.sql", "0035_capacity_boundary_m08_06.up.sql"]) migrations.push(await applyMigration(pool, releaseRoot, name));
  ...
  for (const percent of [5, 25, 100]) {
    ...
    const log = await readFrom(timingLog, startSize).catch(() => ""), parsedRecords = parseTimingLines(log, candidatePort), transportAborts = parsedRecords.filter((item) => !item.upstreamReached), records = parsedRecords.filter((item) => item.upstreamReached);
    ...
    const breach = writeDeliveryFailures > 0 ? "release_write_probe_delivery_failed" : writeProbeBreach(writes, durableWriteSamples) ?? breachFor(metrics, reads.length, durableWriteSamples, minimumSamples, thresholds);
    if (breach) {
      await writeGate(pool, effectiveReleaseId, `canary_${percent}`, "failed", { ...metrics, failureCode: breach, startedAt: gateStarted, metadata: { read_samples: reads.length, write_samples: writes.length, durable_write_samples: durableWriteSamples, pre_upstream_transport_aborts: transportAborts.length, write_delivery_failures: writeDeliveryFailures } });
