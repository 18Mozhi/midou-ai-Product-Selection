...
const args = process.argv.slice(2),
  smoke = process.env.P72_SMOKE === "1";
assert.ok(
  args.length === 0 ||
    (args.length === 2 && args[0] === "--capture-review" && /^r[1-9]\d*$/.test(args[1])),
);
const widths = process.env.P72_VIEWPORT
    ? [Number.parseInt(process.env.P72_VIEWPORT, 10)]
    : smoke
      ? [390]
      : [1440, 390],
  motions = process.env.P72_MOTION
    ? [process.env.P72_MOTION]
    : smoke
      ? ["reduce"]
...
const output = args.length
  ? path.resolve(`output/playwright/p72-page-composition-${args[1]}`)
  : null;
  ...
  for (const width of widths)
    for (const motion of motions) {
      ...
      try {
        ...
        check(
          await root
            .locator(".p72-state-picker button")
            .evaluateAll((nodes) =>
        ...
        for (const [label, kind] of [
          ...
          check(
            await root.locator("#ui-state-preview").getAttribute("data-kind"),
            kind,
            `${kind} selection`,
          );
        }
        ...
        await trigger.focus();
        check(
          await trigger.evaluate((node) => getComputedStyle(node).outlineWidth),
          "3px",
      ...
      JSON.stringify(
        {
          ...
          scope: "local actual Vue C review; dev-only state demonstration with no API writes",
