        ...
        for (const mode of ["success-close", "failure-close", "refresh-failure", "reason-back"]) {
          ...
          try {
            ...
            if (mode === "reason-back") {
              ...
              check(
                baseline
                  ? "baseline reason remains after leaving"
                  : "pending reason invalidated on leaving",
              ...
              if (!baseline) {
                check(
                  "leaving unsubmitted reason performs no write",
                ...
                scenarios.push({
                  ...
                  mode,
                  requests,
                  finalState: await snapshot("settled"),
                  observedOutcome: "pending-reason-invalidated",
                });
                continue;
              }
            }
            ...
            if (mode === "failure-close") {
              ...
              if (!baseline)
                check(
                  "closed action has no hidden error",
                  await detail.locator('[role="alert"]').allTextContents(),
                  [],
                );
              ...
              check(
                baseline
                  ? "BUG late success reopens detail"
                  : "late success cannot reopen closed detail",
                await detail.isVisible(),
                baseline,
              );
              if (!baseline) {
                check(
                  "closed action triggers no overview reread",
                  requests.filter((r) => r.method === "GET" && r.afterWrite).length,
                  0,
                );
                check(
                  "closed action has no success feedback",
                  await detail.locator('[role="status"]').allTextContents(),
                  [],
                );
              }
...
if (capture) {
    ...
    JSON.stringify(
      {
        ...
        remaining: [
          "successful write plus failed reread still shows old overview",
          "full App/KeepAlive and real API/RBAC/DB not proven",
        ],
        scope: baseline
          ? "P42 pre-repair parent replay with existing review layout. All requests intercepted. Not current acceptance or full App/KeepAlive/production."
          : "P42 current production scripts with existing review layout. All requests intercepted. Ownership regression only, not full App/KeepAlive/production or refresh-failure feedback acceptance.",
        ...bindings,
        checks,
        scenarios,
        screenshots,
  ...
  await writeFile(
    `${output}/index.html`,
    `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P42写入生命周期问题证据</title><style>body{max-width:1440px;margin:24px auto;padding:20px;font-family:'Microsoft YaHei',sans-serif;background:#edf1f6}img{max-width:100%}section{margin:36px 0}</style><h1>P42 ${baseline ? "修复前回放" : "操作归属修复后"} · 完整验收仍待</h1><p>合成响应，未修改真实组织。${baseline ? "回放原父脚本" : "当前实际父脚本"}，复用审核布局；非完整App/KeepAlive验证。</p>
