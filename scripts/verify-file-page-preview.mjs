...
import { filePagePlugin, filePageSources } from "./lib/file-page-preview.mjs";
...
const output = args.length
  ? path.resolve(`output/playwright/p69-page-composition-${args[1]}`)
  : null;
if (output) await mkdir(output);
const { data: designData } = await buildFilesDesignData(process.cwd());
...
const sources = new Set([
    ...
    "scripts/verify-file-page-preview.mjs",
...
try {
  ...
  console.log(`P69 actual Vue review ${origin}`);
    ...
    for (const motion of mobileStateReview ? ["reduce"] : ["reduce", "no-preference"]) {
      ...
      try {
        ...
        await page.route("**/*", async (route) => {
          ...
          const envelope = (data) => ({
            data,
            request_id: "p69-local-fixture",
            trace_id: "p69-local-fixture",
          });
          ...
          if (key !== "GET /api/v1/platform/operations/files" || url.search) {
            unexpected.push(key);
            return route.abort();
          }
          reads.push(key);
        ...
        for (const [name, fixture] of Object.entries(designData.datasets).filter(([name]) =>
          ...
          check(
            await root.locator(".p69-findings article").count(),
            fixture.findings.length,
            name + " findings",
          );
          check(
            await root.locator(".p69-directory").count(),
            fixture.directories.length,
            name + " keeps raw directory count",
          );
          check(
          ...
          const validProgress = fixture.directories.filter(
            (directory) =>
              directory.available &&
              directory.total_bytes > 0 &&
              directory.usage_basis_points >= 0 &&
              directory.usage_basis_points <= 10000,
          ).length;
          check(
            await root.getByRole("progressbar").count(),
            validProgress,
            name + " only displays measured filesystem watermarks",
          );
        }
        ...
        check(
          await root
            .locator(".file-resilience__footer")
        ...
        if (output && motion === "reduce") {
          const target = root.locator(".file-resilience__hero");
        ...
        current = {
          error: {
            status: 503,
            error: {
              code: "file_resilience_dependency_unavailable",
              message: "本机文件事实暂不可用。",
              action_hint: "请在宝塔核对 Node API 与受控目录。",
            },
            request_id: "p69-local-retained-failure",
            trace_id: "p69-local-retained-failure",
          },
        };
        await refresh.click();
        const retained = root.locator(".file-resilience__refresh-notice");
        await retained.waitFor();
        check(await retained.isVisible(), true, "retained unavailable notice visible");
        check(
          await root.locator(".p69-directories").count(),
          1,
          "retained unavailable keeps facts",
        );
        check(
          (await retained.textContent())?.includes("已保留上次成功的本机文件事实") ?? false,
          true,
        ...
        if (output && motion === "reduce") {
          const buffer = await retained.screenshot({ animations: "disabled" });
          const file = `${width}-retained-focus.png`;
          await writeFile(path.join(output, file), buffer);
          images.push({
            file,
            sha256: hash(buffer),
        ...
        current = {
          error: {
            ...
            error: {
              ...
              message: "当前账号无权读取本机文件事实。",
              action_hint: "需要 platform:operate 能力。",
            },
            request_id: "p69-local-forbidden",
            trace_id: "p69-local-forbidden",
          },
        };
        await refresh.click();
        const forbidden = root.locator(".file-resilience__state--danger");
        await forbidden.waitFor();
        check(await forbidden.isVisible(), true, "forbidden initial state visible");
        check(await root.locator(".p69-directories").count(), 0, "forbidden clears facts");
        check(
        ...
        if (output && motion === "reduce")
          for (const [name, selector] of [
            ["default", null],
            ["directories", ".p69-directories"],
            ["integrity", ".p69-integrity"],
            ["recovery", ".p69-recovery"],
          ]) {
      ...
      JSON.stringify(
        {
          ...
          scope:
            "local actual Vue C review; inert source datasets, not live filesystem or recovery",
