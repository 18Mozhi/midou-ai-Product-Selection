...
// Independent supplement: do not alter the original 72-image assembly package.
...
const sources = new Set([
  ...
  "scripts/verify-ui-phase2-admin-page-boundary-preview.mjs",
]);
...
${baseline ? "" : `import '/@fs/${path.resolve(boundaryStyle).replaceAll("\\", "/")}';`}
...
${baseline ? "" : "document.body.classList.add('p44-boundary');"}
...
try {
  ...
  for (const width of [1200, 1201]) {
    ...
    try {
      ...
      const shot = async (state, focus) => {
        ...
        if (capture && captureStates.has(state)) {
      ...
      const beforeKeyboard = requests.length,
        keyboardUrl = page.url();
      const keyboardTargets = [
        [
          "create",
          page.locator(".account-hero").getByRole("button", { name: "新建管理员", exact: true }),
          "rgb(66, 118, 223)",
        ],
        ["navigation", page.locator('.account-tabs a[aria-current="page"]'), "rgb(255, 255, 255)"],
        [
          "email",
          page.locator("form.account-filter").getByRole("textbox", { name: /^账号邮箱/ }),
          "rgb(66, 118, 223)",
        ],
        [
          "differences",
          page.getByRole("checkbox", { name: "只看差异", exact: true }),
          "rgb(255, 255, 255)",
        ],
        [
          "left-role",
          page.getByRole("combobox", { name: "左侧角色", exact: true }),
          "rgb(66, 118, 223)",
        ],
      ];
      const keyboardFocus = async (name, locator, color) => {
        const trace = [];
        for (let i = 0; i < 30; i++) {
          ...
          if (await locator.evaluate((n) => n === document.activeElement)) break;
        }
        check(
          name + ":reached by Tab",
          await locator.evaluate((n) => n === document.activeElement),
        );
        ...
        await page
          ...
          .catch(() => {});
        const actual = await locator.evaluate((n) => {
        ...
        check(name + ":focus visible", actual.focusVisible);
        if (!baseline) {
          check(name + ":outline color", actual.outlineColor, color);
          check(name + ":outline width", actual.outlineWidth, "3px");
        }
        check(name + ":outline nonzero", Number.parseFloat(actual.outlineWidth) > 0);
        check(name + ":outline solid", actual.outlineStyle, "solid");
        if (!actual.inViewport) console.log(JSON.stringify({ width, name, actual }));
        check(name + ":inside viewport", actual.inViewport);
        if (!baseline) check(name + ":outline inside viewport", actual.outlineInViewport);
        focusObservations.push({
          ...
          trace,
          expectedColor: color,
          matchesDesign: actual.outlineColor === color && actual.outlineWidth === "3px",
        });
        await shot(
          "keyboard-" + name,
          await locator.evaluate((n) => {
            // Temporary DOM-only marker for screenshot scrolling; never added to source or runtime contract.
            n.dataset.boundaryFocus = "current";
            return '[data-boundary-focus="current"]';
          }),
        );
        await locator.evaluate((n) => delete n.dataset.boundaryFocus);
      };
      for (const [name, locator, color] of keyboardTargets)
        await keyboardFocus(name, locator, color);
      const compareSearch = page
        .locator(".role-comparison")
        .getByRole("textbox", { name: "搜索权限", exact: true });
      const compareReset = page
        .locator(".role-comparison")
        .getByRole("button", { name: "重置", exact: true });
      check("comparison reset initially disabled", await compareReset.isDisabled());
      await compareSearch.fill("报表");
      check("comparison reset enabled with search", await compareReset.isEnabled());
      await keyboardFocus("reset", compareReset, "rgb(66, 118, 223)");
      // Restore the fixture state before the existing behavioral replay, without activating reset.
      await compareSearch.fill("");
      await compareSearch.evaluate((n) => n.blur());
      check("keyboard traversal no requests", requests.length, beforeKeyboard);
      check("keyboard traversal no URL changes", page.url(), keyboardUrl);
  ...
  if (capture) {
    await writeFile(
      ...
      JSON.stringify(
        {
          kind: "P44-PAGE-BOUNDARY-VUE-PREVIEW-r1",
          variant: baseline ? "baseline" : "focus-corrected-review",
          approval: "pending-user-review",
          scope:
            "1200/1201 breakpoint and six keyboard focus targets only. Independent supplement using current Vue and unchanged assembled C review CSS; original72 images preserved. " +
    ...
    const gallery =
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P44 断点与键盘焦点补充审核</title><style>body{font:16px/1.6 "Microsoft YaHei",sans-serif;background:#eef2f7;color:#142a46;margin:24px}article{margin:24px 0;padding:20px;background:white}img{max-width:100%;height:auto}nav{display:flex;gap:12px;flex-wrap:wrap}</style><h1>P44 1200/1201px · 断点与键盘焦点</h1><p>测试数据，未上线，待用户审核。详情及比较仅证明入口组合，不代表所有子区域C设计或权限通过。</p><nav>' +
      [1200, 1201].map((w) => '<a href="#' + w + '-default">' + w + "px</a>").join("") +
      "</nav>" +
      screenshots
        .map(
          (s) =>
