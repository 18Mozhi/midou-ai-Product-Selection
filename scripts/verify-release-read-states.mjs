...
const output = args.length ? path.resolve(`output/playwright/p65-read-states-${args[1]}`) : null;
...
const sources = new Set([
    ...
    "scripts/verify-release-read-states.mjs",
...
const envelope = (data, id = "p65-local-snapshot") => ({ data, request_id: id, trace_id: id });
let browser;
try {
  await server.listen();
  browser = await chromium.launch();
  const origin = `http://127.0.0.1:${port}`;
  console.log(`P65 actual Vue read states ${origin}`);
    ...
    cases.map(async ({ width, motion }) => {
      ...
      try {
        ...
        await page.route("**/*", async (route) => {
          ...
          return route.fulfill({
            ...
            json: {
              ...
              request_id: "p65-local-failure-" + mode,
            },
          });
        });
        await page.addInitScript(() => {
          window.__releaseCopyWrites = [];
          window.__releaseCopyReject = false;
          Object.defineProperty(navigator, "clipboard", {
            configurable: true,
            value: {
              writeText: async (value) => {
                window.__releaseCopyWrites.push(value);
                if (window.__releaseCopyReject) throw new Error("local clipboard denial");
              },
            },
          });
        });
        ...
        const surface = page.locator(".release-center--review"),
          state = surface.locator(".state"),
          notice = surface.locator(".refresh-notice"),
          header = surface.locator(".hero"),
          truth = surface.locator(".verdict"),
        ...
        const snapshotTrace = async () => {
          check(
            await surface.locator("footer .technical-details code").textContent(),
            "p65-local-snapshot",
            "retained snapshot keeps successful request identity",
          );
        };
        await state.getByRole("heading", { name: "正在核验当前发布", exact: true }).waitFor();
        check(await state.getAttribute("aria-busy"), "true", "loading region busy");
        check(
          await surface.getByRole("region", { name: "正在核验当前发布", exact: true }).count(),
          1,
        ...
        await state
          .getByRole("heading", { name: "发布事实读取超时", exact: true })
        ...
        check(
          await state
            .getByText("读取超过 15 秒，已停止本次等待。请重新核验。", { exact: true })
            .count(),
          1,
        ...
        const firstFailure = async (code, name, attempts, image) => {
          ...
          check(
            await surface.locator("#p65-metrics").count(),
            0,
            "first failure has no gate metrics",
          );
          check(await state.getAttribute("aria-busy"), "false", "settled named failure not busy");
          await trace(state, "p65-local-failure-" + code);
          check(await surface.locator("footer").count(), 0, "no snapshot trace on first failure");
          await capture(image, state);
        };
        await firstFailure("503", "发布事实暂不可用", 3, "initial-unavailable");
        ...
        check(
          await truth.getAttribute("data-state"),
          "verified",
        ...
        check(await surface.locator("#p65-metrics").count(), 1, "timeout keeps old metrics");
        await trace(notice, requests.at(-1).requestId);
        await snapshotTrace();
        await capture("retained-timeout", notice);
        await trace(surface.locator("footer"), "p65-local-snapshot");
        await capture("retained-snapshot-trace", surface.locator("footer"));
        const snapshotCopy = surface
          .locator("footer")
          .getByRole("button", { name: "复制请求编号", exact: true });
        await snapshotCopy.click();
        await surface.locator("footer").getByText("已复制", { exact: true }).waitFor();
        check(
          await page.evaluate(() => window.__releaseCopyWrites.at(-1)),
          "p65-local-snapshot",
          "snapshot copy consumer does not use failure ID",
        );
        await capture("snapshot-copy-success", surface.locator("footer"));
        await page.evaluate(() => {
          window.__releaseCopyReject = true;
        });
        await snapshotCopy.click();
        await surface.locator("footer").getByRole("status").waitFor();
        check(
          await surface.locator("footer").getByRole("status").textContent(),
          "暂时无法复制请求编号，可以选中上方内容后手动复制。",
          "copy denial remains explicit",
        );
        check(
          await page.evaluate(() => window.__releaseCopyWrites.length),
          2,
          "two simulated clipboard attempts only",
        );
        await capture("snapshot-copy-denied", surface.locator("footer"));
        for (const code of ["503", "429"]) {
          ...
          check(await surface.locator("#p65-metrics").count(), 1, "retained data after " + code);
          await trace(notice, "p65-local-failure-" + code);
          await snapshotTrace();
        ...
        check(await surface.locator("#p65-metrics").count(), 0, "403 clears old metrics");
        check(await notice.count(), 0, "403 is not a retained failure");
        await trace(state, "p65-local-failure-403");
        check(await surface.locator("footer").count(), 0, "403 clears snapshot trace");
        ...
        check(await surface.locator("#p65-metrics").count(), 0, "401 clears old metrics");
        await trace(state, "p65-local-failure-401");
        check(await surface.locator("footer").count(), 0, "401 clears snapshot trace");
        await capture("cleared-expired", state);
        mode = "empty";
        await refresh().click();
        await truth.getByText("尚无发布记录", { exact: true }).waitFor();
        check(
          await surface.getByText("暂无记录", { exact: true }).count(),
          1,
  ...
  if (output) {
    ...
    await writeFile(
      ...
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>P65读取状态审核</title><style>body{font:16px/1.7 Micr' +
        "osoft YaHei;margin:24px;color:#182739}img{max-width:100%;display:block;border:1px solid #c7d3e4}article{margin" +
        ":28px 0}</style><h1>P65 实际Vue读取状态待审</h1><p>本地接口样例，未运行真实发布/回滚。四组检查，双端减少动效图；不代表真实权限或完整读屏验收。</p>" +
        images
          .map(
            (i) =>
