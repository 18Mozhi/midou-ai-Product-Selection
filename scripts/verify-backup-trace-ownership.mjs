...
const output = args.length
  ? path.resolve(`output/playwright/p64-trace-ownership-${args[1]}`)
  : null;
...
const envelope = (data) => ({
  data,
  request_id: "p64-trace-fixture",
  trace_id: "p64-trace-fixture",
});
...
const sources = new Set([
    ...
    "scripts/verify-backup-trace-ownership.mjs",
...
try {
  ...
  console.log(`P64 trace ownership ${origin}`);
      ...
      ["reduce", "no-preference"].map(async (motion) => {
        ...
        try {
          ...
          const surface = page.locator(".backup-center--review"),
            ...
            snapshot = surface.locator("footer .technical-details");
          const failureTrace = () =>
            surface.locator(".state-card .technical-details, .refresh-notice .technical-details");
          ...
          check(
            copyStyle,
            ["rgb(255, 255, 255)", "rgb(18, 73, 184)", "1px"],
            "C copy button replaces native bevel",
          );
          check(
            await failureTrace()
              .locator("summary")
          ...
          await open(snapshot);
          check(
            await surface.locator("footer").evaluate((el) => getComputedStyle(el).textAlign),
            "left",
            "snapshot footer aligns with trace",
          );
          await capture("snapshot-expanded", surface.locator("footer"));
          mode = "failure";
          ...
          await capture("retained-snapshot-expanded", surface.locator("footer"));
          ...
          check(
            await surface.locator(".truth-banner").count(),
            0,
          ...
          failure.json.request_id = "empty-snapshot-failure";
          await refresh.click();
          await notice.waitFor();
          check(
            await snapshot.locator("code").textContent(),
          ...
          check(
            await failureTrace().locator("code").textContent(),
            "empty-snapshot-failure",
            "empty refresh failure ID independent",
          );
          await open(snapshot);
          await capture("empty-snapshot-expanded", surface.locator("footer"));
          await open(failureTrace());
          await page.evaluate(() => {
          ...
          check(requests.length, 9, "nine reads and no duplicates");
  ...
  if (output) {
      ...
      JSON.stringify(
        {
          ...
          scope:
            "actual Vue local trace ownership and clipboard consumer checks; no real backup/restore or permission acceptance",
    ...
    await writeFile(
      ...
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>P64读取追踪审核</title><style>body{font:16px/1.7 Microsoft YaHei;margin:24px}img{max-width:100%;display:block;border:1px solid #c7d3e4}article{margin:28px 0}</style><h1>P64读取追踪局部待审</h1><p>本地样例；不代表真实权限、恢复或全页验收。</p>' +
        images
          .map(
            (i) =>
