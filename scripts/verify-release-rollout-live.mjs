...
async function cleanup() {
  ...
  try { await pool.query("DELETE FROM deployment_release_write_probes WHERE release_id=?", [ids.release]); } catch {}
...
try {
  ...
  await pool.query("INSERT INTO deployment_releases(id,stage,app_version,build_sha,config_fingerprint,migration_version,status,approved_by,request_id,trace_id,started_at,finished_at,created_at,updated_at) VALUES(?,'S0','0.1.0',?,?,?,'healthy',NULL,?,?,?,?,?,?)", [ids.release,"a".repeat(40),"b".repeat(64),"0027_release_write_probe_m07_05.up.sql",requestId,traceId,new Date(now.getTime()-5_400_000),new Date(now.getTime()-60_000),new Date(now.getTime()-5_400_000),now]);
  const signingKey="m07-05-live-signing-key-with-32-characters",sampleId=randomUUID(),nonce=randomUUID(),timestamp=Math.floor(now.getTime()/1000),signatureInput={timestamp,nonce,requestId,traceId,releaseId:ids.release,sampleId};
  const writeProbe = new ReleaseWriteProbeService(new MySqlReleaseRolloutRepository(pool),signingKey,"a".repeat(40),60,()=>now);
  await writeProbe.record({...signatureInput,signature:signReleaseProbe(signatureInput,signingKey)});
  const [probes]=await pool.query("SELECT build_sha,request_id,trace_id FROM deployment_release_write_probes WHERE release_id=? AND sample_id=?",[ids.release,sampleId]);
  if(probes.length!==1||probes[0].build_sha!=="a".repeat(40)||probes[0].request_id!==requestId||probes[0].trace_id!==traceId)throw new Error("signed durable write probe mismatch");
  for (const percent of [5,25,100]) await pool.query("INSERT INTO deployment_release_gates(id,release_id,gate_kind,status,traffic_percent,observe_seconds,sample_count,error_rate_percent,read_p95_ms,write_p95_ms,async_lag_seconds,failure_code,started_at,finished_at,request_id,trace_id,metadata) VALUES(?,?,?,'passed',?,1800,20,0,100,200,2,NULL,?,?,?,?,NULL)", [randomUUID(),ids.release,`canary_${percent}`,percent,new Date(now.getTime()-2_000_000),new Date(now.getTime()-60_000),requestId,traceId]);
