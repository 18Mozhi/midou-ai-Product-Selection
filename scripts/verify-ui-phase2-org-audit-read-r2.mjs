...
const output = "output/playwright/p37-parent-read-vue-r2";
...
const host = `import {createApp,h,KeepAlive,nextTick} from 'vue';
...
window.__away=()=>router.push('/__away');window.__tick=()=>nextTick();
...
try {
  ...
  console.log(`p37_current_read_r2_host ${origin}`);
  ...
  for (const width of smoke ? [390] : [390, 1440]) {
    ...
    try {
      ...
      await page.waitForFunction(() => document.querySelectorAll(".org-audit-row").length === 55);
      check("normal-page:55-rows", await rows.count(), 55);
      check("normal-page:exact-cursor", requests.at(-1).query.cursor, data.events[49].id);
      await shot("normal-page", ".org-audit-ledger");
      for (const [oldStatus, refreshStatus, oldFirst] of [
        [200, 200, false],
        [200, 200, true],
        [403, 200, false],
        [500, 200, false],
        ...
        await visit();
        await waitState("ready");
        mode.status = oldStatus;
        mode.hold = true;
        await page.getByRole("button", { name: "加载更多记录", exact: true }).click();
        await eventually(() => held.length === 1);
        check("race:old-exact-cursor", requests.at(-1).query.cursor, data.events[49].id);
        const requestCount = requests.length;
        await page
          .getByRole("button", { name: "正在加载…", exact: true })
          .evaluate((node) => node.click());
        check("race:no-duplicate-page", requests.length, requestCount);
        await page.evaluate(() => {
          window.__rowCounts = [];
          new MutationObserver(() =>
            window.__rowCounts.push(document.querySelectorAll(".org-audit-row").length),
          ).observe(document.querySelector(".org-admin-center"), {
            childList: true,
            subtree: true,
          });
        });
        ...
        const oldResponse = held.shift(),
          refreshResponse = held.shift();
        if (oldFirst) {
          await oldResponse();
          ...
          await refreshResponse();
          await page.waitForFunction(() => {
            const node = document.querySelector(".org-admin-center");
            return (
              node?.dataset.state === "forbidden" ||
              document.querySelectorAll(".org-audit-row").length === 10 ||
              document.querySelector(".org-admin-notice")
            );
          });
          await oldResponse();
        ...
        const expected = refreshStatus === 200 ? 10 : refreshStatus === 403 ? 0 : 50;
        const name = `page-${oldStatus}-refresh-${refreshStatus}-${oldFirst ? "old-first" : "new-first"}`;
        check(name + ":latest-row-count", await rows.count(), expected);
        check(
          name + ":never-append-old-five",
          await page.evaluate(() => window.__rowCounts.every((n) => [0, 10, 50].includes(n))),
          true,
        );
        if (refreshStatus === 403)
          check(
            name + ":permission-state-retained",
            await page.locator(".org-admin-center").getAttribute("data-state"),
            "forbidden",
          );
        if (refreshStatus === 500)
          check(
            name + ":new-notice-retained",
            (await page.locator(".org-admin-notice").innerText()).includes("隔离读取样例 500"),
            true,
          );
        if (refreshStatus === 200)
          check(name + ":no-stale-error", await page.locator(".org-admin-notice").count(), 0);
        await shot(name, refreshStatus === 403 ? ".org-admin-center" : ".org-audit-ledger");
      }
      await visit();
      await waitState("ready");
      ...
      await page.evaluate(() => window.__tick());
      check(
        "copy:old-feedback-ignored",
        (await page.locator(".org-audit-correlation button").first().textContent()).trim(),
        "复制",
      );
      ...
      await shot("late-copy-ignored", ".org-audit-detail");
      check(
...
if (!smoke) {
  ...
  const result = {
    kind: "P37-PARENT-READ-VUE-r2",
    scope:
      "Actual parent/child/API client in isolated Router/KeepAlive host; synthetic GET fixtures only. Not real backend/RBAC/shell/production or C design acceptance. Current pagination ownership and copy regressions pass; two history restoration classes remain unresolved. Not whole-page acceptance.",
    sourceHashes,
    checks,
    findings,
    requestEvidence,
...
<style>body{font:16px/1.6 'Microsoft YaHei';margin:24px;background:#f5f6f8;color:#202c3d}figure{padding:16px;background:white;margin:24px 0}img{max-width:100%;height:auto}</style><h1>P37 实际父子组件 · 问题核对</h1><p>旧UI实际运行样例，不是新C稿；分页归属与复制局部修复；两类查询历史问题仍待处理，整页验收不通过。截图不含地址栏，URL与字段差异见证据。</p><a href="evidence.json">完整证据</a>
