...
const widths = [1440, 390],
  motions = ["reduce", "no-preference"],
  output = args.length ? path.resolve(`output/playwright/p24-page-composition-${args[1]}`) : null;
...
const nav = {
  shell: "member",
  organization_id: "p24-org",
  workspace_id: "p24-workspace",
  roles: ["member"],
  capabilities: ["task:read", "task:update", "task:assign"],
  platform_roles: [],
...
const detail = (overrides = {}) => ({
  id: taskId,
  title: "复核便携咖啡机成本",
  description: "核对成本证据、运费与目标利润，再决定是否推进。",
  status: "in_progress",
  priority: "high",
  ...
  progress_percent: 40,
  progress_note: "已完成成本初检，等待运费确认。",
  version: 2,
  comments: [
    {
      id: "comment-1",
      body: "供应商报价已补齐，待核对最低起订量。",
      created_by: "member-2",
...
try {
      ...
      try {
        ...
        await context.route("**/*", async (route) => {
          ...
          if (key === "GET /api/v1/auth/session-status")
            return route.fulfill({ json: envelope({ authenticated: true }, "p24-session") });
          if (key === "GET /api/v1/me/navigation")
            return route.fulfill({ json: envelope(nav, "p24-nav") });
          if (key === "GET /api/v1/me/ui-preferences")
            return route.fulfill({
              json: envelope({ theme: "deep-ocean", version: 1 }, "p24-theme"),
            });
          if (key === `GET /api/v1/tasks/${taskId}`)
            return failure
              ? route.fulfill({
                  status: 404,
                  json: {
                    ...
                    request_id: "p24-missing",
                    trace_id: "p24-missing",
                  },
                })
              : route.fulfill({ json: envelope(detail(), "p24-detail") });
          ...
          if (key === `POST /api/v1/tasks/${taskId}/actions`) {
            writes.push({ key, body: request.postDataJSON() });
            return route.fulfill({
              json: envelope(
                detail({
                  progress_percent: 45,
                  progress_note: "已核对成本与运费，下一步确认最低起订量。",
                  version: 3,
                }),
                "p24-action",
              ),
        ...
        await page.goto(`${origin}/tasks/${taskId}?from=%2Ftasks`, {
          waitUntil: "domcontentloaded",
        });
        const root = page.locator(".p24-workspace");
        await root
          .getByRole("heading", { name: "复核便携咖啡机成本", exact: true })
          .waitFor({ timeout: 8000 });
        check(
          await root
            .getByText("版本、负责人、期限与活动记录均来自当前任务响应；操作按现有权限显示。", {
              exact: true,
            })
            .count(),
          1,
        ...
        check(
          await root.getByText("当前成员", { exact: true }).count(),
          2,
          "member directory labels",
        );
        const progress = root.getByRole("button", { name: "更新进度", exact: true });
        await progress.focus();
        check(
          await progress.evaluate((node) => getComputedStyle(node).outlineWidth),
          "3px",
          "progress focus visible",
        );
        await capture(page, "ready");
        await progress.click();
        const dialog = page.getByRole("dialog", { name: "任务操作表单" });
        await dialog.getByLabel("完成进度（0–100）").fill("45");
        await dialog.getByLabel("本次进展说明").fill("已核对成本与运费，下一步确认最低起订量。");
        await capture(page, "progress-dialog");
        await dialog.getByRole("button", { name: "确认提交", exact: true }).click();
        await page.waitForFunction(() => !document.querySelector(".p24-action-dialog[open]"));
        check(
          writes[0],
          {
            key: `POST /api/v1/tasks/${taskId}/actions`,
            body: {
              action: "progress",
              expected_version: 2,
              progress_percent: 45,
          ...
          "exact progress action body",
        );
        check(writes.length, 1, "single action write");
        failure = true;
        const missing = await context.newPage();
        await missing.goto(`${origin}/tasks/${taskId}`, { waitUntil: "domcontentloaded" });
        const missingRoot = missing.locator(".p24-workspace");
        await missingRoot
          .getByText("任务不存在或已删除", { exact: true })
          .waitFor({ timeout: 8000 });
        check(
          await missingRoot.getByRole("button", { name: "重新加载", exact: true }).count(),
          1,
          "not found retry",
        );
        await capture(missing, "not-found");
  ...
  const sources = [
    ...
    "scripts/lib/task-detail-page-preview.mjs",
    "scripts/verify-task-detail-page-preview.mjs",
    taskDetailReviewCss,
  ];
  if (output)
    await writeFile(
