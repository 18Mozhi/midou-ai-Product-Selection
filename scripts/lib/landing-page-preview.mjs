...
export const landingReviewCss = "design-plans/ui-phase-2-2026-09-07/implementation/landing-page-preview.css";
export const landingPageSources = [landingReviewCss, "scripts/lib/landing-page-preview.mjs", "apps/web/src/components/LandingRedirect.vue", "apps/web/src/components/UiStatePanel.vue"];
...
export function landingPagePlugin() {
  return { name:"p01-actual-vue-review", enforce:"pre", transform(source,id) { const file=id.replaceAll("\\","/"), target=path.resolve("apps/web/src/components/LandingRedirect.vue").replaceAll("\\","/"); if(file===target)return {code:previewLandingPage(source),map:null}; }, transformIndexHtml(html) { assert.equal(html.split("<body>").length,2,"P01 body anchor"); return html.replace("<body>",'<body class="p01-review">').replace("</head>",`<link rel="stylesheet" href="/@fs/${path.resolve(landingReviewCss).replaceAll("\\","/")}"></head>`); } };
}
