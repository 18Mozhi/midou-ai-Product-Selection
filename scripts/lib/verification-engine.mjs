...
const SECRET_PATTERN =
  /(password|secret|token|cookie|api[_-]?key|private[_-]?key)(\s*[=:]\s*)([^\s,;'\"]+)/gi;
...
export function expectedAtomicTaskIds(moduleId) {
  return Array.from(
    { length: 17 },
    (_, index) => `${moduleId}.A${String(index + 1).padStart(2, "0")}`,
  );
}

export function validateModuleRegistry(registry, requestedModuleId) {
  if (!MODULE_PATTERN.test(requestedModuleId)) {
    throw new VerificationError(`Invalid module ID ${requestedModuleId}`, {
      code: "invalid_module_id",
    });
  }
  if (registry.moduleId !== requestedModuleId) {
    throw new VerificationError(`${requestedModuleId} registry moduleId mismatch`, {
      code: "registry_mismatch",
    });
  }
  if (!PHASE_PATTERN.test(registry.phaseId ?? "")) {
    throw new VerificationError(`${requestedModuleId} has invalid phaseId`, {
  ...
  for (const taskId of expectedAtomicTaskIds(requestedModuleId)) {
    if (!actual.has(taskId)) {
      throw new VerificationError(
        `${requestedModuleId} missing atomic verification entry ${taskId}`,
        {
          code: "atomic_task_missing",
  ...
  if (
    !registry.atomicTasks?.every(
      (task) => typeof task.evidence === "string" && task.evidence.trim(),
    )
  ) {
    throw new VerificationError(`${requestedModuleId} has atomic tasks without evidence`, {
      code: "evidence_missing",
    });
  }
  if (!Array.isArray(registry.commands) || registry.commands.length === 0) {
    throw new VerificationError(`${requestedModuleId} has no executable commands`, {
...
async function updateVerificationState(root, update) {
  ...
  const next = update({
    ...
    moduleEvidence: { ...(current.moduleEvidence ?? {}) },
    phaseEvidence: { ...(current.phaseEvidence ?? {}) },
  });
  ...
  try {
    ...
    const diff = execFileSync(
      ...
      [
        ...
        "--",
        ".",
        ":(exclude)verification/state.json",
        ":(exclude).artifacts/**",
      ],
    ...
    const untracked = execFileSync("git", ["ls-files", "--others", "--exclude-standard"], {
      ...
      .split(/\r?\n/u)
      .filter(Boolean)
      .filter((path) => path !== "verification/state.json" && !path.startsWith(".artifacts/"))
      .sort();
    ...
    return {
      commitSha,
      worktreeDirty: diff.length > 0 || untracked.length > 0,
      worktreeFingerprint: fingerprint.digest("hex"),
    };
  } catch {
    return { commitSha: null, worktreeDirty: true, worktreeFingerprint: null };
  }
}

export function completionEvidenceReusable(evidence, identity) {
  return Boolean(
    evidence &&
    identity?.commitSha &&
    evidence.commitSha === identity.commitSha &&
    ...
    evidence.worktreeFingerprint === identity.worktreeFingerprint,
  );
}

async function markModuleCompleted(moduleId, root, report, identity) {
  return updateVerificationState(root, (state) => ({
    ...
    moduleEvidence: {
      ...state.moduleEvidence,
      [moduleId]: {
        ...identity,
        completedAt: report.finished_at,
        evidenceSummary: {
          runId: report.run_id,
          status: report.status,
          passedCommands: report.commands.filter((item) => item.status === "passed").length,
          atomicTaskCount: report.atomic_tasks.length,
        },
      },
    },
  }));
...
async function markPhaseCompleted(phaseId, root, report, identity) {
  return updateVerificationState(root, (state) => ({
    ...state,
    ...
    phaseEvidence: {
      ...
      [phaseId]: {
        ...
        evidenceSummary: {
          ...
          reusedModules: report.reused_modules,
        },
      },
    },
  }));
...
export async function assertModuleDependencies(
  registry,
  root = process.cwd(),
  ...
  const missing = (registry.dependencies ?? []).filter(
    (id) =>
      !(state.completedModules ?? []).includes(id) ||
      !completionEvidenceReusable(state.moduleEvidence?.[id], identity),
  );
  if (missing.length) {
    throw new VerificationError(
      `[${registry.moduleId}] prerequisite modules are not completed: ${missing.join(", ")}`,
      {
...
export function runVerificationCommand(command, { root = process.cwd(), timeoutMs = 120000 } = {}) {
  ...
  return {
    ...
    status:
      result.status === 0 && !timedOut
        ? "passed"
        : result.status === 2 && !timedOut
          ? "blocked"
          : "failed",
...
export async function runModule(moduleId, options = {}) {
  ...
  const identity = options.identity ?? repositoryIdentity(root);
  const registry = await loadModuleRegistry(moduleId, root);
  await assertModuleDependencies(registry, root, identity);
  ...
  if (status !== "passed") {
    throw new VerificationError(`[${moduleId}] module verification failed`, {
      code: failure?.code,
      status,
      details: { report_file: reportFile, failure },
    });
  }
  await markModuleCompleted(moduleId, root, report, identity);
  return report;
}
...
export async function modulesForPhase(phaseId, root = process.cwd()) {
  ...
  const planFile = files.find(
    (file) => file.startsWith(`phase-${phaseNumber}-`) && file.endsWith(".md"),
  );
  if (!planFile) {
    throw new VerificationError(`[${phaseId}] phase plan is missing`, {
...
export async function assertPhaseDependencies(
  phaseId,
  root = process.cwd(),
  identity = repositoryIdentity(root),
) {
  ...
  const missing = Array.from(
    ...
    (id) =>
      !(state.completedPhases ?? []).includes(id) ||
      !completionEvidenceReusable(state.phaseEvidence?.[id], identity),
  );
  if (missing.length) {
    throw new VerificationError(
      `[${phaseId}] prerequisite phases are not completed: ${missing.join(", ")}`,
      {
        code: "phase_dependency_blocked",
        status: "blocked",
...
export async function runPhase(phaseId, options = {}) {
  ...
  let caught = null;
  try {
    await assertPhaseDependencies(phaseId, root, identity);
    const moduleIds = await modulesForPhase(phaseId, root);
    const state = await loadVerificationState(root);
    for (const moduleId of moduleIds) {
      if (
        (state.completedModules ?? []).includes(moduleId) &&
        completionEvidenceReusable(state.moduleEvidence?.[moduleId], identity)
      )
        reusedModules.push(moduleId);
      else reports.push(await runModule(moduleId, { ...options, config, identity }));
    }
  } catch (error) {
    caught = error;
  }
  ...
  if (!caught) await markPhaseCompleted(phaseId, root, report, identity);
  const reportFile = await writeReport(report, config);
  if (caught) {
    throw new VerificationError(`[${phaseId}] phase verification ${status}`, {
      code: report.failure.code,
      status,
      details: { report_file: reportFile, failure: report.failure },
    });
  }
  return report;
}
  ...
  try {
    ...
    for (let index = 0; index <= 8; index += 1) {
      ...
      if (
        ...
        completionEvidenceReusable(state.phaseEvidence?.[phaseId], identity)
      )
        reusedPhases.push(phaseId);
      else phases.push(await runPhase(phaseId, { ...options, config, identity }));
    }
  } catch (error) {
    caught = error;
  }
