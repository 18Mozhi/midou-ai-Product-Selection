...
const output = `output/playwright/p46-field-semantics/${mode}`;
...
const previous = execFileSync("git", ["show", `52099bc3:${registry}`], {
  encoding: "utf8",
}).replaceAll("\r\n", "\n");
assert.equal(hash(previous), "87728f892d895f3be4ecb0ecf016125745ea99fc9898f4d3c8c1845013b19a90");
assert.equal(source.split("</script>")[0], previous.split("</script>")[0]);
...
const sources = new Set([
  ...
  "scripts/verify-ui-phase2-provider-field-semantics.mjs",
]);
...
try {
  ...
  console.log("p46_fields_host " + origin);
  ...
  for (const width of [390, 760, 761, 1440]) {
    ...
    try {
      ...
      check("no review body marker", await page.locator("body").getAttribute("class"), null);
      ...
      for (let step = 0; step < groups.length; step++) {
        ...
        const inspect = async (phase) => {
          for (let i = 0; i < groups[step].length; i++) {
            ...
            if (!baseline) {
              ...
              check(
                `${phase}:${key} old no description`,
                await control.getAttribute("aria-describedby"),
                null,
              );
              check(
                `${phase}:${key} old no invalid`,
                await control.getAttribute("aria-invalid"),
                null,
              );
            }
        ...
        for (let i = 0; i < groups[step].length; i++) {
          const { key } = groups[step][i],
            label = fields.nth(i),
            control = label.locator("input,select");
          if (await label.locator("small").count()) {
            await control.focus();
  ...
  if (capture) {
      ...
      JSON.stringify(
        {
          ...
          scope:
            "Real App route with supplied GET only; 23 field names and 20 existing error bindings. No writes, no screen-reader audio certification, no full modal or production acceptance.",
        },
        null,
        2,
      ) + "\n",
    ...
    await writeFile(
      output + "/index.html",
      '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P46字段交互</title><style>body{font:16px/1.6 sans-serif;margin:24px}img{max-width:100%}article{margin:32px 0}</style><h1>P46 字段名称与错误关联 · ' +
        mode +
