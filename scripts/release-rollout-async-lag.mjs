const readyAge = (table, ready, extra = "", leased = "status='leased'") => ({
  table,
  sql: `SELECT COALESCE(MAX(GREATEST(0,TIMESTAMPDIFF(SECOND,CASE WHEN ${leased} THEN lease_expires_at ELSE available_at END,NOW(3)))),0) lag_seconds FROM \`${table}\` WHERE (${ready})${extra}`,
});
...
export const RELEASE_ASYNC_QUEUE_PROBES = Object.freeze([
  readyAge(
    ...
    "(status IN ('queued','retry_scheduled') AND available_at<=NOW(3)) OR (status='leased' AND lease_expires_at<=NOW(3))",
  ),
  readyAge(
    "collection_tasks",
    "(status IN ('scheduled','queued','retry_scheduled','rate_limited') AND available_at<=NOW(3)) OR (status IN ('leased','running') AND lease_expires_at<=NOW(3))",
    "",
    "status IN ('leased','running')",
  ),
  readyAge(
    "trend_projection_jobs",
    "(status='scheduled' AND available_at<=NOW(3)) OR (status='leased' AND lease_expires_at<=NOW(3))",
  ),
  readyAge(
  ...
  readyAge(
    "sourcing_outbox",
    "(status='queued' AND available_at<=NOW(3)) OR (status='leased' AND lease_expires_at<=NOW(3))",
  ...
  readyAge(
    "outbox_events",
    "(status='pending' AND available_at<=NOW(3)) OR (status='leased' AND lease_expires_at<=NOW(3))",
