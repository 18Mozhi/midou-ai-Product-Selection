...
- M02-03 壳层合同：公开根路径与登录/MFA 成功后先调用 `/me/landing`，按平台角色、当前组织管理员角色、成员上下文进入 `/platform-admin`、`/org-admin`、`/home` 或 `/select-context`；完全没有活动组织成员资格的有效账号进入账号级 `/me`，可维护本人资料、会话与 MFA，但不得访问任何组织业务数据。三套业务导航继续由服务端审计放行。平台管理员进入平台壳层不依赖组织上下文，也不显示成员专用搜索与通知入口。壳层仅展示中文业务角色名；平台菜单按业务运营、采集与数据、治理与安全、高级运维分层，长菜单支持按入口或分组搜索，故障关联编号收进折叠详情。
...
- M02-06 首页合同：首页只读取当前组织、工作区、当前用户受众和 capability 过滤后的真实数据；今日行动直接合并本人任务、当前审批节点和待决策机会，并与已有投影按 `逾期 > 阻断 > 高风险 > 高价值 > 普通`、风险、价值和时限排序且最多五项。健康提示必须精确影响当前用户；变化、关注和健康尚无投影时保持诚实空状态，移动机会详情不生成缺失数据。
...
M04-02 实现合同：`opportunities`、`opportunity_rule_matches`、趋势证据引用、人工决策、刷新任务、事件和 Outbox 全部按组织与工作区隔离。候选可手工创建、从当前工作区的活动趋势主题转入，或由 M04-01 中至少命中一条已启用监控规则的商品证据自动发现；通用新闻、数据、社区频道以及未命中规则的商品不得自动生成候选。自动发现写精确规则关联、`opportunity.candidate.discovered` 审计与 Outbox，后续同主题信号只增补证据和新增规则关联。人工创建与决策沿用既有 `opportunity:decide`，要求同源校验、幂等键和事务审计。宝塔 Node Worker 只把已持久化的趋势信号链接为市场证据，并将仅有趋势证据的覆盖状态标记为 `partial`；机会列表展示命中规则数，并按持久化 `coverage_status` 提供不完整、部分完整、完整筛选，不以利润或风险字段推断证据完整度。机会详情的业务血缘只读聚合当前组织与工作区内已持久化的来源、采集任务、尝试、证据、质量问题、趋势、评分、利润、任务和通知，逐节点保留 `request_id`、`trace_id`、观测时间及真实失败状态，缺失节点不得补造。存在显式启用的 M04-03 评分规则时，新候选自动排队评分；M04-03 至 M04-06 未提供足够评分、费用、竞品与风险输入前，综合分、趋势分、竞争分和置信度保持空值，利润为 `insufficient_data`、风险为 `unknown`，不得生成默认推荐或可靠 ROI。系统可自动给出满足既有证据阈值的推荐，但采纳、继续观察和驳回必须由人工填写原因并形成独立决策记录，不能改写原始分数、证据或历史。
