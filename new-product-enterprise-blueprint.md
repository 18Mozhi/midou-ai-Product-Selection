...
- M02-06 首页合同：首页只读取当前组织、工作区、当前用户受众和 capability 过滤后的真实数据；首屏直接提供自动选品规则的首次创建或暂停恢复，用户只需配置商品关键词、排除词、市场、分类、周期和进入推荐所需独立来源数。今日行动直接合并本人任务、当前审批节点和待决策机会，并与已有投影按 `逾期 > 阻断 > 高风险 > 高价值 > 普通`、风险、价值和时限排序且最多五项。健康提示必须精确影响当前用户；变化、关注和健康尚无投影时保持诚实空状态，移动机会详情不生成缺失数据。
