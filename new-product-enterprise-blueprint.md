...
- 接口覆盖证据基线（2026-09-01）：`/platform-admin/api-coverage` 仅作为平台超级管理员的二级发布排障工具，不进入主导航；每次读取同提交 OpenAPI、六角色目录、接口元数据和最近一次宝塔生产验收报告。只有报告 schema、223/256 数量与完整操作目录 SHA-256 同时匹配时才计入实时覆盖，按角色、成功/空结果/受阻/越权、数据来源、UI 消费方和爬虫副作用展示。缺失或旧报告必须显示 0% 与待补充，不得用静态目录冒充运行通过。
