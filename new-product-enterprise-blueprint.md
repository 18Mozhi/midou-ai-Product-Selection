...
- 接口覆盖看板基线（2026-08-23）：`/platform-admin/api-coverage` 每次读取同提交 OpenAPI、六角色目录、接口元数据和最近一次宝塔生产验收报告；只有报告 schema、222/255 数量与完整操作目录 SHA-256 同时匹配时才计入实时覆盖，按角色、成功/空结果/受阻/越权、数据来源、UI 消费方和爬虫副作用展示。缺失或旧报告必须显示 0% 与待验收，不得用静态目录冒充运行通过。
