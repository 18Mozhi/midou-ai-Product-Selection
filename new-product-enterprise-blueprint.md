...
M08-02 将同一台服务器上的宝塔 Redis 固定为 S0 单实例韧性边界：仅监听本机并保持 protected-mode，启用 AOF everysec 且保留 RDB save 规则，设置 `maxmemory 512mb`、`maxmemory-policy noeviction` 与 `maxclients 512`。平台运维 API 和桌面/390px 页面只返回脱敏的持久化、内存、连接、拒绝/淘汰与恢复结论，要求 `platform:operate` 并同步写入 MySQL 5.7 审计。75%/90% 水位分别告警和失败关闭；生产完成声明必须有宝塔配置备份、受控重启、PING、随机组织范围读写清理、API/Worker/Crawler 恢复和同提交证据。上述上限是资源保护配置，不是容量承诺；不启用 Sentinel、Redis Cluster、副本或备用服务器。
