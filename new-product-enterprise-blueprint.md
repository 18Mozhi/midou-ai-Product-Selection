...
- M02-01 视觉治理基线（2026-08-23）：三套主题统一使用中文界面字体栈、控件/卡片/弹窗三级圆角和单一强调色家族；深海主题降低荧光青与紫色渐变，极光紫只作为用户主动选择的个性主题，云雾白提高边框与次级文字对比度，组织与平台后台禁用装饰性光晕。正文与表单控件继续保持 16px，元数据与辅助说明不得低于 13px，所有密度和 390/768/1024/1440 断点保持 44px 触控目标。
...
- 生产页面验收：普通成员、选品经理、组织管理员、平台运营管理员、安全管理员和平台超级管理员必须使用角色互斥的受限账号；按 `/me/navigation` 真实能力覆盖 Route Catalog 全部授权静态路由，并从真实列表打开任务和机会详情。只遍历侧栏、角色叠加、前端推测权限或用占位对象代替动态详情均不得通过。
