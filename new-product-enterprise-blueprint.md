...
M07-05 发布合同以 `infra/baota/release-rollout-manifest.json` 锁定 4101 稳定 API、4103 同机候选 API、宝塔手工发布任务和 Nginx 5% → 25% → 100% 分流。生产每阶段至少观察 30 分钟，候选读写 P95、5xx、MySQL 异步延迟和最小样本全部来自运行证据；写样本使用宝塔受限 HMAC 签名、唯一 sample/nonce、单次 InnoDB 提交并保留 request_id/trace_id 的 M07-05 专用探针，不能用多次业务提交或幂等回放替代。缺样本或超过阈值必须自动把流量退回 4101 并追加停止/回滚审计。候选项目不是备用服务器，也不提供主机故障保护、多节点或 10,000 用户能力。
