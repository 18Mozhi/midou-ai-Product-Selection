...
- 统一响应：`data`、`meta`、`request_id`；错误响应含 `code`、`message`、`action_hint`、`request_id`。前端统一请求客户端负责 Cookie 会话、`request_id`/`trace_id`、JSON 信封、写请求幂等键和业务错误分类；导航权限、壳层与独立主题偏好、首页、全局发现、登录与会话、租户选择、落地解析、个人中心和接口就绪入口不得绕过该客户端直接请求。
