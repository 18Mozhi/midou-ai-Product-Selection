...
1688 专用验收页以最新真实浏览器作业建立搜索、详情、翻页覆盖矩阵：搜索和详情只有在对应版本快照实际解析通过时才算覆盖；翻页以执行计划上限和真实 `page_count` 为准，计划仅允许一页时必须显示“未演练”。矩阵用于暴露覆盖缺口，不替代登录态、验证码和当前解析器固定样本三项启用门，也不得因首屏成功宣称分页已覆盖。
