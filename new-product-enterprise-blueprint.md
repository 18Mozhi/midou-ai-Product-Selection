...
- 统一响应：`data`、`meta`、`request_id`；错误响应含 `code`、`message`、`action_hint`、`request_id`。前端统一请求客户端负责 Cookie 会话、`request_id`/`trace_id`、JSON 信封、写请求幂等键和业务错误分类；导航权限、壳层与独立主题偏好、首页、全局发现、登录与会话、租户选择、落地解析、个人中心、接口就绪、任务、趋势、评分规则、成本规则、机会、竞品、供应链、浏览器采集运行、采集调度、单机运行拓扑和容量边界入口不得绕过该客户端直接请求。
...
千行级前端页面必须按独立业务面板逐步拆成可测试子组件；拆分只移动渲染与交互边界，原页面继续拥有数据请求和业务编排，不得借组件拆分改变 API、状态机或计算规则。机会详情的利润与成本面板作为首个强制边界，父页面与该子面板分别保持在 1000 行和 200 行以内。
...
- E2E API 基线必须至少保留一条不使用浏览器路由拦截的真实链路，由浏览器经 Vite 代理访问实际 Fastify `buildApp()`；测试只可注入内存依赖检查，不得用手写 HTTP 假响应替代 API 路由栈，也不得连接生产数据库。
