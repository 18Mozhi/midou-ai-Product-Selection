...
- 接口覆盖证据基线（2026-09-02）：`/platform-admin/api-coverage` 仅作为平台超级管理员的二级发布排障工具，不进入主导航；每次读取同提交 OpenAPI、六角色目录、接口元数据和最近一次宝塔生产验收报告。schema 3 把 256 个操作按 `method_path_v1` 固定为唯一 `operation_id`，逐项保留正常、鉴权、参数、幂等和故障测试 ID 与最近结果；未执行维度必须显示 `not_run`。只有报告 schema、ID 策略、223/256 数量与完整操作目录 SHA-256 同时匹配时才计入覆盖，缺失或旧报告显示 0%，不得用静态目录或一次阻断探针冒充各维度通过。
