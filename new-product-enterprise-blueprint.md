...
`1688_search` 的浏览器提取结果必须按版本化合同进入来源规范化层：搜索、商品详情、供应商分别使用 `1688.search.v1`、`1688.offer-detail.v1`、`1688.supplier.v1`，当前解析器版本为 `1688-browser-contract-v2`。合同只接受 HTTPS 的 1688 规范链接，搜索入口限定 `s.1688.com`；当前搜索页商品卡以 `data-tracker="offer"` 标识，浏览器仅从卡片移动详情链接的数字 `offerId` 建立 `detail.1688.com/offer/{id}.html` 规范链接，不保存或跟随 HTTP 移动详情地址。搜索合同覆盖标题、供应商、报价、币种、MOQ、地点和规范链接；详情合同补规格与交期；供应商合同独立保存供应商名称、地点和规范链接。每条输出必须保留观测时间、最多 250 KB 的 DOM 证据片段及必需字段路径，缺失事实写入 `missing_fields_json`，不得补零或猜值。

1688 专用验收页以最新真实浏览器作业建立搜索、详情、翻页覆盖矩阵：搜索和详情只有在对应版本快照实际解析通过时才算覆盖；翻页以执行计划上限和真实 `page_count` 为准，计划仅允许一页时必须显示“未演练”。解析器门必须与来源启用事务读取同一事实，只接受活动样本最近一次持久化回放在 Provider 当前解析器版本上通过且已获第二人审批；旧版本回放必须明确提示重新创建、回放并审批当前版本样本，不能误报为只差审批。矩阵用于暴露覆盖缺口，不替代登录态、验证码和当前解析器固定样本三项启用门，也不得因首屏成功宣称分页已覆盖。
