...
- M02-03 壳层合同：公开根路径与登录/MFA 成功后先调用 `/me/landing`，按平台角色、当前组织管理员角色、成员上下文进入 `/platform-admin`、`/org-admin`、`/home` 或 `/select-context`；三套导航继续由服务端审计放行。平台管理员进入平台壳层不依赖组织上下文，也不显示成员专用搜索与通知入口。壳层仅展示中文业务角色名；平台菜单按业务运营、采集与数据、治理与安全、高级运维分层，长菜单支持按入口或分组搜索，故障关联编号收进折叠详情。
