...
固定样本必须来自同时具备结构化快照、截图、DOM 片段与一致解析版本的成功真实浏览器作业，客户端不能提交任意样本 JSON。平台保存不可变快照、基线、哈希和逐次回放差异；样本由另一名 `provider:configure` 管理员审批，创建人不能自审，驳回样本保留审计且修正必须创建新样本。1688 只在至少一个审批通过的活动样本对当前解析版本的最近一次回放为通过时允许启用。当前真实 Playwright 尚未生成结构化快照，因此回放与审批基础设施已交付但没有合格生产候选，来源继续停用。
