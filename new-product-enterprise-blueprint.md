...
- 桌面端必须采用“顶部身份/全局搜索/主操作/通知/个人菜单 + 横向模块索引 + 账页主内容”的信号账页壳层；组织后台和平台后台使用各自权限过滤的模块索引，不能与普通成员菜单混用。840px 及以下切换为抽屉和四入口加“更多”的底部导航。
- 移动端底栏固定为四个当前角色高频入口加“更多”；“更多”抽屉必须继续提供成员全局搜索和创建选品入口，不能因底栏收敛而让核心动作不可达。底栏与抽屉触控目标均不得小于 44 像素，页面内容必须为固定底栏保留安全间距。
- 建立可切换的设计令牌纸张：保留兼容 ID `deep-ocean`、`aurora-purple`、`cloud-white`，用户名称分别为“信号纸”（默认）、“档案纸”、“净页白”。三套均使用纸张底色、墨色正文、朱砂主动作、酸性绿信号与核验绿状态；不得改变权限、数据范围、功能可见性或业务结论，选择仍保存到用户偏好。
- M02-01 实现合同：主题偏好按用户、当前活动组织和当前活动工作区保存，采用版本锁、幂等键和同步审计；未保存时返回兼容 ID `deep-ocean` 对应的“信号纸”，不新增异步服务或生产环境变量。生产样式直接消费语义令牌或组件内语义别名，不保留 `!important` 覆盖层。正文与表单控件的浏览器计算字号不得低于 16 像素；元数据不得低于 13 像素；按钮、表单字段、按钮角色和移动导航的计算触控区域不得小于 44×44 像素，并由桌面和移动端 E2E 校验实际尺寸。
- M02-02 页面合同：登录、注册、找回、验证、重置、组织/工作区选择复用 P01 的真实接口与数据范围；三步快速引导不落库。登录品牌入口保持至少 44px 触控高度，移动端辅助账号入口以两列 44px 触控格降低纵向占用。图片中的手机、Google、Microsoft 与 SSO 入口在能力未启用前不得伪装可用。
- M02-03 壳层合同：公开根路径与登录/MFA 成功后先调用 `/me/landing`，按平台角色、当前组织管理员角色、成员上下文进入 `/platform-admin`、`/org-admin`、`/home` 或 `/select-context`；完全没有活动组织成员资格的有效账号可在 `/select-context` 幂等创建仅属于本人的“我的选品空间”和默认工作区，系统同时赋予该成员组织管理员角色、组织范围并选中当前会话，不能借此创建任意命名组织、添加其他成员或访问其他组织。三套业务导航继续由服务端审计放行。平台管理员进入平台壳层不依赖组织上下文，也不显示成员专用搜索与通知入口。壳层仅展示中文业务角色名；平台菜单按业务运营、采集与数据、治理与安全、高级运维分层，长菜单支持按入口或分组搜索，故障关联编号收进折叠详情。
- M02-03 导航连续性基线（2026-08-23）：成员、组织、平台与个人中心继承同一主题，后台仅切换紧凑密度，不得强制改为云雾白；顶栏显示当前中文身份并保留可达壳层切换，组织后台可直接返回最近成员页，平台返回成员页仍先选择组织与工作区。限定浏览器存储只保存安全同源的最近成员路径、最近有效路径、最多五个组织 ID 与主题 ID，不保存身份、权限、查询结果或凭证；根路径可恢复最近成员页，404 可返回最近有效页。面包屑只链接真实上级路由，移动二级页高亮“更多”且移动顶栏保留当前角色主操作。平台组织列表、创建和详情分别使用 `/platform-admin/organizations`、`/platform-admin/organizations/new`、`/platform-admin/organizations/{organizationId}`；竞品监控规则使用 `/competitors/monitoring-rules` 独立路由。
- M02-01 视觉治理基线（2026-09-05）：生产界面统一采用“Signal Ledger / 信号账页”：零圆角、无漂浮卡片墙、无蓝紫渐变，以边框、纸张层级和排版建立深度；中文宋体类展示字用于页标题，界面字体用于正文，等宽字体用于元数据。主内容上限 1440px；每页只突出一个朱砂主动作，酸性绿只标识需关注信号，核验绿只表达通过状态。确认弹窗使用“影响清单 + 签认”结构；正文与表单控件保持 16px，元数据与辅助说明不低于 13px，所有密度和 390/768/1024/1440 断点保持 44px 触控目标。
- M02-01 Signal Ledger 2.1 基线（2026-09-07）：成员业务页进一步压缩面包屑、页码标题和上下文占高；桌面保留四栏上下文账条，840px 及以下只显示可展开的当前范围摘要。主动作、次动作、静默动作和危险动作必须使用显式语义类，不再依赖按钮文案或 DOM 位置推断；核心工作流容器、确认表单和移动全屏层保持零圆角、无阴影、无渐变。
- M02-04 状态合同：加载、空、错误、无权限、过期、受阻、恢复、404 与确认弹窗使用统一文字、符号、下一步及安全关联标识；高影响确认默认失败关闭。组件不推断权限、不自行写 API，也不新增数据库、异步服务或环境变量。
- M02-05 搜索合同：全局搜索固定在服务端解析的当前组织与工作区，并可按真实对象类型、对象状态、任务负责人或机会负责人筛选；没有负责人合同的对象不得把创建人伪装成负责人。快捷创建按显式 `shell` 校验并返回已授权、可直接打开真实表单的页面入口，先按壳层角色排序，再在当前 SPA 内将最多五个最近使用入口前置，不持久化行为画像，平台壳层不要求组织上下文。搜索投影由资源所属模块在真实事务成功后同步维护，本模块不新增异步生产者或环境变量。
- M02-06 首页合同：首页只读取当前组织、工作区、当前用户受众和 capability 过滤后的真实数据；首屏直接提供自动选品规则的首次创建或暂停恢复，用户只需配置商品关键词、排除词、市场、分类、周期和形成规则命中候选所需独立来源数。今日行动直接合并本人任务、当前审批节点和待决策机会，并与已有投影按 `逾期 > 阻断 > 高风险 > 高价值 > 普通`、风险、价值和时限排序且最多五项。健康提示必须精确影响当前用户；变化、关注和健康尚无投影时保持诚实空状态，移动机会详情不生成缺失数据。
- M02-06 首屏任务基线（2026-09-07）：首页先呈现唯一主动作和待人工处理的推荐清单，再呈现运行计数与自动化详情；技术运行说明保持渐进披露。移动端主动作独占首行，两个次动作并列，首个真实待办在固定底栏上方可见且可操作。
...
M04-01 实现合同：`trend_topics`、`trend_signals`、主题关键词、关注和监控规则全部按组织与工作区隔离。手动 Google News、96 条固定市场/主题频道、Amazon 公开 `/s` 搜索页及代码目录中已登记的非 Google RSS/Atom/公开页频道规范化证据由宝塔 Node Worker 租约任务投影为主题；Amazon 记录只有在监控规则子查询中才进入自动选品链。市场、语言和类目来自代码内 Provider 目录，同一市场、语言和规范化标题合并，多来源不得覆盖。投影 Worker 的队列编排、输入计算、事务写入和监控告警分别维护，但仍在同一主循环与原事务合同内运行，不新增生产服务。热度仅表示实际信号数并携带 `signals` 单位；在没有已批准的测量规则前，环比和置信度返回 `insufficient_data`，不得填充默认分数。关注、标记无关、创建或暂停规则要求 `trend:manage`、同源校验、幂等键、事务审计和 Outbox；标记无关不删除证据。只有命中至少一条已启用监控规则的商品型主题才可调用 M04-02 合同生成待评估选品，并持久化精确规则关联；存在显式启用的评分规则时可自动排队评分，但不得伪造输入、分数或利润，最终采纳始终由人工完成。
...
M04-02 实现合同：`opportunities`、`opportunity_rule_matches`、趋势证据引用、人工决策、刷新任务、事件和 Outbox 全部按组织与工作区隔离。候选可手工创建、从当前工作区的活动趋势主题转入，或由 M04-01 中至少命中一条已启用监控规则的商品证据自动发现；通用新闻、数据、社区频道以及未命中规则的商品不得自动生成候选。自动发现写精确规则关联、`opportunity.candidate.discovered` 审计与 Outbox，后续同主题信号只增补证据和新增规则关联。人工创建与决策沿用既有 `opportunity:decide`，要求同源校验、幂等键和事务审计。宝塔 Node Worker 只把已持久化的趋势信号链接为市场证据，并将仅有趋势证据的覆盖状态标记为 `partial`；自动补偿竞品与供应链采集任务时，必须分别把已有竞品和找货投影按组织、工作区及对象键一次聚合后再关联候选，不得为每个候选重复跨租户扫描全部 JSON 目标并持有事务锁。至少一条当前启用的精确命中规则达到用户显式选择的 1–10 个独立持久化来源门槛后，页面只能显示“规则命中候选”，不得仅凭来源数量把 `recommendation_status` 写成 `recommend`。只有评分结论为 `recommend` 且评分覆盖完整、市场分与竞争分已有真实输入、利润成本已计算、最新评分运行中的风险维度有真实证据时，读模型才显示“建议采纳”并允许人工采纳；任一门未通过仍保持“规则命中候选”。机会列表展示命中规则数，并按持久化 `coverage_status` 提供不完整、部分完整、完整筛选，不以利润或风险字段推断基础证据完整度。机会详情的业务血缘只读聚合当前组织与工作区内已持久化的来源、采集任务、尝试、证据、质量问题、趋势、评分、利润、任务和通知，逐节点保留 `request_id`、`trace_id`、观测时间及真实失败状态，缺失节点不得补造。存在显式启用的 M04-03 评分规则时，新候选自动排队评分并以版本化评分结果为准；M04-03 至 M04-06 未提供足够评分、费用、竞品与风险输入前，综合分、趋势分、竞争分和置信度保持空值，利润为 `insufficient_data`、风险为 `unknown`，不得生成可靠 ROI。最终采纳始终只由人工对“建议采纳”商品填写原因并形成独立决策记录，不能改写原始分数、证据或历史。

决策后的经营复盘必须把实际销量与销售额、广告花费、退货量、采购交期和实际利润作为不可变事实回写，并快照当时的评分规则、利润规则、预测利润、报价交期与人工决策。系统可展示同币种利润偏差、报价交期偏差、退货率和广告投入占比供人校准规则与提示，但不得自动改规则、重算历史事实或替代人工决策。
...
M04-03 实现合同：评分规则不预置或推断业务权重、推荐阈值和观察阈值，创建者必须显式提交 0–100 维度权重（合计 100）及阈值。输入按机会、维度、来源、观测时间、版本和证据 ID 不可变保存；缺失值必须列出缺失字段，不能按 0 分或默认分补齐。规则状态依次为草稿、待审批、已批准、已启用，并支持拒绝、停用和回滚；提交使用既有 `opportunity:decide`，批准、启用与回滚使用既有 `opportunity:approve`，所有写入要求同源校验、幂等键、版本锁、审计事件和 Outbox。草稿、待审批和已批准但未启用的规则须提供发布前机会影响预览；预览与生产 Worker 共用计算函数，按页展示当前分数、试算分数、分差、结论变化、覆盖率和缺失项，只读取当前持久化输入，不创建评分运行、Job、审计、Outbox，也不更新规则或机会。宝塔 Node Worker 按租约执行版本化加权计算：必需输入覆盖率低于 50% 时综合分和置信度保持 `insufficient_data`；达到 50% 可形成观察性结果；只有覆盖率至少 80% 且市场、竞争、成本三类各有真实证据时，评分门才可产出 `recommend`。评分门结论不等于用户可见的“建议采纳”；后者还必须同时通过 M04-02 的市场、竞争、成本和风险质量门。回滚只改变后续任务使用的活动规则，既有输入、分项、总分、缺失项和运行快照保持可复现。

自动评估可使用版本化确定公式把 Amazon 评价数、评分、库存、新鲜度、Provider/数据质量状态、竞品快照和利润运行转换为 0–100 输入，但公式版本和全部证据 ID 必须随输入保存；公式不提供业务权重或推荐阈值，也不得为缺字段生成分数。当前 `crawler-evidence-v1` 只支持市场需求、竞争、利润、风险和数据质量维度，活动规则要求其他维度时必须保留缺失状态。

M04-04 实现合同：费用规则按组织、工作区、市场和平台版本化保存，平台费、支付手续费、税费和履约成本四项必须由创建者显式填写，不提供或推断默认费率；规则可额外显式填写固定入仓物流。规则提交后必须分别由真实 `selection_manager` 和 `organization_admin` 角色审批，双审批完成才可发布；拒绝、换版和回滚保留全部审批与历史运行。人工含税售价、采购价和物流输入必须保存金额、ISO 4217 币种、来源、证据、观测时间和不可变版本，并由另一名 `cost:confirm` 成员复核；自动成本输入只允许来自机会自身 Amazon 售价证据以及至少三条同商品族、同型号且版本兼容的 1688 高置信报价，采购价取匹配组保守 75 分位并标记 `automatic_evidence`，不得覆盖更新的人工当前输入。跨币种换算优先接受已启用且声明 `exchange_rate` 能力的 Provider 历史报价；没有报价时只允许使用同一双角色审批规则中明确保存的币种对、数值、生效日期和 HTTPS 来源页面。宝塔 Node Worker 固定按“含税售价 − 采购 − 物流 − 平台费 − 支付手续费 − 税费 − 已知履约成本”计算净利润和净利率；任何必需输入或汇率缺失时状态必须为 `insufficient_data`，净利润、总成本和 ROI 数值保持空值。规则、成本输入、汇率、任务、分项和运行均保留 request_id、trace_id、审计事件与 Outbox；既有利润运行不可改写。
...
竞品 URL 同时携带搜索词和竞品 ID 时，合法的深链竞品详情必须优先恢复且保持可见；用户随后主动修改搜索词时清除旧竞品参数，再按新条件展示匹配结果或可恢复的空态。

竞品详情将每条持久化变化的影响结论、按 `change_id` 关联的告警通知状态和下游任务状态合并为处理时间轴；没有关联告警的变化必须明确标为未命中阈值，不得推断告警。价格与库存快照时间轴保持独立，避免把采集事实与处理状态混写。
...
M04-06 实现合同：找货请求明确记录关键词、图片、机会或商品链接之一作为输入类型；未引用既有任务时，由 `dhgate_supplier_search`、`made_in_china_search` 与 `ec21_supplier_search` 三个独立公开页 Provider 建立同一范围的可审计子查询，单一来源出现验证码、robots 拒绝、限流或结构变化时如实受阻或失败，其他来源继续执行。现行真实运行以 robots 允许且可解析的 DHgate 批发页作为可用主来源，Made-in-China 与 EC21 保留为独立降级证据；既有 `manual_product_supply_csv` / `product-supply-csv-v1` 仍可投影，不虚构 1688 登录态或外部接口。供应商名称、标题、MOQ、报价、币种、原始页面、观测时间和证据来自不可变采集事实；规格、交期、所在地、可信度、稳定性和风险缺失时保持 `incomplete` 并逐项展示，只有采购成员使用证据显式确认后才创建新的不可变报价版本。对比仅接受同组织、同工作区内当前有效的 2–5 家完整报价，并按统一字段顺序展示供应商、规格、MOQ、报价与交期；采购任务必须满足报价 MOQ，并以 `queued` 事件和 Outbox 交给 P05 任务中心消费，不能显示为已执行。当前生产找货仅使用公开页执行器，未定义 `blocked_login` 搜索状态或续期任务关联，因此页面不得伪造登录续期入口。读取使用 `sourcing:read`，找货、报价、对比和采购任务写入使用 `supplier_quote:manage`；所有写入要求同源校验、幂等键、审计、request_id 和 trace_id。物流、平台费与汇率来源继续使用 M04-04 已交付的版本化成本与利润合同，不在本模块复制或推断。

机会成本提交必须指定另一名活动 `cost:confirm` 复核人；提交记录在复核通过前不成为当前成本，也不触发基于新值的利润重算。复核期限为 24 小时，截止前 4 小时提醒复核人，逾期升级到复核人与组织管理员，但系统不自动通过；驳回保留不可变输入和结论审计。

报价比较在规格文本不一致时必须提示采购成员先核对型号、容量、包装和计量单位；系统只能识别大小写、空白和全半角格式差异，不自动换算单位，也不判断两种规格语义等价。
...
- 组织审批模板治理保持只读，并以当前版本对比最近的上一持久化版本；按节点顺序展示新增、移除、名称、审批人、SLA 和超时接收人的真实差异。首版和无差异版本必须明确说明，不得用名称猜测节点身份，也不得从组织后台绕过工作区审批合同发布或回滚。
...
- 发布门禁必须使用 MySQL 5.7 中真实的 `roles`、`role_capabilities` 与 `platform_role_assignments` 核对三种固定平台角色，并以隔离临时账号对全部能力逐项验证允许和拒绝结果及 `authorization_decisions`；不得用页面可见按钮或前端角色常量代替服务端权限实测。
...
- 驾驶舱 MySQL 读查询按平台规模、采集运行、质量风险和文件容量四个指标域拆分；主仓储只编排并组装既有响应语义，并继续独占读取记录与审计事务。拆分不改变管理写接口、数据库结构或运行服务。
...
- 链路日志页把最新 200 条 API 审计、Worker 任务和 Crawler 运行事件按真实 `trace_id` 分组，组内按发生时间正序展示；异常 Worker 事件直达已持久化任务，异常 Crawler 事件只在浏览器作业与 Provider 关联存在时直达任务和来源，不为 API 或多来源 Worker 事件猜测单一来源。受控 CSV 导出必须复用页面当前查询与运行面筛选及同一 200 条上限，防公式注入并审计操作者、筛选、原因和记录数；页面与导出都不复制原始日志，也不返回 metadata、payload、凭证、Cookie 与 stderr。
...
- API Client 创建、轮换和撤销写入前展示权限风险预览，明确组织范围、真实 scope、分钟配额及旧密钥失效或访问终止后果；没有权威风险分级规则时只展示事实，不生成高/中/低标签。
...
- 配额方案、组织分配、账期和人工调整写入前必须展示影响范围：方案变更列出仍分配该方案的组织数及配额前后值，组织级变更列出目标组织、有效配额和余量前后值；新账期用量只能在写入后按事实重算，不得在预览中猜测。
...
停用的 `public_page` 或 `public_rss` 正式启用前必须对刚保存的目标与超时配置执行真实页面烟测并完成一次有效解析；成功健康记录必须不早于该配置版本。管理页采用“先保存停用配置 → 真实烟测 → 版本锁启用”的两阶段流程，失败保持停用，配置 API 与历史版本回滚不能绕过。登录型来源仍以真实登录运行和当前解析器固定样本作为启用依据，导入型来源不伪造页面烟测。

来源配置保存前展示拟议频率下的同频启用来源数量，并展示当前 Provider 的待执行/执行中子查询数与配置并发上限。没有逐来源调度相位时只能称为潜在同窗，不能宣称必然冲突；当前占用不外推为未来容量或 SLA，也不自动改变频率与并发。
...
以下 RSS/Atom 地址在计划编写时返回 XML 或 RSS；上线时由来源健康检查验证 HTTP 状态、内容类型、字段质量与解析结果。robots 和条款复核不阻塞开发或本地固定样本测试；生产公开来源必须先保存负责人批准状态和真实条款 HTTPS 参考地址，Worker 每次进入适配器前执行同源 robots 检查，明确禁止时以 `robots_disallowed` 受阻且不得绕过。公开来源的子查询完成事件必须记录 robots 判定器版本、HTTP 状态、命中 User-agent 和最终胜出的 Allow/Disallow 规则；规则只保存有界预览与完整 SHA-256，旧任务缺少判定时不得反推。
...
6. RSS 无新内容是成功状态 `succeeded_empty`，不是失败；任务明细按子查询完成事件进一步区分合法 Feed 零条目的 `empty_success`、成功解析但全部命中既有证据的 `no_new_content`，以及载荷未通过解析合同的 `parse_failed`，不得把解析失败伪装成空成功。

主题人工合并与拆分采用双人确认：提案人与确认人必须是不同的活动 `trend:manage` 用户。合并保留目标主题并归档来源主题，整合信号、关键词和关注；多个既有机会发生身份冲突时拒绝执行，单个机会改绑到目标。拆分只迁移明确选择的信号，要求显式命名新主题，既有机会留在原主题且新主题不自动创建机会。

### 5.3 商品、竞品、供应链与社区来源目录

| ID               | 网站/端点                                                                   | 方式                 | 采集/读取内容                                       | 用户入口                              | 技术前提                                   |
| ---------------- | --------------------------------------------------------------------------- | -------------------- | --------------------------------------------------- | ------------------------------------- | ------------------------------------------ |
| `amazon_product` | `https://www.amazon.com/s?k={keyword}` / `https://www.amazon.com/dp/{asin}` | 公开页面爬虫         | 标题、品牌、价格、货币、类目、评价、图片、变体      | 规则自动选品、ASIN/商品 URL、竞品监控 | 每个市场独立限流、公开页解析与字段回放     |
| `keepa`          | `https://api.keepa.com/`                                                    | 付费 API             | 价格历史、销量排名历史、Offer、类目                 | 机会/竞品详情                         | API Key、额度、成本与字段映射检查          |
| `1688_search`    | `https://s.1688.com/selloffer/offer_search.htm?keywords={keyword}`          | 已认证浏览器爬虫任务 | 供应商、标题、规格、MOQ、报价、地点、链接           | 供应链找货、图片找货                  | 平台资产浏览器档案、字段回放、登录健康检查 |
...
任务中心以组织/工作区范围的 `tasks`、不可变评论与任务事件为事实源。任务状态固定为待处理、进行中、已暂停、完成、取消；SLA 只由真实 `due_at` 派生，缺少期限显示 `not_set`。创建、状态、延期、转交和评论使用幂等键与版本锁，并在同一事务写审计和 Outbox。列表支持先预览范围再批量暂停、继续、取消、延期或调整负责人；每个任务继续使用自身版本、独立幂等键和独立审计，批量转交只能选择当前工作区活动成员。详情固定展示最近一次持久化暂停原因与当前负责人对应的下一负责人，不从描述或评论猜阻塞事实。页面首屏采用单一事实焦点条和状态队列，不并列堆放同权指标卡；状态数量进入筛选标签，搜索与排序按需展开。任务详情只把当前有效状态动作、完成和进度更新放在主操作区，延期、转交、编辑、取消和删除归入具名的更多操作，并保证桌面及 390px 移动端均不越界；收起时二级动作必须真实隐藏。前端由任务工作台保留路由与 API 编排，并把列表、批量操作、详情分别拆为独立组件；拆分不改变状态机、版本锁或接口字段。M04-06 采购任务由宝塔 Node Worker 通过租约投影，来源唯一键保证重放不重复；审批、通知消费和 SSE 由 M05-02 至 M05-04 后续模块交付。

M05-01任务读取生命周期（2026-09-07）：今日工作、任务列表和详情被壳层缓存后，只允许当前激活且路由归属匹配的实例读取。返回缓存页按当前URL重新读取一次；筛选、详情ID和业务/导出视图由同一监听入口编排。新的读取或离开页面会取消旧GET；成功、失败、分页总量及成员目录都校验读取归属，迟到响应不得覆盖新状态。写请求不随切页取消或自动重放；操作后的详情刷新同时完成任务事实和成员目录。本人/工作区范围、URL字段、权限、状态机、幂等与版本合同不变，无新增运行配置。

#### M05-02 审批流程实现基线

审批模板采用草稿与不可变已发布版本分离；每个节点显式保存审批人、顺序、SLA 和超时接收人。审批请求只绑定当前组织、当前工作区内存在的任务或机会决策，并锁定创建时的模板版本。机会决策审批在发起时同时锁定市场证据、评分运行、利润运行、风险识别、评分/利润规则版本与申请决策原因；详情以同一事实查询器只读比较提交快照与当前上下文，展示完整度、证据检查、决策依据和规则版本的逐项变化，再提供批准或驳回动作。历史审批若早于快照字段，只能明确显示当前事实回退且不提供伪造的差异，不得伪装成发起时快照；任务审批没有独立证据规则时显示“不适用”，不得伪造满分完整度。节点时间线固定展示原审批人、当前审批人、超时接收人和升级时间，形成可核对的“原审批人 → 超时接收人”路径，但不得把升级显示为代理批准。审批页首屏采用单一待决焦点条，只保留待处理、逾期和已发布模板等可核对事实；“待我处理/我发起的”和状态筛选合并为一个队列面板，空状态只给出当前可执行的下一步。没有已发布模板时只引导配置模板，存在已发布模板后才显示发起审批；队列行优先展示决策对象、当前节点、期限和证据完整度，详情继续承担完整判断依据。批准与驳回均要求原因并追加不可变动作历史；请求 version 防止并发覆盖。宝塔 Node Worker 以租约处理超时节点，只升级 active approver 并写审计和 `approval.overdue` Outbox，绝不自动批准或驳回；通知投递和 SSE 仍由 M05-03、M05-04 交付。
...
报表导出由唯一宝塔 Node Worker 按全局 `available_at + id` 顺序租约领取。API 只返回当前组织和工作区的导出记录，但为活动记录提供其在真实全局领取序列中的 1 基位置。预计完成时间仅以最近最多 20 次成功导出的 `created_at -> updated_at` 完整耗时中位数顺序推算，并返回样本数；没有成功样本时保持 `null`，不得使用轮询间隔、租约时长或固定常量伪造 ETA。报表页和任务中心复用同一队列事实；估算不是 SLA，重试、依赖故障和报表行数变化仍按真实状态更新。
...
P01 首个模块实现本地已验证邮箱账号、自有会话及可选唯一用户名登录别名：注册、邮箱验证、邮箱或用户名登录、登出、找回/重置密码、改密及会话查看/撤销。现有账号在个人中心主动设置用户名之前继续仅以邮箱登录；注册、验证和密码找回仍以邮箱为可信渠道。密码使用 Argon2id；随机会话与动作令牌只存哈希，生产会话仅通过 Secure、HttpOnly、SameSite=Strict Cookie 传递。验证/重置投递负载以 AES-256-GCM 加密写入 MySQL Outbox，由宝塔管理的 Node Worker 以租约、重试、死信和审计处理。生产邮件 Provider 尚未确认时必须显式阻断投递，不把原始令牌返回页面，也不把阻断误报为成功。手机号、工号登录及 MFA 与 OIDC/SAML/SCIM 仍由独立模块实现，组织与权限不在本模块提前创建。
...
`authenticated_browser` 使用项目依法持有的账号和 M03-02 加密浏览器档案。Node Worker 是业务 `collection_tasks` 的唯一领取者，并为登录型子查询写入与任务、子查询关联的 `browser_collection_jobs`；Python Crawler 从内部服务接口领取浏览器作业，通过无 shell 插值的 stdin/stdout 桥接调用 Node Playwright Chromium，再把有界结果回写到原业务子查询。Worker 必须把 `succeeded` 与 `succeeded_empty` 浏览器作业都作为终态读取，不能把 Crawler 已完成的空结果继续轮询成超时。搜索滚动造成动态列表短暂回到骨架屏时，Playwright 要求同一正数条目数以 250 ms 间隔连续稳定三次，且不超过既有动作超时，再生成数量、快照和证据；瞬时旧卡片或一次非零计数不能提前结束等待。无待处理作业时返回 204，不发送空闲心跳，也不读取静态执行请求文件。执行计划仅接受 HTTP(S)、明确 origin 白名单和有上限的搜索、分页、滚动及详情动作；登录、验证码、robots、429、超时、Parser 变化和依赖失败必须如实受阻或失败，禁止绕过登录、验证码、付费墙和站点限制。档案以受限 `tar.gz` 临时解包，拒绝路径穿越、链接和资源超限，并在全部结果路径关闭 context、清空 Buffer 和删除准确临时目录。
...
MySQL 5.7 是采集任务、子查询、执行尝试、租约、死信、事件和 Outbox 的事实源；Redis 只保存按组织/工作区隔离的就绪信号与短租约，不能决定任务完成。调度与领取使用行锁，Worker 仅持有原始租约令牌，数据库保存带域分离的 SHA-256 摘要。Worker 的单轮编排、状态机、已持久化子查询结果汇总与死信处理分别维护，但继续运行在同一宝塔 Node Worker 和原事务合同内，不新增生产服务，也不替代 M03-06 原始证据持久化。租约到期或 Redis 协调冲突必须显式回收，不能让任务永久停留在 leased。浏览器运行页的登录档案到期预警只依据明确凭证有效期和服务端观测时间；活动档案显示占用实例与来源，过期租约只标为“僵尸占用风险”并进入受控回收，不虚构操作系统进程探测结果。
...
来源 API 仓储按目录、固定样本与配置版本划分 SQL 所有权：兼容门面保留人工回放和全量刷新事务，目录仓储维护代码目录同步与登记，样本仓储维护 1688 验收和固定样本回放，版本仓储维护配置历史、更新与回滚。拆分不新增服务、表、路由或配置，也不改变既有幂等、审计、事务和错误合同。

#### 1688 浏览器输出合同纠偏基线（2026-08-19）

`1688_search` 的浏览器提取结果必须按版本化合同进入来源规范化层：搜索、商品详情、供应商分别使用 `1688.search.v1`、`1688.offer-detail.v1`、`1688.supplier.v1`，当前解析器版本为 `1688-browser-contract-v3`。合同只接受 HTTPS 的 1688 规范链接，搜索入口限定 `s.1688.com`；当前搜索页商品卡以 `data-tracker="offer"` 标识，浏览器仅从卡片移动详情链接的数字 `offerId` 建立 `detail.1688.com/offer/{id}.html` 规范链接，不保存或跟随 HTTP 移动详情地址。一次任务最多执行两页、每页最多提取 15 条并按 `offerId` 去重，翻页点击当前页码容器内未禁用的下一页箭头，且必须等到 SPA URL 或首商品身份变化后再读取。任务最多打开一个由规范 URL 导航的详情页，提取真实标题、供应商与店铺 ID、规格、可见报价、MOQ 和发货地；页面只给出的目的地到货预计不得冒充供应商交期。搜索合同覆盖标题、供应商、报价、币种、MOQ、地点和规范链接；详情合同补规格与交期；供应商合同独立保存供应商名称、地点和规范链接。每条输出必须保留观测时间、最多 250 KB 的 DOM 证据片段及必需字段路径，缺失事实写入 `missing_fields_json`，不得补零或猜值。

1688 专用验收页以最新真实浏览器作业建立搜索、详情、翻页覆盖矩阵：搜索和详情只有在对应版本快照实际解析通过时才算覆盖；翻页以执行计划上限和真实 `page_count` 为准，当前两页计划只有在实际完成两页时才算覆盖，少于两页必须显示“未演练”。解析器门必须与来源启用事务读取同一事实，只接受活动样本最近一次持久化回放在 Provider 当前解析器版本上通过且已获第二人审批；旧版本回放必须明确提示重新创建、回放并审批当前版本样本，不能误报为只差审批。矩阵用于暴露覆盖缺口，不替代登录态、验证码和当前解析器固定样本三项启用门，也不得因首屏成功宣称分页已覆盖。
...
固定样本必须来自同时具备结构化快照、截图、DOM 片段与一致解析版本的成功真实浏览器作业，客户端不能提交任意样本 JSON。平台保存不可变快照、基线、哈希和逐次回放差异；周期性 1688 商品观测使用“任务 + 外部商品 ID”的证据键并保留稳定外部商品 ID，使页面证据或观测时间变化形成可审计版本而不是跨任务去重冲突。样本由另一名 `provider:configure` 管理员审批，创建人不能自审，驳回样本保留审计且修正必须创建新样本。1688 只在至少一个审批通过的活动样本对当前解析版本的最近一次回放为通过时允许启用。当前真实 Playwright 已按页面 GBK 搜索表单和带 `offerId` 的商品卡生成有界 `1688.search.v1` 快照；明确标记的 1688 验收任务可穿过该来源由历史失败留下的运行熔断做受控恢复探测，普通任务和其他来源仍失败关闭。是否存在合格生产候选仍以部署后的真实登录验收作业为准，固定样本回放与第二人审批完成前来源继续停用。

来源中心的运营信息以持久化事实为准：负责人来自 Provider 定义，最近成功任务来自完成的来源子查询，影响范围只组合来源类别、市场与字段集合。页面按“市场热点与消费者信号 / 商品与竞品观察 / 供应链找货”三个业务用途组织真实来源，开发验收说明留在技术文档和专用检查页。当前模型没有独立 SLA 字段，因此界面只展示“沿用采集计划”的更新目标，不推断达标率；待实施和手动来源显示未设自动 SLA，所有未接通来源统一标为待实施。
...
- 当前目录共 181 项，自动层共 138 个频道：96 个固定 Google News RSS、40 个非 Google RSS/Atom 新闻/电商/社区/趋势频道，以及 Amazon、eBay 2 个固定公开榜单页面爬虫，覆盖 24 个独立主机；其余 37 个待配置来源和 6 个手动来源不会冒充自动采集成功。Walmart 公开端点当前返回人工验证页，因此只保留为受控页面来源。Worker 为每个活动组织默认工作区循环自动调度，热点页也提供 `trend:read` 保护的立即刷新。
...
千行级前端页面必须按独立业务面板逐步拆成可测试子组件；拆分只移动渲染与交互边界，原页面继续拥有数据请求和业务编排，不得借组件拆分改变 API、状态机或计算规则。机会工作台已经将列表、阻断与决策、利润与成本、创建与导入弹窗拆为独立展示边界；趋势工作台已经将筛选、详情和证据时间线拆为独立展示边界；供应链工作台已经将找货编排、报价对比和成本确认表单拆为独立展示边界；平台账号已经按组织、普通用户、管理员和服务端权限对比拆分，来源中心已经将来源配置和固定样本回放拆为独立展示边界；对应父页面均保持在 1000 行以内，各子组件保持在对应门禁上限内。
...
前端构建必须先执行严格 `vue-tsc` 再进入 Vite 打包，类型错误直接阻断 `build:web`；`apps/web/tsconfig.json` 启用 `incremental` 与 `noEmit`，只保存被 Git 忽略的 `.web.tsbuildinfo` 编译状态，不生成第二份浏览器产物。全仓 `npm run build` 从 `apps/*`、`packages/*` 的真实包依赖生成拓扑层，拒绝缺失构建脚本与循环依赖，每层最多并行四个编译进程，并在整层成功后才进入下游 API、Worker 或 Web；Python Crawler 继续走独立 Python 门禁，不伪装为 Node workspace。
...
- E2E API 基线必须至少保留两条不使用浏览器路由拦截的本地 Fastify 链路，由浏览器经 Vite 代理访问实际 `buildApp()` 并生成有效 PNG，至少覆盖就绪状态和发布身份；测试只可注入内存依赖检查，不得用手写 HTTP 假响应替代 API 路由栈，也不得连接生产数据库。一次性宝塔验收另以不拦截请求的浏览器覆盖“趋势→机会决策→任务”和“找货候选→报价确认→成本/利润”，且执行前必须由 `/health/ready` 证明真实 MySQL 与 Redis 可用。被真实生产链截图替代的机会详情与找货 Mock 只保留功能断言；匿名鉴权相关 MFA 场景也只保留行为断言。带 `page.route` 的文件中截图调用占全部截图调用的比例必须低于 50%；按 2026-08-23 机器清单为 15/33（45.45%），这些 Mock 截图只能作为视觉回归。成员与平台移动壳层必须复用零重叠遮挡检查。
- 全量 E2E 门必须把 desktop-chromium 与 mobile-390 作为两个独立 Playwright 进程依次执行，每个进程分别启动并关闭 Fastify/Vite 测试服务；不得共享服务生命周期、浏览器上下文或进程内状态，桌面失败后不得继续用同一服务跑移动端。
...
- 生产页面验收：普通成员、选品经理、组织管理员、平台运营管理员、安全管理员和平台超级管理员必须使用角色互斥的受限账号；按 `/me/navigation` 真实能力覆盖 Route Catalog 全部授权静态路由，并从真实列表打开任务和机会详情。只遍历模块索引、角色叠加、前端推测权限或用占位对象代替动态详情均不得通过。
- 路由验收真相基线（2026-08-23）：`config/route-catalog.json` 是前端 Route Catalog、Feature Map 路由段、六角色生产遍历和允许/拒绝回归矩阵的唯一机器可读来源。新增受保护路由必须同时声明壳层、能力、Feature Map 元数据；动态路由还必须声明从真实父列表解析持久化对象的规则。`npm run verify:route-artifacts` 在生成物漂移、六角色目录缺失或动态解析规则缺失时失败关闭。
- 生产验收租户基线（2026-08-23）：宝塔手工一次性任务创建六个邮箱已验证、角色互斥的账号和一个隔离组织/工作区，写入带同一 trace 的趋势、机会、采集证据、找货候选、成本输入、利润运行与任务；从同提交 OpenAPI 自动覆盖 222 个路径、255 个操作，再从同一路由目录覆盖 60 条受保护路由和 360 个角色允许/拒绝单元。通用 API 写覆盖只允许安全受阻探测；只有机器清单授权的两条核心 E2E 可提交有效业务写，并且必须通过真实 Fastify/MySQL/Redis、不得使用浏览器请求拦截。任务成功或失败都必须删除账号、会话、上下文、审计和关联业务数据，逐表输出删除计数并证明剩余引用为零。
- 接口覆盖证据基线（当前源码复核 2026-09-23；替代 2026-09-02 历史目录计数）：`/platform-admin/api-coverage` 仅作为平台超级管理员的二级发布排障工具，不进入主导航；每次读取同提交 OpenAPI、六角色目录、接口元数据和最近一次宝塔生产验收报告。当前 OpenAPI 源解析为 225 个路径、258 个操作，按 `method_path_v1` 固定唯一 `operation_id`；逐项保留正常、鉴权、参数、幂等和故障测试 ID 与最近结果，未执行维度必须显示 `not_run`。只有报告 schema、ID 策略、当前 225/258 数量与完整操作目录 SHA-256 同时匹配才可关联，缺失或旧报告的运行结果归零；运行记录包含受阻/拒绝，不等于成功率，报告可关联也不等于验证维度通过。不得用静态目录或一次阻断探针冒充各维度通过。

生产前端不得注册或打包 `/ui-states` 与自动验收框架；二者只允许在本地开发构建中作为组件合同工具使用。1688 启用检查同样不进入主导航，仅允许 `platform:superadmin` 从来源详情的二级入口进入；普通 `platform:operate` 与 `provider:configure` 账号对两个检查页面及其接口均失败关闭。
...
M07-05 发布中心只对带 `timing_schema=2` 的发布门展示迁移与自动回滚耗时：迁移从第一条迁移执行前到全部迁移完成，回滚从开始切回稳定流量到 Nginx reload 完成。旧记录、未发生动作和不完整或逆序时间区间必须显示未知，不得按 0 秒、观察时长或整轮发布时长代替。
...
2026-09-09 用户确认 M07-06/P16 采纳与 P18 统一质量门：必须关联同组织/工作区已评估且待决策的机会，满足有效规则及来源门槛、评分/市场/竞争/成本/风险五门，事务内重检后方可采纳。不得由有主题的采集候选直接生成未知分数机会并采纳，不改写已评估证据覆盖；观察、驳回、原始证据、历史决策与幂等重放保持。此为本地规则实施，旧生产验收不覆盖新增采纳门。
...
系统状态页同时把现有 API、MySQL、Redis、文件、Worker 与 Crawler 事实按访问入口、共享依赖和异步执行组织成依赖拓扑；异常节点只提示“如异常持续”时应核查的业务面与关联服务，不把拓扑关系冒充已发生的下游故障，也不生成故障数量、可用性或容量结论。运行拓扑的队列监控保留基础/有效优先级、老化间隔、最大老化增益与实际调度延迟；仅在队列达到自身最大老化增益后仍未运行时提示饥饿风险，不另设猜测阈值。运行监控由现有宝塔 Node API 进程按固定列表连续探测本机 `live`、`ready`、`available` 三个健康端点，只持久化状态码、耗时、超时/网络结果和 correlation，不保存响应正文；控制台展示配置窗口内的样本量、超时量、可用率及 P50/P95/P99。该进程内探测不能证明已经完全停止的事件循环仍可用，整进程失联继续以运行节点心跳过期为准。
...
- 浏览器只保存当前旅程 ID；页面重开后必须从服务端重新读取租户范围内的任务、候选、阻断和决策状态，不能用浏览器缓存代替业务事实。
- 旅程页首次挂载只读一次；KeepAlive停用或卸载时停止轮询并使在途读取失效，重新激活时重新读取服务端事实。旧读取的成功、失败及404均不得覆盖新的页面状态或清除其活动ID。“开始下一次”使旧读取失效但不取消后台采集；恢复失败的显式重试继续读取保存的ID，不自动创建任务。读取中不允许创建或保存决定，写入中不允许再次提交或重置；不取消或自动重放写请求。跨标签存储竞争和离开期间写请求归属仍需独立验收，不由读取回归代替。
- 同一采集任务最多返回 20 条范围化候选供成员比较。多条候选必须明确选择后才能采纳；只有“采纳”会生成机会，“观察”和“拒绝”只保存旅程决策。
- 生产验收阈值继续作为后端与运维门禁保留，不在普通成员页面展示内部状态码、计时阈值或验收说明。

### 15.2 今日行动跨模块聚合

- 今日行动直接合并当前用户负责的未完成任务、当前节点待审批事项和本人待决策机会，不能依赖尚无生产者维护的首页行动投影。
- 排序保持 `逾期 > 阻断 > 高风险 > 高价值 > 普通`，同级再按持久化风险、机会评分和截止时间比较；无评分或无截止时间时保持未知，不填充默认业务值。
- 每个行动必须链接到现有任务详情、审批详情或机会详情。阻断项的主动作明确为打开完整处理上下文。

### 15.3 趋势异常证据进入质量闭环（2026-08-22）

- 趋势详情的每条真实信号可直接报告异常，创建或返回既有未关闭的数据质量工单。
- 工单的组织、工作区、来源、原始证据、规范化记录和解析器版本全部由服务端根据当前主题信号推导，前端只提交风险等级与异常说明。
- 写入沿用数据质量域的幂等操作、审计事件和 Outbox；同一原始证据不重复创建未关闭的趋势异常工单。

### 15.4 机会阶段、阻断解除与重新决策提醒（2026-08-22）

- 机会列表同时展示当前阶段、负责人和独立持久化的阶段停留时长，普通内容更新不得重置阶段计时。
- 机会详情集中展示采纳阻断项，并用真实补采任务进度与评分任务状态说明解除进度。
- 补采任务触发的评分完成后，向机会负责人或任务执行人发送站内“重新决策”提醒并直达机会详情。

### 15.5 数据质量下钻与评分阻断（2026-08-22）

- 核对运行按真实运行编号下钻到异常字段、关联证据样本和 Parser 版本；没有逐样本核对存储时不得按样本量生成虚构明细。
- 评分 Worker 在写入新评分运行前核对当前输入证据。相同来源与 Parser 的最近核对失败，或证据仍有开放严重质量问题时，终止本次评分并保留既有分数。
- 质量阻断通过评分 Job 错误、机会阻断摘要、事件和 Outbox 留痕；解决质量问题后重新创建评分 Job，不改写历史运行。

### 15.6 本机文件水位（2026-08-22）

- 文件韧性同时展示证据、导出和运行临时目录的真实文件系统水位；三者必须使用互不相同、位于网站根目录之外的宝塔受控路径。
- 临时目录不建立持久文件索引，不参与证据/导出的校验和与恢复副本结论；页面不得把临时文件数或体积伪装成已索引业务资产。

### 15.7 MySQL 与 Redis 风险解释（2026-08-22）

- MySQL 页面分开解释当前慢查询窗口速率、门禁状态、实例启动后的累计行锁等待和当前运行线程，不把累计计数描述为实时延迟。
- Redis 页面按 `maxmemory-policy`、内存水位、实例运行期和累计淘汰键解释淘汰风险。键空间占用热点只允许对 `scoutops:v1:*` 固定最多采样 128 个键，以 `MEMORY USAGE` 按代码白名单中的用途和资源类别聚合；原始键、租户范围与载荷不得返回或审计。
- 占用热点只表示受限样本的内存份额。当前 `noeviction` 策略没有可信 LFU 访问频率，必须明确返回访问频率不可用，不能把内存份额、最近访问或键数量包装成高频热键。

### 15.8 来源错误预算与恢复门（2026-08-22）

- 来源运行错误预算沿用各 Provider 已持久化的连续失败熔断阈值，展示当前连续运行失败、阈值和剩余额度；不新增统一百分比或时间窗。
- 运行熔断打开后，只有晚于开启时间的一次真实 `ready` 健康检查才满足恢复门。健康检查不自动解除熔断，平台运营人员仍须从采集调度执行现有幂等、审计恢复动作。
- 适配器健康探针的连续失败与真实采集运行连续失败保持两个事实，页面不得混算。

### 15.9 运行根因、队列与业务对象关联（2026-08-22）

- 背压、最近失败、卡死和熔断告警只关联调度快照中实际命中的队列；没有队列事实时保持空关联，不按故障类型扩散推断。
- Worker 业务结果只允许提取稳定状态、错误码和队列白名单中的业务对象 ID。最近一分钟的真实失败结果可关联采集任务、业务任务、机会等现有对象；原始结果载荷、凭证、任意 URL 和未知字段不得进入运行快照。
- 页面优先提供现有精确详情路由；对象类型没有稳定详情入口时只展示类型和脱敏短编号。没有精确对象时只按队列排查，不把最近一次成功对象冒充当前故障对象。
