...
- 统一响应：`data`、`meta`、`request_id`；错误响应含 `code`、`message`、`action_hint`、`request_id`。前端统一请求客户端负责 Cookie 会话、`request_id`/`trace_id`、JSON 信封、写请求幂等键和业务错误分类；导航权限、壳层主题偏好、首页与全局发现入口不得绕过该客户端直接请求。
