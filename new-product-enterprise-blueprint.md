...
M04-01 实现合同：`trend_topics`、`trend_signals`、主题关键词、关注和监控规则全部按组织与工作区隔离。手动 Google News、96 条固定市场/主题频道及代码目录中已登记的非 Google RSS/Atom/公开页频道规范化证据由宝塔 Node Worker 租约任务投影为主题；市场和语言来自代码内 Provider 目录，同一市场、语言和规范化标题合并，多来源不得覆盖。投影 Worker 的队列编排、输入计算、事务写入和监控告警分别维护，但仍在同一主循环与原事务合同内运行，不新增生产服务。热度仅表示实际信号数并携带 `signals` 单位；在没有已批准的测量规则前，环比和置信度返回 `insufficient_data`，不得填充默认分数。关注、标记无关、创建或暂停规则要求 `trend:manage`、同源校验、幂等键、事务审计和 Outbox；标记无关不删除证据。监控规则首批只支持站内通知，邮件 Provider 未确认时不得显示为已接通。商品型自动热点的新主题可调用 M04-02 合同生成有真实趋势证据的待评估选品，但不得自动评分、推荐、采纳或伪造利润。
...
M04-02 实现合同：`opportunities`、趋势证据引用、人工决策、刷新任务、事件和 Outbox 全部按组织与工作区隔离。候选可手工创建、从当前工作区的活动趋势主题转入，或由 M04-01 的爆款商品、Amazon、TikTok Shop、Etsy、eBay、新品发布自动频道发现；通用新闻、数据和社区频道不自动生成候选。自动发现写 `opportunity.candidate.discovered` 审计与 Outbox，后续同主题信号只增补证据。人工创建与决策沿用既有 `opportunity:decide`，要求同源校验、幂等键和事务审计。宝塔 Node Worker 只把已持久化的趋势信号链接为市场证据，并将仅有趋势证据的覆盖状态标记为 `partial`；机会列表按持久化 `coverage_status` 提供不完整、部分完整、完整筛选，不以利润或风险字段推断证据完整度。机会详情的业务血缘只读聚合当前组织与工作区内已持久化的来源、采集任务、尝试、证据、质量问题、趋势、评分、利润、任务和通知，逐节点保留 `request_id`、`trace_id`、观测时间及真实失败状态，缺失节点不得补造。M04-03 至 M04-06 未提供评分、费用、竞品与风险输入前，综合分、趋势分、竞争分和置信度保持空值，利润为 `insufficient_data`、风险为 `unknown`，不得生成默认推荐或可靠 ROI。采纳、继续观察和驳回必须填写原因并形成独立决策记录，不能改写原始分数、证据或历史。
