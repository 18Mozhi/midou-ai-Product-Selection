...
- 生产验收租户基线（2026-08-23）：宝塔手工一次性任务创建六个邮箱已验证、角色互斥的账号和一个隔离组织/工作区，写入仅供动态详情解析的一条机会与一条任务；从同提交 OpenAPI 自动覆盖 220 个路径、253 个操作，再从同一路由目录覆盖 59 条受保护路由和 354 个角色允许/拒绝单元。写操作只允许安全受阻探测，不提交有效生产业务载荷；任务成功或失败都必须删除账号、会话、上下文、审计和关联业务数据，逐表输出删除计数并证明剩余引用为零。
