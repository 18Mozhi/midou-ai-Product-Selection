...
M04-02 实现合同：`opportunities`、`opportunity_rule_matches`、趋势证据引用、人工决策、刷新任务、事件和 Outbox 全部按组织与工作区隔离。候选可手工创建、从当前工作区的活动趋势主题转入，或由 M04-01 中至少命中一条已启用监控规则的商品证据自动发现；通用新闻、数据、社区频道以及未命中规则的商品不得自动生成候选。自动发现写精确规则关联、`opportunity.candidate.discovered` 审计与 Outbox，后续同主题信号只增补证据和新增规则关联。人工创建与决策沿用既有 `opportunity:decide`，要求同源校验、幂等键和事务审计。宝塔 Node Worker 只把已持久化的趋势信号链接为市场证据，并将仅有趋势证据的覆盖状态标记为 `partial`；未产生评分规则结果的待决策机会，只有在至少一条当前启用的精确命中规则达到用户显式选择的 1–10 个独立持久化来源门槛后，才能由 `insufficient_data` 变为 `recommend`，规则暂停、恢复或证据刷新都必须重新评估，且不得生成分数、利润或风险结论。机会列表展示命中规则数，并按持久化 `coverage_status` 提供不完整、部分完整、完整筛选，不以利润或风险字段推断证据完整度。机会详情的业务血缘只读聚合当前组织与工作区内已持久化的来源、采集任务、尝试、证据、质量问题、趋势、评分、利润、任务和通知，逐节点保留 `request_id`、`trace_id`、观测时间及真实失败状态，缺失节点不得补造。存在显式启用的 M04-03 评分规则时，新候选自动排队评分并以版本化评分结果为准；M04-03 至 M04-06 未提供足够评分、费用、竞品与风险输入前，综合分、趋势分、竞争分和置信度保持空值，利润为 `insufficient_data`、风险为 `unknown`，不得生成可靠 ROI。采纳、继续观察和驳回必须由人工填写原因并形成独立决策记录，不能改写原始分数、证据或历史。
