...
- 移动端底栏固定为四个当前角色高频入口加“更多”；“更多”抽屉必须继续提供成员全局搜索和创建选品入口，不能因底栏收敛而让核心动作不可达。底栏与抽屉触控目标均不得小于 44 像素，页面内容必须为固定底栏保留安全间距。
- 建立可切换的设计令牌主题：`deep-ocean`（深海蓝，默认）、`aurora-purple`（极光紫）、`cloud-white`（云雾白）。主题只改变颜色、阴影、背景和图表语义色，不得改变权限、数据范围、功能可见性或业务结论；主题选择保存到用户偏好。
- M02-01 实现合同：主题偏好按用户、当前活动组织和当前活动工作区保存，采用版本锁、幂等键和同步审计；未保存时返回深海蓝默认值，不新增异步服务或生产环境变量。生产样式直接消费语义令牌或组件内语义别名，不保留全局兼容补丁，也不允许使用 `!important` 覆盖层。正文与表单控件的浏览器计算字号不得低于 16 像素；按钮、表单字段、按钮角色和移动导航的计算触控区域不得小于 44×44 像素，并由桌面和移动端 E2E 校验实际尺寸。
- M02-02 页面合同：登录、注册、找回、验证、重置、组织/工作区选择复用 P01 的真实接口与数据范围；三步快速引导不落库。图片中的手机、Google、Microsoft 与 SSO 入口在能力未启用前不得伪装可用。
- M02-03 壳层合同：公开根路径与登录/MFA 成功后先调用 `/me/landing`，按平台角色、当前组织管理员角色、成员上下文进入 `/platform-admin`、`/org-admin`、`/home` 或 `/select-context`；三套导航继续由服务端审计放行。平台管理员进入平台壳层不依赖组织上下文，也不显示成员专用搜索与通知入口。
- M02-04 状态合同：加载、空、错误、无权限、过期、受阻、恢复、404 与确认弹窗使用统一文字、符号、下一步及安全关联标识；高影响确认默认失败关闭。组件不推断权限、不自行写 API，也不新增数据库、异步服务或环境变量。
- M02-05 搜索合同：全局搜索固定在服务端解析的当前组织与工作区；快捷创建按显式 `shell` 校验并返回已授权、可直接打开真实表单的页面入口，平台壳层不要求组织上下文。搜索投影由资源所属模块在真实事务成功后同步维护，本模块不新增异步生产者或环境变量。
...
首屏只放“今日行动”和“异常与数据健康”，先帮助用户完成必须处理的工作；变化雷达与我的关注下移到后续关注区。首页不承载全链路教学，相关教学只在首次引导中出现。变化雷达的每条记录必须直接提供进入其真实站内路由的处理动作。
...
列表必须排在教学说明之前，教学只放在折叠帮助面板；每项直接展示来源数、来源新鲜度与可信度（没有测量规则时明确数据不足）。时间线可按真实 Provider 来源筛选，筛选标签使用来源名称而不向普通界面展示 UUID。“关注”“创建监控”“转为机会”是三种不同动作，不得合并为含义模糊的单一按钮。

M04-01 实现合同：`trend_topics`、`trend_signals`、主题关键词、关注和监控规则全部按组织与工作区隔离。手动 Google News、96 条固定市场/主题频道及代码目录中已登记的非 Google RSS/Atom/公开页频道规范化证据由宝塔 Node Worker 租约任务投影为主题；市场和语言来自代码内 Provider 目录，同一市场、语言和规范化标题合并，多来源不得覆盖。热度仅表示实际信号数并携带 `signals` 单位；在没有已批准的测量规则前，环比和置信度返回 `insufficient_data`，不得填充默认分数。关注、标记无关、创建或暂停规则要求 `trend:manage`、同源校验、幂等键、事务审计和 Outbox；标记无关不删除证据。监控规则首批只支持站内通知，邮件 Provider 未确认时不得显示为已接通。商品型自动热点的新主题可调用 M04-02 合同生成有真实趋势证据的待评估选品，但不得自动评分、推荐、采纳或伪造利润。
...
M04-02 实现合同：`opportunities`、趋势证据引用、人工决策、刷新任务、事件和 Outbox 全部按组织与工作区隔离。候选可手工创建、从当前工作区的活动趋势主题转入，或由 M04-01 的爆款商品、Amazon、TikTok Shop、Etsy、eBay、新品发布自动频道发现；通用新闻、数据和社区频道不自动生成候选。自动发现写 `opportunity.candidate.discovered` 审计与 Outbox，后续同主题信号只增补证据。人工创建与决策沿用既有 `opportunity:decide`，要求同源校验、幂等键和事务审计。宝塔 Node Worker 只把已持久化的趋势信号链接为市场证据，并将仅有趋势证据的覆盖状态标记为 `partial`；机会列表按持久化 `coverage_status` 提供不完整、部分完整、完整筛选，不以利润或风险字段推断证据完整度。M04-03 至 M04-06 未提供评分、费用、竞品与风险输入前，综合分、趋势分、竞争分和置信度保持空值，利润为 `insufficient_data`、风险为 `unknown`，不得生成默认推荐或可靠 ROI。采纳、继续观察和驳回必须填写原因并形成独立决策记录，不能改写原始分数、证据或历史。
...
M04-05 实现合同：竞品按组织、工作区、市场、来源站点和外部商品 ID 唯一保存，首个完整快照只建立基线。所有快照必须来自已启用 Provider，并包含价格、ISO 4217 币种、排名、评价数、评分、库存、抓取时间、新鲜度、来源状态、来源引用和证据 ID；缺字段不得补示例值。快照不可变，宝塔 Node Worker 使用租约、重试和死信比较上一快照，按固定格式保存变更；页面明确区分最早基线、当前快照、已记录变动与适用阈值，价格变动按证据 ID 使用对应快照币种并展示变化时间。价格、排名和评价阈值必须显式提供，库存只支持任意变化或变为缺货；达到阈值时在本模块可靠记录通知与任务 `queued` 状态并写 Outbox，由后续所属模块消费。读取使用 `competitor:read`，创建、快照、阈值和启停使用 `competitor:manage`；审计员保持只读。所有写入要求同源校验、幂等键、组织/工作区隔离、request_id、trace_id、审计事件与回滚说明。
...
M04-06 实现合同：找货请求只引用当前组织、当前工作区内已经完成的采集任务，并明确记录关键词、图片、机会或商品链接之一作为输入类型；本阶段只投影已启用 `manual_product_supply_csv` Provider 产生的 `product-supply-csv-v1` 规范化记录，不虚构 1688 或其他外部接口。供应商名称、标题、MOQ、报价、币种、原始页面、观测时间和证据来自不可变采集事实；规格、交期、所在地、可信度、稳定性和风险缺失时保持 `incomplete` 并逐项展示，只有采购成员使用证据显式确认后才创建新的不可变报价版本。对比仅接受同组织、同工作区内当前有效的 2–5 家完整报价，并按统一字段顺序展示供应商、规格、MOQ、报价与交期；采购任务必须满足报价 MOQ，并以 `queued` 事件和 Outbox 交给 P05 任务中心消费，不能显示为已执行。当前生产找货仅使用公开页执行器，未定义 `blocked_login` 搜索状态或续期任务关联，因此页面不得伪造登录续期入口。读取使用 `sourcing:read`，找货、报价、对比和采购任务写入使用 `supplier_quote:manage`；所有写入要求同源校验、幂等键、审计、request_id 和 trace_id。物流、平台费与汇率来源继续使用 M04-04 已交付的版本化成本与利润合同，不在本模块复制或推断。
...
#### 个人中心完整实现基线（2026-08-18）

- `/me` 使用成员工作台壳层，汇总版本化个人资料、服务端权限与数据范围、MFA、设备会话、改密、通知偏好、关注热点、本人决策和本人任务；全部数据来自当前账号与当前组织/工作区事实。
- 资料修改要求同源、`Idempotency-Key`、原因和乐观版本，手机号变化会清除验证状态；没有短信服务时不得显示已验证。邮箱仍沿用账号验证合同，不允许个人资料页绕过邮箱验证直接改登录邮箱。
- 组织 Token 只在拥有 `organization_token:manage` 时通过组织 Token 页面出现；普通成员不获得或看到 Token 管理入口。个人中心不新建浏览器 Token、官方 API Key 或面板外服务。
...
- 组织资料与成员管理保持独立路由；用户以工作区名称、成员邮箱和中文能力名称完成管理，UUID 仅作为既有 API 的内部值并收进折叠技术详情，不能作为主界面识别文本或要求用户手工填写。
...
#### 平台账号管理纠偏基线（2026-08-18）

- 平台超级管理员必须有可直接操作的“组织管理 / 用户管理 / 管理员管理”页面，不得只给组织后台或数据库表；创建组织同时创建默认工作区、首位组织管理员关系和组织级数据范围。
- 停用用户必须同步撤销活动会话；创建组织、启停组织/用户、平台角色授权均要求同源、幂等键、原因和平台审计。禁止当前超级管理员停用自己或撤销自己的超级管理员角色。
- 平台、组织管理员和普通成员继续使用三套导航。平台小白主导航只展示平台概览、组织与用户、热点来源、采集任务、数据质量、安全与审计和高级设置；基础设施详情不作为一级业务菜单。
...
- 错误根因卡明确标出网络、登录、验证码、解析四类采集告警；这只是固定用户界面分类，未知码显示“其他”，精确下钻、重放、重试和审计仍使用原始错误码，不按类别推断或改写失败。
- 控制台不返回凭证、Cookie、采集 payload 或文件路径。读取要求 `platform:operate` 并记录全局审计；重放仍要求 `collection:replay`、同源、原因与 `Idempotency-Key`。
- 批量安全重放只允许从当前结果明确勾选最多 20 条开放死信，先固定展示死信数、根因、组织数和工作区数，再要求影响确认和“确认重放”短语。执行逐条复用 M03-05 单任务重放事务，并以批次 UUID 与来源任务 UUID 派生独立幂等键；每条分别校验权限、状态和子查询，保留原任务历史并报告逐项成功/失败，不以批量入口覆盖或删除旧任务。
...
来源接入分为 `public_page`（公开页面爬虫）、`public_rss`（公开 RSS/Atom 抓取）、`authenticated_browser`（自有账号登录页面爬虫）、`import`（文件导入）和 `manual`（人工录入）。系统不使用需要官方 API 凭证的采集模式；原有 `official_api` 来源在迁移中统一转换为固定目标页面爬虫，并继续受超时、限流、重试、熔断和审计约束。编写、测试和部署爬虫代码，不要求项目所有者向开发人员提交授权书、合同、法务审核记录或来源准入证明；robots、网站条款和来源运营审核也不作为开发或本地测试的前置条件，最多作为生产启用时的可选管理信息。
...
以下 RSS/Atom 地址在计划编写时返回 XML 或 RSS；上线时由来源健康检查验证 HTTP 状态、内容类型、字段质量与解析结果。robots 和条款复核不阻塞开发或本地固定样本测试；生产公开来源必须先保存负责人批准状态和真实条款 HTTPS 参考地址，Worker 每次进入适配器前执行同源 robots 检查，明确禁止时以 `robots_disallowed` 受阻且不得绕过。
...
审批模板采用草稿与不可变已发布版本分离；每个节点显式保存审批人、顺序、SLA 和超时接收人。审批请求只绑定当前组织、当前工作区内存在的任务或机会决策，并锁定创建时的模板版本。机会决策审批在发起时同时锁定市场证据、评分运行、利润运行、风险识别、评分/利润规则版本与申请决策原因；详情先展示证据完整度、缺失项、规则版本和决策依据，再提供批准或驳回动作。历史审批若早于快照字段，只能明确显示当前事实回退，不得伪装成发起时快照；任务审批没有独立证据规则时显示“不适用”，不得伪造满分完整度。批准与驳回均要求原因并追加不可变动作历史；请求 version 防止并发覆盖。宝塔 Node Worker 以租约处理超时节点，只升级 active approver 并写审计和 `approval.overdue` Outbox，绝不自动批准或驳回；通知投递和 SSE 仍由 M05-03、M05-04 交付。
...
- 统一响应：`data`、`meta`、`request_id`；错误响应含 `code`、`message`、`action_hint`、`request_id`。前端统一请求客户端负责 Cookie 会话、`request_id`/`trace_id`、JSON 信封、原始文件响应、写请求幂等键和业务错误分类；全部前端业务模块不得绕过该客户端直接调用 `fetch`，自动测试必须递归扫描所有 TypeScript 与 Vue 源文件。
...
来源采集设置的每次变更都追加不可变 Provider 版本。配置中心只展示频率、超时、重试和启停状态的逐版差异；回滚必须携带当前版本锁和原因，并重新通过公开条款或登录解析验收，再把历史安全设置写成新的当前版本。不得覆盖历史快照，也不得把凭证、Cookie、宝塔受限配置值、操作人标识或关联标识下发到浏览器。
...
固定样本必须来自同时具备结构化快照、截图、DOM 片段与一致解析版本的成功真实浏览器作业，客户端不能提交任意样本 JSON。平台保存不可变快照、基线、哈希和逐次回放差异；1688 只在至少一个活动样本对当前解析版本的最近一次回放为通过时允许启用。当前真实 Playwright 尚未生成结构化快照，因此回放基础设施已交付但没有合格生产候选，来源继续停用。

来源中心的运营信息以持久化事实为准：负责人来自 Provider 定义，最近成功任务来自完成的来源子查询，影响范围只组合来源类别、市场与字段集合。当前模型没有独立 SLA 字段，因此界面只展示“沿用采集计划”的更新目标，不推断达标率；待实施和手动来源显示未设自动 SLA，所有未接通来源统一标为待实施。

采集质量的准确率不得合成为未定义总分。页面分别展示最近一次持久化核对中的标题、价格、币种、外部 ID、规范 URL 准确率、重复率和来源新鲜度，并保留阈值、样本量、样本不足与无数据状态；内部英文状态只在合同中保存，用户界面使用中文结论。
...
本地账号与 MFA 完成后的正式会话有效期统一为 30 天（`AUTH_SESSION_TTL_MINUTES=43200`），Cookie 使用 Secure、HttpOnly、SameSite=Strict、Path=/ 和 `Max-Age=2592000`；服务器仍以 MySQL 到期时间和会话状态为最终事实，停用用户或安全操作可提前撤销。
...
生产长期固定为**单机企业应用阶段（S0）**，不是已承诺的 10,000 用户高可用 SaaS：惠州 `192.168.1.220` 上由宝塔管理一个网站、一个名为“ai选品”的统一 Node 后端（内部监督 API、Worker 与采集执行通道）、MySQL 5.7、Redis 和本地受控文件目录。不得再创建独立 API、Worker、Canary 或 Python 常驻项目。当前决策不增加服务器、不启用负载均衡、Redis Sentinel、MySQL 只读副本、共享文件节点或横向 Crawler。容量与设备性能不属于产品功能完成门；未执行可选容量测量时始终显示 `capacity_claim=unverified`。同机加密备份与逻辑隔离恢复不保护整机、磁盘或机房故障，也不构成自动高可用或异地灾备。
...
P00–P08 当前按“单后端、真实首页、可稳定启动、功能完整”重新验收。旧运行 ID 和旧生产证据只作历史记录，不能替代当前代码、统一宝塔对象和真实浏览器流程；只有新的全量功能门与同提交生产核验通过后才可重新声明完成。
...
API 启动文件只负责共享依赖和生命周期，业务路由装配必须按工作流、平台管理、运行运维等边界拆分；域注册器只组合既有 Service、Repository 和 Route，不得借拆分改变 URL、权限或数据合同。自动测试必须阻止路由注册重新回流到单一启动文件。

千行级前端页面必须按独立业务面板逐步拆成可测试子组件；拆分只移动渲染与交互边界，原页面继续拥有数据请求和业务编排，不得借组件拆分改变 API、状态机或计算规则。机会详情、平台账号和来源中心已经分别将利润与成本、管理员记录、固定样本回放拆为独立展示边界；三个父页面均保持在 1000 行以内，各子组件保持在对应门禁上限内。
...
- E2E API 基线必须至少保留两条不使用浏览器路由拦截的真实链路，由浏览器经 Vite 代理访问实际 Fastify `buildApp()` 并生成有效 PNG，至少覆盖就绪状态和发布身份；测试只可注入内存依赖检查，不得用手写 HTTP 假响应替代 API 路由栈，也不得连接生产数据库。带 `page.route` 的文件中截图调用占全部截图调用的比例必须低于 50%，且这些 Mock 截图只能作为视觉回归；成员与平台移动壳层必须复用零重叠遮挡检查。
...
M07-06 可复用上述 ScoutOps 项目专用 HTTP CONNECT 代理访问代码目录中已登记的自动来源主机；代理主机白名单由固定来源目录生成，不接受成员或请求参数覆盖。代理配置不属于成员输入，普通成员验收账号也不得获得或读取代理凭证。生产签发仍以同一惠州单机、同一构建、固定 10 秒来源健康门和 180 秒旅程门为准；代理仅改变允许的出站传输路径，不改变来源、数据合同、权限或阈值，也不设置服务器全局代理。
